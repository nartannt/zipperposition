...
let update feature value =
    match feature with
        | Counter name -> StrTbl.replace cnt_map name (StrTbl.get_or cnt_map name ~default:0 + value)
        | Vector name -> StrTbl.replace vec_map name (value :: StrTbl.get_or vec_map name ~default:[])
        ...
        match T.view t with
            ...
            | Bind (b, v, t) ->
                if Binder.equal b Binder.Lambda then (
                  ...
                  let var_order =
                      (if snd @@ T.Ty.arity @@ Var.ty v != 0 then 1 else 0) + T.Ty.order (Var.ty v)
                  in
    ...
    CCVector.iter
      (fun stm ->
        (match Statement.view stm with
            ...
            | _ -> CCFormat.fprintf CCFormat.stderr "Warning: %a is ignored." Statement.pp_input stm);
...
let process file =
    ...
    let res =
        ...
        let input = Parsing_utils.input_of_file file in
            ...
            if !cnf_features then
              collect_clause_features (Cnf.cnf_of_iter ~ctx:(Skolem.create ()) (CCVector.to_iter st))
    in
    ...
    match res with
        | CCResult.Ok () ->
            List.iter
              (function
                    | Counter n -> if CCOpt.is_none (StrTbl.get cnt_map n) then StrTbl.replace cnt_map n 0
                    | Vector n -> if CCOpt.is_none (StrTbl.get vec_map n) then StrTbl.replace vec_map n [ -1 ])
              all_features;
