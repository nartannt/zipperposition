...
module CheckedTrace = struct
  ...
  (** Maps every step of the given trace to a list of results obtained by trying to verify it using provers *)
...
let all_paths_correct ~valid ~checked =
  ...
  let rec check_proof proof =
    ...
    (* ok *)
      ...
      (match proof with
      | TT.InferClause (_, (lazy step)) | TT.InferForm (_, (lazy step)) -> Array.iter check_proof step.TT.parents
...
let main file =
  E.(
    ...
    check_all ~progress:!progress ~provers ~checked ~timeout:!timeout >>= fun () ->
    (* print failures *)
    List.iter
      (fun (proof, res) ->
        match res with
        | Failed prover -> Util.debugf 1 "trying to prove %a with %s failed" (fun k -> k TT.pp1 proof prover)
        | _ -> assert false)
