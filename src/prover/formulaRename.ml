...
module Make (C : Clause.S) = struct
  ...
  let mk_renaming_clause parent ~polarity_aware ~renamer ~form sign =
    ...
    let res =
      if polarity_aware then
        if sign then
          C.create ~penalty:1 ~trail:Trail.empty [ L.mk_false renamer; L.mk_true form ] proof
        else
          C.create ~penalty:1 ~trail:Trail.empty [ L.mk_true renamer; L.mk_false form ] proof
      else
  ...
  let get_skolem ~parent ~mode f =
    ...
    | `SkolemRecycle ->
      ...
      (match gen with
      | Some (orig, (skolem, _), subst) -> Subst.FO.apply Subst.Renaming.none subst (skolem, 0)
      | None ->
        let t = mk_skolem () in
