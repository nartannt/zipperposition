...
module Make (X : sig
  ...
  let multi_simplify ~depth c : (C.t * int) list option =
    ...
    let rec try_next ~depth c rules =
      if flex_get k_max_multi_simpl_depth != -1 && depth > flex_get k_max_multi_simpl_depth then
        None
      else (
        match rules with
        | [] -> None
  ...
  (** Use all simplification rules to convert a clause into a list of maximally simplified clauses.

      Stop applying mutlti_simpl rules after a certain depth. Especially dangerous rules are the ones that do
      boolean hoisting as simplification *)
  let all_simplify c =
  ...
  let convert_input_statements stmts : C.t Clause.sets =
    ...
    CCVector.iter
      (fun st ->
        ...
        match Statement.view st with
        | Statement.Assert _ when has_sos_attr st -> CCVector.append_list c_sos cs
