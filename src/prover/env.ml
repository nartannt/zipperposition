...
module Make (X : sig
  ...
  let immediate_simplify given immediate =
      let rec aux = function
          | [] -> immediate
          | f :: fs -> ( match f given immediate with Some res -> res | None -> aux fs)
      in
                  ...
                  match r t with
                      ...
                      | Some (t', proof) ->
                          ...
                          let new_t = match !_norm_rule t' with None -> t' | Some tt -> tt in
                              Util.debugf ~section 5 "@[<2>rewrite `@[%a@]`@ into `@[%a@]`@ :proof (@[%a@])@]"
      ...
      let lits' = Array.map (fun lit -> Lit.map (reduce_term !_rewrite_rules) lit) (C.lits c) in
          if StrSet.is_empty !applied_rules then SimplM.return_same c (* no simplification *)
            ...
            let c' = C.create_a ~trail:(C.trail c) ~penalty:(C.penalty c) lits' proof in
                ...
                Util.debugf ~section 3 "@[term rewritten clause `@[%a@]`@ into `@[%a@]`" (fun k ->
                    k C.pp c C.pp c');
                SimplM.return_new c')
  ...
  let ho_normalize c =
      ...
      in
          if not !did_reduce then SimplM.return_same c (* no simplification *)
            ...
            let c' = C.create_a ~trail:(C.trail c) ~penalty:(C.penalty c) lits' proof in
                assert (not (C.equal c c'));
                Util.debugf ~section 3 "@[lambda rewritten clause `@[%a@]`@ into `@[%a@]`" (fun k ->
                    k C.pp c C.pp c');
                SimplM.return_new c')
                  ...
                  match r lit with
                      ...
                      | Some (lit', proof, tgs) ->
                          ...
                          Util.debugf ~section 5
                            "@[rewritten lit `@[%a@]`@ into `@[%a@]`@ (using %s)@ :proof (@[%a@]) :tags %a@]"
                            (fun k ->
                              k Lit.pp lit Lit.pp lit' name (Util.pp_list Proof.pp_parent) proof Proof.pp_tags
                                tgs);
  ...
  let unary_simplify c =
      ...
      fix_simpl c ~f:(fun c ->
          ...
          (match !_lit_rules with [] -> SimplM.return_same c | _ :: _ -> rewrite_lits c) >>= fun c ->
          ...
          fix_simpl c ~f:(fun c ->
              let old_c = c in
                  ...
                  if not (Lits.equal_com (C.lits c) (C.lits old_c)) then
                    Util.debugf ~section 2 "@[clause `@[%a@]`@ simplified into `@[%a@]`@]" (fun k ->
                        k C.pp old_c C.pp c);
                  c)
      in
          ZProf.exit_prof _span;
  ...
  let multi_simplify ~depth c : (C.t * int) list option =
      ...
      let set_children c children =
          let d' = get_depth c + 1 in
              depth_map :=
                List.fold_left (fun map child -> Util.Int_map.add (C.id child) d' map) !depth_map children
      in
      let init_cl = c in
      ...
      let rec try_next ~depth c rules =
          if flex_get k_max_multi_simpl_depth != -1 && depth > flex_get k_max_multi_simpl_depth then None
            ...
            match rules with
                | [] -> None
                | r :: rules' -> ( match r c with Some l -> Some l | None -> try_next ~depth c rules')
      in
      ...
      let back_simplify c =
          ...
          fix_simpl c ~f:(fun c ->
              let old_c = c in
                  ...
                  >|= fun c ->
                  if not (Lits.equal_com (C.lits c) (C.lits old_c)) then
                    Util.debugf ~section 1 "@[clause `@[%a@]`@ simplified into `@[%a@]`@]" (fun k ->
                        k C.pp old_c C.pp c);
                  c)
      in
              ...
              match f c with
                  ...
                  | Some clauses ->
                      ...
                      in
                          ...
                          Util.debugf ~section 3 "@[active clause `@[%a@]`@ simplified into clauses `@[%a@]`@]"
