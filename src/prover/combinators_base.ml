          ...
          if Builtin.equal Builtin.SComb c_kind then
            match (args, ty_args) with
                | [ x; u ], [ alpha; beta; gamma ] -> (
                    match unpack_comb u with
                        | Builtin.KComb, _, [ y ] -> Some (mk_c ~args:[ x; y ] ~alpha ~beta ~gamma)
                        | _ -> None)
                | _ -> None
          else None
    with IsNotCombinator -> None
...
let opt7 t =
    try
      let c_kind, ty_args, args = unpack_comb t in
          if Builtin.equal Builtin.BComb c_kind then
            match args with
                | [ x; i ] -> ( match unpack_comb i with Builtin.IComb, _, [] -> Some x | _ -> None)
                | _ -> None
          else None
    with IsNotCombinator -> None
                ...
                | [ i ] -> (
                    let alpha = Term.of_ty @@ List.hd @@ Type.expected_args @@ T.ty t in
                        match unpack_comb i with
                            ...
                            | _ -> None)
                | [ i; x ] -> ( match unpack_comb i with Builtin.IComb, _, [] -> Some x | _ -> None)
                | _ -> None
          else None
    with IsNotCombinator -> None
...
let max_weak_reduction_length var_handler ~state orig_t =
    let rec aux t =
        match T.view t with
            | T.DB _ | T.Fun _ ->
                let err_msg =
                    CCFormat.sprintf "lambdas should be removed@.orig:@[%a@];subterm:@[%a@]@." T.pp orig_t T.pp
                      t
                in
                    invalid_arg err_msg
            | T.Const _ -> 0
            ...
            | T.AppBuiltin (b, l) when Builtin.is_combinator b -> (
                ...
                let c_kind, _, args = unpack_comb t in
                    match c_kind with
                        | Builtin.IComb -> if CCList.is_empty args then 0 else aux (narrow_one t) + 1
                        | Builtin.KComb ->
...
let cmp_by_max_weak_r_len t1 t2 =
    ...
    let state =
        { pos_counter = 0; neg_counter = 0; balance = Term.Tbl.create numvars; var_map = Term.Tbl.create 16 }
    in
    ...
    let rec aux t =
        match T.view t with
            | T.AppBuiltin _ | T.App _ -> (
                ...
                assert (not (T.is_fun hd_mono));
                try
                  if T.same_l args args' then t else T.app hd_mono args' (* flattens AppBuiltin if necessary *)
