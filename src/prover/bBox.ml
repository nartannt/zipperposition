...
let pp_payload out = function
    ...
    | Clause_component lits -> Format.fprintf out "@<1>⟦@[<hv>%a@]@<1>⟧" Lits.pp lits
...
let negate_ground lits =
    match lits with
        | [| lit0 |]
          when Literal.is_ground lit0 && Literal.is_negativoid lit0 && not (Literal.is_constraint lit0) ->
...
let to_s_form (lit : t) =
    ...
    let f =
        match payload lit with
            ...
            | Clause_component lits -> F.box_opaque (Literals.Conv.to_s_form lits |> F.close_forall)
...
let pp_tstp out b =
    let pp_box_unsigned out b =
        match payload b with
            ...
            | Clause_component lits -> CCFormat.within "(" ")" Literals.pp_tstp_closed out lits
