...
module type S = sig
  ...
  (** Perform one step of the given clause algorithm. It performs generating inferences only if [generating] is
      true (default); other parameters are the iteration number and the environment *)

  val given_clause : ?generating:bool -> ?steps:int -> ?timeout:float -> unit -> szs_status * int
  (** run the given clause until a timeout occurs or a result is found. It returns a tuple (new state, result,
      number of steps done). It performs generating inferences only if [generating] is true (default) *)

  val presaturate : unit -> szs_status * int
  (** Interreduction of the given state, without generating inferences. Returns the number of steps done for
      presaturation, with status of the set. *)
end
...
module Make (E : Env.S) = struct
  ...
  let given_clause_step ?(generating = true) num =
    ...
    | Some c ->
      ...
      Util.debugf ~section 3 "@[<2>@{<green>given@} (before simplification):@ `@[%a@]`@]" (fun k -> k Env.C.pp c);
      ...
      | c :: l', state ->
        ...
        if Env.C.is_redundant c then (
          ...
          let inferred_clauses =
            ...
            Iter.flat_map_l (fun c -> CCOpt.get_or ~default:[ c ] (Env.cheap_multi_simplify c)) inferred_clauses
          in
    ...
    let rec do_step num =
      if check_timeout timeout then
        ...
        | _ ->
          ...
          let res_opt =
            if should_try_e num timeout then begin
