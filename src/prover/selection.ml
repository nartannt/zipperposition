...
(** Selection functions. Note for splitting: SelectComplex already selects in priority "big" negative literals,
    ie literals that are not split symbols. *)
...
let can_select_lit ~ord (lits : Lits.t) (i : int) : bool =
  if Lit.is_negativoid lits.(i) then (
    ...
    let var_headed_subterms which =
      let lits =
        if which = `Max then
          Lits.maxlits_l ~ord lits |> CCList.map (fun (l, _) -> l) |> Array.of_list
        else
          lits
      in
      lits
...
let validate_fun_ ~ord lits bv =
  if BV.is_empty bv then
    ...
    Iter.of_array_i lits |> Iter.exists (fun (i, _) -> (not (BV.get bv i)) || can_select_lit ~ord lits i)
      ...
      if T.is_var (T.head_term lhs) then
        ...
        let alpha_rank =
          if not hd_is_cst then
            ...
            ID.Map.get_or ~default:max_int (T.as_const_exn (T.head_term lhs)) alpha_map
        in
...
let from_string ~ord s =
  ...
  | Some name ->
    ...
    let bool_blocker i l = Lit.Seq.terms l |> Iter.flat_map selectable_sub |> fun i -> not @@ Iter.is_empty i in
