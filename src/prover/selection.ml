...
let can_select_lit ~ord (lits : Lits.t) (i : int) : bool =
    if Lit.is_negativoid lits.(i) then
      ...
      let var_headed_subterms which =
          let lits =
              if which = `Max then Lits.maxlits_l ~ord lits |> CCList.map (fun (l, _) -> l) |> Array.of_list
              else lits
          in
              lits
              |> CCArray.fold
          ...
          match !_ho_restriction with
              ...
              | `NoUnappliedVarOccurringApplied ->
                  ...
                  let vars_args = var_headed_subterms `All in
                      ...
                      |> Iter.exists (fun (t, _) ->
                             vars_args
                             |> CCList.exists (fun (head, args) -> T.equal head t && not (CCList.is_empty args)))
            ...
            match l with
                | Equation (lhs, _, _) when not @@ blocker idx l ->
                    if T.is_var (T.head_term lhs) then (sign_val, 0, 0, diff_val)
                      ...
                      let prec_weight =
                          if not hd_is_cst then 0 else Precedence.sel_prec_weight prec (T.head_exn lhs)
                      in
                      let alpha_rank =
    ...
    let chooser (i, l) =
        ...
        let hd_freq = get_pred_freq ~freq_tbl l in
            if Lit.is_ground l then (sign, 0, -T.ho_weight lhs, hd_freq)
              ...
              let max_term_weight =
                  Iter.of_list (Lit.Comp.max_terms ~ord l) |> Iter.map (T.weight ~var:0) |> Iter.sum
              in
...
let e_sel6 ~blocker ~ord lits =
    ...
    let is_oriented lit =
        match lit with
            | Lit.Equation (l, r, _) -> Ordering.compare ord l r != Comparison.Incomparable
            | _ -> true
    in
    let chooser (i, l) =
...
let e_sel9 ~blocker ~ord lits =
    ...
    let chooser (i, l) =
        ((if is_truly_equational l then max_int else 0), lhs_head_arity l, lhs_head_alpha l, 0)
    in
        weight_based_sel_driver ~can_sel:can_select_lit ~ord ~blocker lits chooser
...
let e_sel11 ~blocker ~ord lits =
    ...
    let lhs_weight = function
        | Lit.Equation (lhs, rhs, _) as l when Lit.is_negativoid l -> Term.ho_weight lhs
        | _ -> max_int
    in
...
let pos_e_sel1 ~blocker ~ord lits =
    let chooser (i, l) =
        ( Lit.is_positivoid l,
          (match l with
              | Literal.Equation (lhs, rhs, _) -> Ordering.compare ord lhs rhs == Comparison.Incomparable
              | _ -> true),
          Lit.ho_weight l )
    in
        ...
        match l with
            | Lit.Equation (lhs, rhs, _) ->
                let num_app_var_sides =
                    (if T.is_app_var lhs then 1 else 0) + if T.is_app_var rhs then 1 else 0
                in
...
let from_string ~ord s =
    ...
    in
        match CCString.chop_prefix ~pre:"bb+" s with
            | Some name ->
                let selectable_sub =
                    Bool_selection.all_selectable_subterms ~ord ~pos_builder:Position.Build.empty
                in
        ...
        Params.add_opts
          [
            ( "--select",
              ...
              " set literal selection function. Prefix selection function with \"bb+\" to block selecting \
               literals that have selectable boolean subterms. " );
