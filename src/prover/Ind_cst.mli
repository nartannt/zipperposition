...
type t
(** A ground term of an inductive type. It must correspond to a term built with the corresponding {!t} only. For
    instance, a constant of type [nat] should be equal to [s^n(0)] in any model. *)
...
(** [dominates c1 c2] if [depth c1 < depth c2]. This way, in coversets, the top constant dominates all
    sub-constants *)
...
(** [id_is_potential_cst id ty] returns [true] if [id:ty] is a skolem constant of an inductive type, or if it is
    already an inductive constant. *)

val find_ind_skolems : Term.t -> ind_skolem Iter.t
(** [find_ind_skolem term] searches subterms of [term] for constants that are of an inductive type and that are
    skolems or (already) inductive constants. *)
