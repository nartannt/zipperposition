...
module Make (Ctx : Ctx.S) : S with module Ctx = Ctx = struct
  ...
  (** Bitvector that indicates which of the literals of [subst(clause)] are maximal under [ord] *)
        ...
        if not (BV.get bv i) then
          ...
          for j = i + 1 to n - 1 do
            ...
            if Lit.is_positivoid lit = Lit.is_positivoid lit' && BV.get bv j then (
              ...
              | Comparison.Incomparable | Eq -> () (* no further information about i-th and j-th *)
