...
let pp_vars out c =
  let pp_vars out = function
    | [] -> ()
    | l -> Format.fprintf out "forall @[%a@].@ " (Util.pp_list ~sep:" " Type.pp_typed_var) l
  in
...
let pp_zf out c =
  if Trail.is_empty c.trail then
    ...
    Format.fprintf out "@[<2>(%a)@ => (%a)@]" pp_trail_zf c.trail Literals.pp_zf_closed c.lits
...
let pp_trail_tstp out trail =
  ...
  let pp_box_unsigned out b =
    ...
    | BBox.Clause_component lits -> CCFormat.within "(" ")" Literals.pp_tstp_closed out lits
...
let pp_tstp out c =
  if Trail.is_empty c.trail then
    ...
    Format.fprintf out "@[<2>(@[%a@])@ <= (%a)@]" Literals.pp_tstp_closed c.lits pp_trail_tstp c.trail
