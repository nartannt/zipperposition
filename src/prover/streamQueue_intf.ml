...
module type S = sig
  ...
  module WeightFun : sig
    type t = Stm.t -> int
    (** attribute a weight to a stream. The smaller, the better (lightweight streams will be favored). A weight
  ...
  (** Attempts to take as many clauses from the queue as there are streams in the queue. Calls take_first to do
      so and stops if its guard is reached *)

  val take_stm_nb_fix_stm : t -> Stm.C.t option list
  (** Attempts to take as many clauses from the queue as there are streams in the queue. Extract as many clauses
      as possible from first stream before moving to a new stream to find more clauses if necessary *)
  ...
  (** Creates a priority queue that uses [weight] to sort streams.
      @param ratio
        pick-given ratio. Only one in [ratio] truly returns a clause if there is one available in calls to
        {!take_first_when_available} and {!take_fair_anyway}.
