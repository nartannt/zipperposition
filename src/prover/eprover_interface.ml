...
module Make (E : Env.S) : S with module Env = E = struct
  ...
  let output_all ?(already_defined = ID.Set.empty) ~out cl_set =
    ...
    Printf.printf "term symbols: %i\n" (Iter.length term_syms);
    Printf.printf "type symbols: %i\n" (Iter.length type_syms);
  ...
  let run_e prob_path =
    ...
    | Some e_path ->
      let to_ = !_timeout in
      let cmd =
        CCFormat.sprintf "timeout %d %s --pos-ext=all --neg-ext=all %s --cpu-limit=%d %s -s -p" (to_ + 2) e_path
          prob_path to_
          (if !_e_auto then
             "--auto"
           else
  ...
  let try_e poly_active_set poly_passive_set =
    ...
    let take_ho_clauses ~converter ~encoded_symbols clauses =
      (*Iter.iter (fun cl -> Printf.printf "clause: %s\n" (C.to_string cl)) clauses;*)
    ...
    try
      ...
      (*Printf.printf "monomorphisation finished!!!\n";*)
      ...
      (*let clauses = Iter.of_list clause_list in*)
