...
module Make (C : Clause_intf.S) = struct
  ...
  module WeightFun = struct
    ...
    let calc_lweight l sg v w c_mul =
      ...
      | Lit.Equation (lhs, rhs, sign) -> calc_tweight lhs sg v w c_mul + calc_tweight rhs sg v w c_mul, sign
      | _ -> 0, false
    ...
    let conj_relative_struct ~inst_penalty ~gen_penalty ~var_w ~sym_w c =
      ...
      let struct_diff_weight t =
        ...
        let rec w_diff ~given_term ~conj_term =
          ...
          | T.App (hd, _), _ when T.is_var hd -> int_of_float (w given_term +. (inst_penalty *. w conj_term))
          | _, T.Var _ -> int_of_float (gen_penalty *. w given_term)
          | _, T.App (hd, _) when T.is_var hd -> int_of_float (w conj_term +. (inst_penalty *. w given_term))
    ...
    let dag_weight ~fweight ~vweight ~pos_multiplier ~dup_weight ~pos_use_dag ~pos_t_reset ~pos_eqn_reset
      ...
      let calc_w use_dag reset_lit reset_term lit =
        ...
        in
        match lit with
        | Lit.Equation (lhs, rhs, _) -> int_of_float (p_m *. float_of_int (t_w false lhs + t_w reset_term rhs))
        | _ -> 0
      in
    ...
    let default_fun =
      penalize
        (combine [ default, 3; favor_all_neg, 1; favor_small_num_vars, 2; favor_goal, 1; favor_pos_unit, 1 ])
  ...
  module PriorityFun = struct
    ...
    let prefer_unit_ground_non_goals c =
      if Iter.exists Literal.is_positivoid (C.Seq.lits c) && C.is_unit_clause c && C.is_ground c then
        0
      else
        1
    ...
    let prefer_formulas c =
      if C.Seq.terms c |> Iter.exists (fun t -> Iter.exists Term.is_formula (Term.Seq.subterms t)) then
        0
      else
        1
  ...
  (** generic clause queue based on some ordering on clauses, given by a weight function *)
  ...
  let goal_oriented () : t =
    ...
    let weight = penalize (combine [ default, 4; favor_small_num_vars, 2; favor_goal, 1; favor_all_neg, 1 ]) in
