          ...
          match CCString.chop_prefix ~pre:"conjecture-relative-var" s with
              | Some suffix ->
                  ...
                  let args =
                      List.map
                        (fun s ->
                          String.trim (CCString.replace ~sub:")" ~by:"" (CCString.replace ~sub:"(" ~by:"" s)))
...
module Make (C : Clause_intf.S) = struct
  ...
  module WeightFun = struct
    ...
    let avoid_expensive c =
        ...
        let signature = C.Ctx.signature () in
            CCArray.foldi
              (fun acc i l ->
                let max_terms =
                    if CCBV.get max_lits i then Literal.Comp.max_terms ~ord:(C.Ctx.ord ()) l else []
                in
                let lit_weight =
                    Literal.Seq.terms l
                    |> Iter.map (fun t ->
                ...
                let w =
                    match lit with
                        | Lit.Equation (l, r, _) ->
                            ...
                            let l_mul =
                                match ord_side with Comparison.Gt | Geq | Incomparable -> max_t_m | _ -> 1.0
                            in
                            let r_mul =
                                match ord_side with Comparison.Lt | Leq | Incomparable -> max_t_m | _ -> 1.0
                            in
                            let eq_inc =
    ...
    let rec calc_tweight t sg v w c_mul =
        match Term.view t with
            | Term.AppBuiltin (_, l) -> w + List.fold_left (fun acc t -> acc + calc_tweight t sg v w c_mul) 0 l
            | Term.Var _ -> v
            | Term.DB _ -> w
            | Term.App (f, l) ->
    ...
    let conj_relative ?(distinct_vars_mul = -1.0) ?(parameters_magnitude = `Large) ?(goal_penalty = false) c =
        ...
        let conj_mul = 0.5 in
            ...
            if distinct_vars_mul < 0.0 then int_of_float res
              ...
              let dist_vars =
                  Literals.vars (C.lits c) |> List.filter (fun v -> not (Type.is_tType (HVar.ty v)))
              in
        ...
        let struct_diff_weight t =
            ...
            let rec w_diff ~given_term ~conj_term =
                match (T.view given_term, T.view conj_term) with
                    ...
                    | _, _ -> int_of_float ((inst_penalty *. w conj_term) +. (gen_penalty *. w given_term))
            and w_diff_l xs ys =
    ...
    let conjecture_relative_e ~conj_mul ~fresh_mul ~f ~cst ~p ~v ~max_term_mul ~max_lit_mul ~pos_mul c =
        ...
        let f_weight sym ty =
            ...
            in
                int_of_float
                  (float_of_int (if Type.returns_prop ty then p else if Type.is_fun ty then f else cst)
                  *. multipliers)
        in
                ...
                match lit with
                    | Literal.Equation (lhs, rhs, _) -> (
                        match Ordering.compare (C.Ctx.ord ()) lhs rhs with
                            | Comparison.Gt | Geq -> t_weight ~mul:max_term_mul lhs + t_weight ~mul:1.0 rhs
    ...
    let dag_weight ~fweight ~vweight ~pos_multiplier ~dup_weight ~pos_use_dag ~pos_t_reset ~pos_eqn_reset
        ...
        let _w = ref 0 in
            CCArray.iter
              (fun lit ->
                if Lit.is_positivoid lit then _w := !_w + calc_w pos_use_dag pos_eqn_reset pos_t_reset lit)
              (C.lits c);
            ...
            CCArray.iter
              (fun lit ->
                if Lit.is_negativoid lit then _w := !_w + calc_w neg_use_dag neg_eqn_reset neg_t_reset lit)
              (C.lits c);
    ...
    let diversity_weight ~var_w ~sym_w ~pos_mul ~max_t_mul ~max_l_mul ~fdiff_a ~fdiff_b ~vdiff_a ~vdiff_b c =
        ...
        let mul_max_t (l, w_l) (r, w_r) =
            match Ordering.compare ord l r with
                | Comparison.Incomparable -> (max_t_mul *. float_of_int w_l) +. (max_t_mul *. float_of_int w_r)
    ...
    let parse_crv s =
        let crv_regex =
            Str.regexp "conjecture-relative-var(\\([0-9]+[.]?[0-9]*\\),\\([lsLS]\\),\\([tfTF]\\))"
        in
            try
              ignore (Str.search_forward crv_regex s 0);
              let distinct_vars_mul = CCFloat.of_string_exn (Str.matched_group 1 s) in
              let parameters_magnitude =
                  Str.matched_group 2 s |> CCString.trim |> String.lowercase_ascii |> fun s ->
                  if CCString.prefix ~pre:"l" s then `Large else `Small
              in
              let goal_penalty =
        ...
        in
            try
              ...
              let pos_neg_reset = parse_bool @@ Str.matched_group 11 s in
                  dag_weight ~fweight ~vweight ~pos_multiplier ~dup_weight ~pos_use_dag ~pos_t_reset
                    ~pos_eqn_reset ~neg_use_dag ~neg_t_reset ~neg_eqn_reset ~pos_neg_reset
            with Not_found | Invalid_argument _ ->
              invalid_arg
    ...
    let parse_pnrefine s =
        ...
        in
            ...
            with Not_found | Invalid_argument _ ->
              ...
              ^ "-fun_weight:int,-var_weight:int" ^ "max_t_mult:float, max_lit_mul:float, pos_lit_mult:float)"
                    ...
                    match lit with
                        | Lit.Equation (l, r, _) ->
                            let term_w () t =
                                float_of_int
                                @@ Term.weight ~var:vw
                                     ~sym:(fun id -> ID.Tbl.get_or _f_weights ~default:!_default id)
                                     t
                            in
                            let ord_side = Ordering.compare ord l r in
                            let l_mul =
                                match ord_side with Comparison.Gt | Geq | Incomparable -> max_t_mul | _ -> 1.0
                            in
                            let r_mul =
                                match ord_side with Comparison.Lt | Leq | Incomparable -> max_t_mul | _ -> 1.0
                            in
                              ...
                              ID.Set.fold
                                (fun id new_syms ->
                                  let cls = ID.Tbl.get_or ~default:[] cl_map id in
                                      ID.Set.fold
                                        (fun id acc ->
                                          if not (ID.Tbl.mem _f_weights id) then ID.Set.add id acc else acc)
                      ...
                      let max_lvl = fill_levels goal_syms 1 in
                          ...
                          ID.Tbl.filter_map_inplace
                            (fun id lvl ->
                              ...
                              let base =
                                  if Type.returns_prop ty then pw else if Type.is_fun ty then fw else cw
                              in
  ...
  module PriorityFun = struct
    ...
    let prefer_unit_ground_goals c =
        if (not (Iter.exists Literal.is_positivoid (C.Seq.lits c))) && C.is_unit_clause c && C.is_ground c then
          0
        else 1
    ...
    let prefer_formulas c =
        if C.Seq.terms c |> Iter.exists (fun t -> Iter.exists Term.is_formula (Term.Seq.subterms t)) then 0
        else 1
        ...
        let app_var_depth t =
            let rec aux depth t =
                match Term.view t with
                    ...
                    | Term.App (hd, args) when Term.is_var hd -> max depth (aux_l (depth + 1) args)
