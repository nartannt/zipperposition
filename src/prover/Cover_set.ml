...
let make_coverset_ ~cover_set_depth ~depth (ty : Type.t) (ity : Ind_ty.t) : t =
    ...
    let rec make (cs_depth : int) (subst : Subst.t) : T.t mm =
        ...
        if cs_depth = 0 then
          ...
          let n_ty_params, ty_args_f, _ = Type.open_poly_fun ty_f_applied in
              ...
              if ty_args_f = [] then
                if cs_depth > 0 then yield (T.app (T.const ~ty:ty_f f) ty_params) (* only one answer : f *)
                else fail
              else
    ...
    and make_of_ty depth ty : T.t mm =
        let subst =
            try Some (Unif.Ty.matching_same_scope ~pattern:ity.Ind_ty.ty_pattern ty ~scope)
            with Unif.Fail -> None
        in
...
let make ?(cover_set_depth = 1) ~depth (ty : Type.t) : t =
    if cover_set_depth <= 0 then
      Util.invalid_argf "Cover_set.make: cover_set_depth=%d must be > 0" cover_set_depth;
    match Ind_ty.as_inductive_type ty with
        | Some (ity, _) ->
            let set = make_coverset_ ~cover_set_depth ~depth ty ity in
                ...
                set
        | None -> Util.errorf ~where:"cover_set.make" "type `@[%a@]`@ is not an inductive type" Type.pp ty
