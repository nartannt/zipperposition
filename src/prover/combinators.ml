...
module Make (E : Env.S) : S with module Env = E = struct
  ...
  let instantiate_var_w_comb ~var =
      CCList.filter_map
        (fun (comb, penalty) ->
          try Some (Unif.FO.unify_syn (comb, 0) (var, 1), penalty) with Unif.Fail -> None)
  ...
  let narrow_app_vars clause =
      ...
      let lits = C.lits clause in
          ...
          (if narrow_app_vars_applicable clause then
             Lits.fold_terms ~vars:false ~var_args:true ~fun_bodies:false ~subterms:true ~ord ~which:`Max
               ~eligible ~ty_args:false lits
           else Iter.empty)
                     ...
                     CCList.filter_map
                       (fun (subst, comb_penalty) ->
                         ...
                         let lit = Lit.apply_subst_no_simp renaming subst (lits.(lit_idx), 1) in
                             ...
                             then (
                               Util.debugf ~section 3 "ordering restriction fail: @[%a@]@." (fun k ->
                                   k Subst.pp subst);
                               None)
                             else
                               let t_depth = Position.size (Literal.Pos.term_pos lits.(lit_idx) lit_pos) in
                               let depth_mul =
                                   if not @@ Env.flex_get k_deep_app_var_penalty then 1 else max t_depth 1
                               in
              ...
              let lhs, rhs = CCPair.map_same comb2lam (lhs, rhs) in
                  match get_unif lhs rhs with
                      | Some subst ->
                          ...
                          let proof =
                              Proof.Step.simp ~rule [ C.proof_parent_subst Subst.Renaming.none (c, 0) subst ]
                          in
