      ...
      if T.is_app_var t then CCOpt.return_if (is_selectable ~top ~forbidden t) (t, PB.to_pos pos_builder)
        ...
        match T.view t with
           | T.App (_, args) | T.AppBuiltin (_, args) ->
              ...
              let outer =
                 if not (is_selectable ~top ~forbidden t) then None else Some (t, PB.to_pos pos_builder)
              in
           ...
           match lits.(idx) with
              | Lit.Equation (lhs, rhs, sign) as lit ->
                 ...
                 in
                    (match Ordering.compare ord lhs rhs with
                       | Comparison.Lt | Leq ->
                          selectable_with_ctx ~block_top:sign ~ord ~forbidden ~pos_builder:(PB.right pos_builder)
                            rhs ctx_sign 0 k
                       | Gt | Geq ->
                          selectable_with_ctx ~block_top:sign ~ord ~forbidden ~pos_builder:(PB.left pos_builder)
                            lhs ctx_sign 0 k
                       | _ ->
                          selectable_with_ctx ~block_top:sign ~ord ~forbidden ~pos_builder:(PB.left pos_builder)
                            lhs ctx_sign 0 k;
