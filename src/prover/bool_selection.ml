...
(** As described in FBoolSup paper, Boolean selection function selects positions in the clause that are
    non-interpreted Boolean subterms. *)
    ...
    if idx >= Array.length lits then
      ...
      | Lit.Equation (lhs, rhs, _) ->
        let in_literal =
          ...
          | Comparison.Lt | Leq -> aux_term ~top:true ~pos_builder:(PB.right pos_builder) rhs
          | Gt | Geq -> aux_term ~top:true ~pos_builder:(PB.left pos_builder) lhs
          | _ ->
  ...
  let rec aux_term ~top ~pos_builder t k =
    ...
    if T.is_app_var t then
      ...
      | T.AppBuiltin ((BIn.Eq | BIn.Neq | BIn.Xor | BIn.Equiv), (([ a; b ] | [ _; a; b ]) as l))
        ...
        | Gt | Geq -> aux_term ~top:false ~pos_builder:(PB.arg (inv_idx l offset) pos_builder) a k
        | _ ->
          aux_term ~top:false ~pos_builder:(PB.arg (inv_idx l (1 + offset)) pos_builder) b k;
          aux_term ~top:false ~pos_builder:(PB.arg (inv_idx l offset) pos_builder) a k)
      | T.App (_, args) | T.AppBuiltin (_, args) -> aux_term_args ~idx:(List.length args - 1) ~pos_builder args k
      | _ -> ()
    )
  ...
  let rec aux_lits idx k =
    if idx < Array.length lits then (
      ...
      | Lit.Equation (lhs, rhs, sign) as lit ->
        ...
        (match Ordering.compare ord lhs rhs with
        | Comparison.Lt | Leq ->
          selectable_with_ctx ~block_top:sign ~ord ~forbidden ~pos_builder:(PB.right pos_builder) rhs ctx_sign 0
            k
        | Gt | Geq ->
...
let fun_names =
  [ "LI", leftmost_innermost; "LO", leftmost_outermost; "smallest", smallest; "largest", largest; "none", none ]
