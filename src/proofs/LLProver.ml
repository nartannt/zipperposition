...
module Branch : sig
  ...
  let debug out (b : t) : unit =
      (* print one branch *)
      let pp_b out b =
          Fmt.fprintf out "(@[<hv>branch/%d :closed %B@ :form (%a)@ :to_expand (@[<hv>%a@])@])" b.id
            (is_closed b) (Fmt.some T.pp) b.form (Util.pp_iter T.pp) (T_set.to_iter b.to_expand)
      in
...
let solve_ (tab : t) : res =
    ...
    match tab.saturated with
        ...
        | None ->
            ...
            Util.debugf ~section 5 "(@[llprover.prove.success@ :branches (@[<v>%a@])@ :saturated %a@])"
              (fun k -> k (Util.pp_list Branch.debug) tab.closed_branches (Fmt.some Branch.debug) tab.saturated);
            R_ok
