...
module CC = Congruence.Make (struct
  ...
  let update_subterms t l =
    ...
    | T.AppBuiltin (b, l1), l1' when List.length l1 = List.length l1' -> T.app_builtin ~ty:(T.ty_exn t) b l1'
    | T.Bind { binder; ty_var; _ }, [ body ] -> T.bind ~ty:(T.ty_exn t) binder ~ty_var body
    | (T.Const _ | T.Var _), [] -> t
...
(** Branches of the tableau. A branch is a conjunction of formulas plus some theory context (congruence closure).
...
module Branch : sig
  ...
  let add1_ (br : t) (f : T.t) : t =
    ...
    | F.Not f' ->
      ...
      | F.Equiv (a, b) -> add_form_to_expand (F.or_ [ F.and_ [ a; F.not_ b ]; F.and_ [ b; F.not_ a ] ]) br
      | F.Xor (a, b) -> add_form_to_expand (F.and_ [ F.or_ [ a; F.not_ b ]; F.or_ [ b; F.not_ a ] ]) br
    ...
    | F.Xor (a, b) -> add_form_to_expand (F.or_ [ F.and_ [ a; F.not_ b ]; F.and_ [ b; F.not_ a ] ]) br
    | F.Equiv (a, b) -> add_form_to_expand (F.and_ [ F.or_ [ a; F.not_ b ]; F.or_ [ b; F.not_ a ] ]) br
...
let can_check : LLProof.tag list -> bool =
  ...
  let f = function
    | T_ho -> true
    | T_lra | T_lia | T_ind | T_data | T_live_cnf | T_ho_norm | T_dont_increase_depth | T_distinct | T_ac _
    | T_ext | T_cannot_orphan ->
      false
  in
  List.for_all f
...
let pp_dot out (s : final_state) : unit =
  ...
  let attrs_v (b : Branch.t) : _ list =
    ...
    let pp_closed out b =
      ...
      | Branch.C_not_closed -> Fmt.fprintf out "<not closed> (%d to expand)" (Branch.to_expand b |> Iter.length)
      | Branch.C_closed_by_diseq (t, u) -> Fmt.fprintf out "<closed by `@[<1>%a ≠@ %a@]`>" T.pp t T.pp u
