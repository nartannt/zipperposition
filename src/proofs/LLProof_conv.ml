      ...
      match Proof.Step.kind @@ Proof.S.step p with
         | Proof.Inference (rule, tags) | Proof.Simplification (rule, tags) ->
            ...
            let local_intros = Var.Subst.to_list !local_intros |> List.rev_map snd in
               LLProof.inference ~intros ~local_intros ~tags (T.rename_all_vars res) (Proof.Rule.name rule)
                 parents
         | Proof.Esa rule ->
            let l =
               List.map
                     ...
                     List.map
                       (fun v ->
                         match Var.Subst.find inst_subst v with
                            ...
                            | None ->
                               errorf
                                 "cannot find variable `%a`@ in inst-subst {%a}@ :inst-res %a@ :res %a@ :parent \
                                  %a@ :intros [@[%a@]]"
                                 Var.pp_fullc v (Var.Subst.pp T.pp) inst_subst T.pp p_instantiated_res T.pp
                                 step_res Proof.pp_parent parent (Util.pp_list ~sep:"," T.pp) intros)
                       vars_p
               in
                         ...
                         | None ->
                            ...
                            in
                               ...
                               Util.debugf ~section 5
                                 "(@[llproof.conv.add_local_intro@ %a := %a@ :p-instantiated `%a`@])" (fun k ->
