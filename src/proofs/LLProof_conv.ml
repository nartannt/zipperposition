                            ...
                            List.map
                              (fun v ->
                                match Var.Subst.find inst_subst v with
                                    ...
                                    | None ->
                                        errorf
                                          "cannot find variable `%a`@ in inst-subst {%a}@ :inst-res %a@ :res \
                                           %a@ :parent %a@ :intros [@[%a@]]"
                                          Var.pp_fullc v (Var.Subst.pp T.pp) inst_subst T.pp p_instantiated_res
                                          T.pp step_res Proof.pp_parent parent (Util.pp_list ~sep:"," T.pp)
                                          intros)
                              vars_p
                    in
