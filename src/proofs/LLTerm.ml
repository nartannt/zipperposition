...
module Int_op = struct
  ...
  let equal a b =
    ...
    | Divisible_by _, _ | _, Divisible_by _ | Not_div_by _, _ | _, Not_div_by _ -> false
    | _ -> a = b
...
module Make_linexp (N : NUM) = struct
  ...
  let ( * ) c e : t =
    if N.equal N.zero c then
      ...
      { const = N.(c * e.const); coeffs = I_map.map (fun (t, n) -> t, N.(c * n)) e.coeffs }
...
module H_cons = Hashcons.Make (struct
  ...
  let equal a b =
    ...
    | Bind b1, Bind b2 -> Binder.equal b1.binder b2.binder && equal b1.ty_var b2.ty_var && equal b1.body b2.body
    | AppBuiltin (b1, l1), AppBuiltin (b2, l2) -> Builtin.equal b1 b2 && CCList.equal equal l1 l2
    | Ite (a1, b1, c1), Ite (a2, b2, c2) -> equal a1 a2 && equal b1 b2 && equal c1 c2
    | Int_pred (l1, o1), Int_pred (l2, o2) -> Int_op.equal o1 o2 && Linexp_int.equal l1 l2
    | Rat_pred (l1, o1), Rat_pred (l2, o2) -> Rat_op.equal o1 o2 && Linexp_rat.equal l1 l2
    | Type, _
    | Const _, _
    | App _, _
    | Arrow _, _
  ...
  let hash (a : t) : int =
    ...
    | Bind b -> CCHash.combine4 40 (Binder.hash b.binder) (hash b.ty_var) (hash b.body)
    | AppBuiltin (b, l) -> CCHash.combine3 50 (Builtin.hash b) (CCHash.list hash l)
...
let rec pp_rec depth out (t : t) =
  ...
  | Arrow (a, b) -> Fmt.fprintf out "@[%a@ @<1>→ %a@]" (pp_rec_inner depth) a (pp_rec depth) b
  | Var v -> Format.fprintf out "Y%d" (depth - HVar.id v - 1)
  | AppBuiltin (Builtin.Box_opaque, [ t ]) -> Format.fprintf out "@<1>⟦@[%a@]@<1>⟧" (pp_rec depth) t
  ...
  | AppBuiltin (b, l) -> Format.fprintf out "(@[<hv>%a@ %a@])" Builtin.pp b (Util.pp_list (pp_rec_inner depth)) l
  | Bind { ty_var; binder; body } ->
    Fmt.fprintf out "@[%a (@[Y%d:%a@]).@ %a@]" Binder.pp binder depth (pp_rec depth) ty_var
      (pp_rec @@ (depth + 1))
      body
  | Ite (a, b, c) ->
    Fmt.fprintf out "@[<hv2>ite %a@ %a@ %a@]" (pp_rec_inner depth) a (pp_rec_inner depth) b (pp_rec_inner depth)
      c
  | Int_pred (l, o) -> Fmt.fprintf out "(@[%a@ %a@])" (Linexp_int.pp (pp_rec_inner depth)) l Int_op.pp o
  | Rat_pred (l, o) -> Fmt.fprintf out "(@[%a@ %a@])" (Linexp_rat.pp (pp_rec_inner depth)) l Rat_op.pp o

and pp_rec_inner depth out t =
  match view t with
  | App _ | Bind _ | AppBuiltin (_, _ :: _) | Arrow _ | Ite _ -> Fmt.fprintf out "(%a)/%d" (pp_rec depth) t t.id
...
and pp_infix_ depth b out l =
  ...
  | t :: l' -> Format.fprintf out "@[%a@]@ %a %a" (pp_rec_inner depth) t Builtin.pp b (pp_infix_ depth b) l'
...
let ite a b c =
  ...
  | _ -> errorf "type error:@ cannot build `@[ite %a %a %a@]`@ incompatible types" pp a pp b pp c
...
let[@inline] map ~f ~bind:f_bind b_acc t =
  ...
  | AppBuiltin (b, l) -> app_builtin ~ty:(f b_acc @@ ty_exn t) b (List.map (f b_acc) l)
...
let app f x =
  ...
  | Bind { binder = Binder.Lambda; ty_var; body } ->
    ...
    | Some ty_x -> errorf "type error: cannot apply `%a`@ to `%a : %a`" pp f pp x pp ty_x
...
let arrow a b =
  ...
  | _ -> errorf "type error: cannot make arrow between non-types@ :from `%a`@ :to `%a`" pp a pp b
...
module Conv = struct
  ...
  type ctx = {
    depth: int;
    (* depth *) vars: (T.t, int * ty) Var.Subst.t (* depth at binding site, + type *);
  }
  ...
  and conv_int_linexp ctx t : Linexp_int.t =
    ...
    | T.AppBuiltin (Builtin.Sum, [ _; a; b ]) -> Linexp_int.(conv_int_linexp ctx a + conv_int_linexp ctx b)
    ...
    | T.AppBuiltin (Builtin.Product, [ _; a; b ]) ->
      ...
      | T.AppBuiltin (Builtin.Int n, []), _ -> Linexp_int.(n * conv_int_linexp ctx b)
      | _, T.AppBuiltin (Builtin.Int n, []) -> Linexp_int.(n * conv_int_linexp ctx a)
  ...
  and conv_rat_linexp ctx t : Linexp_rat.t =
    ...
    | T.AppBuiltin (Builtin.Sum, [ _; a; b ]) -> Linexp_rat.(conv_rat_linexp ctx a + conv_rat_linexp ctx b)
    | T.AppBuiltin (Builtin.Difference, [ _; a; b ]) ->
      Linexp_rat.(conv_rat_linexp ctx a - conv_rat_linexp ctx b)
    | T.AppBuiltin (Builtin.Product, [ _; a; b ]) ->
      (match T.view a, T.view b with
      | T.AppBuiltin (Builtin.Rat n, []), _ -> Linexp_rat.(n * conv_rat_linexp ctx b)
      | _, T.AppBuiltin (Builtin.Rat n, []) -> Linexp_rat.(n * conv_rat_linexp ctx a)
