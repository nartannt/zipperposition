    ...
    match (ty f, ty x) with
        ...
        | Some { view = Bind { binder = Binder.ForallTy; ty_var; body }; _ }, Some ty_x ->
            ...
            if not (equal ty_x t_type) then
              errorf "cannot apply@ `@[<2>%a@ : %a@]`@ to non-type `@[<2>%a@ : %a@]`" pp f (Fmt.opt pp) (ty f)
                pp x (Fmt.opt pp) (ty x);
            if not (equal ty_var t_type) then errorf "ill-formed polymorphic type@ `%a`" pp (ty_exn f);
            let ty = db_eval ~sub:x body in
                app_ f x ~ty
        | _ ->
            errorf "type error: cannot apply `@[<2>%a@ : %a@]`@ to `@[<2>%a@ : %a@]`" pp f (Fmt.opt pp) (ty f)
              pp x (Fmt.opt pp) (ty x)
      ...
      match T.view t with
          ...
          | T.AppBuiltin (b, l) -> (
              let ty = T.ty_exn t in
                  match b with
                      | Builtin.Greatereq | Builtin.Lesseq | Builtin.Less | Builtin.Greater | Builtin.Eq
                      | Builtin.Neq
