...
let parse_file file =
    ...
    let input = Parsing_utils.input_of_file file in
        ...
        do_extensions ~field:(fun e -> e.Extensions.post_parse_actions) ~x:parsed >>= fun () ->
    ...
    if !_sine_threshold < 0 || CCVector.length stmts < !_sine_threshold then stmts
      ...
      let filtered =
          Statement.sine_axiom_selector ~ignore_k_most_common_symbols:!_ignore_k_most_common_symbols
            ...
            ~trim_implications:!_trim_implications ~depth_start:!_sine_d_min ~depth_end:!_sine_d_max
...
let typing ~file prelude (input, stmts) =
    ...
    let def_as_rewrite = params.Params.def_as_rewrite in
        TypeInference.infer_statements ~on_var:(Input_format.on_var input)
          ...
          ~implicit_ty_args:(Input_format.implicit_ty_args input) ~def_as_rewrite ?ctx:None ~file
        ...
        let stmts = sine_filter stmts in
            ...
            >>= fun () ->
            do_extensions ~field:(fun e -> e.Extensions.post_typing_actions) ~x:stmts >>= fun () ->
...
let cnf ~sk_ctx decls =
    ...
    let stmts =
        ...
        |> Cnf.convert
    in
        do_extensions ~field:(fun e -> e.Extensions.post_cnf_actions) ~x:stmts >>= fun () ->
        Phases.return_phase stmts
    ...
    let cp =
        ...
        |> Compute_prec.set_weight_rule (fun stmts ->
               ...
               let clauses = Iter.map Statement.Seq.lits stmts in
                   Precedence.weight_fun_of_string ~signature ~clauses ~lm_w:!_lmb_w ~db_w:!_db_w !_kbo_wf
                     sym_depth)
...
let compute_ord_select precedence =
    ...
    let ord = Ordering.by_name !(params.Params.ord) precedence in
        ...
        let bool_select = Bool_selection.from_string ~ord params.Params.bool_select in
            do_extensions ~field:(fun e -> e.Extensions.ord_select_actions) ~x:(ord, fst select) >>= fun () ->
...
let print_stats_env (type c) (module Env : Env.S with type C.t = c) =
    ...
    let print_hashcons_stats what (sz, num, sum_length, small, median, big) =
        Format.printf
          "@[<h>%shashcons stats for %s: size %d, num %d, sum length %d, buckets: small %d, median %d, big \
           %d@]@."
...
let print_stats () =
    ...
    let print_gc () =
        let stats = Gc.stat () in
            Format.printf
              "@[<h>%sGC: minor words %.0f; major_words: %.0f; max_heap: %d; minor collections %d; major \
               collections %d@]@."
            ...
            let proof =
                if Env.params.Params.dot_all_roots then
                  ...
                  |> Iter.filter_map (fun c ->
                         if Literals.is_absurd (Env.C.lits c) then Some (Env.C.proof c) else None)
        ...
        match res with
            | Saturate.Unsat p when params.Params.check ->
                ...
                let stop = Util.total_time_s () in
                    Format.printf "%s(@[<h>proof_check@ :res %a@ :stats %a :time %.3fs@])@." comment
