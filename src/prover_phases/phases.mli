...
(** {1 Phases of the Prover}
    ...
    To process a file, the prover goes through a sequence of phases that are used to build values. This module
    reifies the phases. *)
...
(** [run_sequentiel l] runs each action of the list in succession, restarting every time with the initial state
    (once an action has finished, its state is discarded). Only the very last state is kept. If any errcode is
    non-zero, then the evaluation stops with this errcode *)
...
(** [run_with state m] executes the actions in [m] starting with [state], returning some value (or error) and the
    final state. *)
