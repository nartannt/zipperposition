  ...
  let walk_cstor_args (t : term) : term Iter.t =
      let rec aux t =
          match as_cstor_app t with
              | Some (_, l) -> Iter.of_list l |> Iter.flat_map (fun u -> Iter.cons u (aux u))
              | None -> Iter.empty
      in
          aux t
  ...
  let acyclicity_simplify c : C.t SimplM.t =
      ...
      in
          if Array.length lits' = Array.length (C.lits c) then SimplM.return_same c
            ...
            let proof =
                Proof.Step.inference ~rule:(Proof.Rule.mk "acyclicity") ~tags:[ Proof.Tag.T_data ]
                  [ C.proof_parent c ]
            in
            let c' = C.create_a ~trail:(C.trail c) ~penalty:(C.penalty c) lits' proof in
                Util.incr_stat stat_acyclicity;
                Util.debugf ~section 3 "@[<2>acyclicity:@ simplify `@[%a@]`@ into `@[%a@]`@]" (fun k ->
                    k C.pp c C.pp c');
                SimplM.return_new c'
      ...
      in
          ...
          |> Iter.map (fun (i, us) ->
                 ...
                 let new_c = C.create ~trail:(C.trail c) ~penalty:(C.penalty c) new_lits proof in
                     Util.incr_stat stat_acyclicity;
                     Util.debugf ~section 3 "@[<2>acyclicity@ :from `@[%a@]`@ :into `@[%a@]`@ :subst %a@]"
          ...
          match find_cstor_pair ~sign:true ~eligible c with
              | Some (idx, s1, l1, s2, l2) when ID.equal s1 s2 ->
                  ...
                  let clauses =
                      List.map
                        (fun lit ->
                          C.create ~trail:(C.trail c) ~penalty:(C.penalty c) (lit :: other_lits) proof)
                        new_lits
                  in
  ...
  let exhaustiveness (c : C.t) : C.t list =
      let mk_sub_skolem (t : term) (ty : Type.t) : ID.t =
          if Ind_ty.is_inductive_type ty then
            ...
            let depth =
                match T.view t with
                    | T.Const id -> Ind_cst.id_as_cst id |> CCOpt.map Ind_cst.depth |> CCOpt.map succ
                    | _ -> None
            in
      ...
      let make_axiom (t : term) : C.t =
          ...
          let ity, ty_params =
              match Ind_ty.as_inductive_type (T.ty t) with Some t -> t | None -> assert false
          in
