...
module Make (E : Env.S) : S with module Env = E = struct
    ...
    let rec aux sign f =
      ...
      | T.AppBuiltin (((Eq | Equiv | Neq | Xor) as hd), ([ _; a; b ] | [ a; b ])) when Type.is_prop (T.ty a) ->
        if Builtin.equal Eq hd || Builtin.equal Equiv hd then
          ...
          aux true (T.Form.and_ (T.Form.or_ (T.Form.not_ a) (T.Form.not_ b)) (T.Form.or_ a b))
  ...
  let rename_eq ~c ~should_rename lhs rhs sign =
    ...
    let yields_clauses f =
      match T.view f with
      | T.AppBuiltin ((Eq | Neq), [ ty; _; _ ]) -> Type.is_prop (Type.of_term_unsafe (ty :> InnerTerm.t))
  ...
  let cnf_scope_form form =
    ...
    let miniscope hd f =
      ...
      if T.is_fun f then (
        ...
        | T.AppBuiltin (Or, l) when Builtin.equal hd ExistsConst -> Some (distribute_quant Or (List.hd ty) l)
        | T.AppBuiltin (And, l) when Builtin.equal hd ForallConst -> Some (distribute_quant And (List.hd ty) l)
        | _ -> None
      ) else
        None
    in
    ...
    | T.AppBuiltin (And, (_ :: _ as l)) when kind = `Maxi ->
      ...
      | Some (ForallConst, ty, bodies) -> Some (T.Form.forall (T.fun_ ty (T.Form.and_l bodies)))
      | _ -> None)
    | T.AppBuiltin (Or, (_ :: _ as l)) when kind = `Maxi ->
      (match maxiscoping_eligible l with
      | Some (ExistsConst, ty, bodies) -> Some (T.Form.exists (T.fun_ ty (T.Form.or_l bodies)))
      | _ -> None)
    | T.AppBuiltin (((ExistsConst | ForallConst) as b), [ _; f ]) when kind = `Mini -> miniscope b f
    | _ -> None
           ...
           if L.is_predicate_lit lit then (
             ...
             | T.AppBuiltin (And, l) when List.length l >= 2 && should_clausify sign lhs ->
               ...
               if sign then
                 ...
                 return acc @@ mk_or ~proof_cons (List.map T.Form.not_ l) c i ~rule_name
             | T.AppBuiltin (Or, l) when List.length l >= 2 && should_clausify sign lhs ->
               ...
               if sign then
                 ...
                 return acc @@ mk_and ~proof_cons (List.map T.Form.not_ l) c i ~rule_name
             | T.AppBuiltin (Imply, [ a; b ]) when should_clausify sign lhs ->
               ...
               if sign then
                 ...
                 then
                   return acc @@ mk_or ~proof_cons [ T.Form.not_ a; b ] c i ~rule_name
                 else
                   continue acc
               else
                 return acc @@ mk_and ~proof_cons [ a; T.Form.not_ b ] c i ~rule_name
  ...
  let rename_subformulas c =
    ...
    let clausify_defs new_defs =
      List.fold_left
        (fun acc c ->
          lazy_clausify_driver ~ignore_eq:false ~force_clausification:true ~proof_cons c |> Iter.to_list
          |> fun l ->
    ...
    let should_rename sign f =
      let will_yield_claues f =
        let rec aux ~sign f =
          match T.view f with
          | T.AppBuiltin (Builtin.And, l) -> (sign && List.length l >= 2) || aux_l sign l
          | T.AppBuiltin (Builtin.Or, l) -> ((not sign) && List.length l >= 2) || aux_l sign l
          | T.AppBuiltin (Builtin.(ForallConst | ExistsConst), [ _; body ]) -> aux ~sign (snd (T.open_fun body))
          | T.AppBuiltin (Builtin.Not, [ body ]) -> aux ~sign:(not sign) body
          | T.AppBuiltin (Builtin.Imply, [ a; b ]) -> (not sign) || aux ~sign:(not sign) a || aux ~sign b
