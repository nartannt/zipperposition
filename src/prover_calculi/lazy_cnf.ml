...
module Make (E : Env.S) : S with module Env = E = struct
  ...
  let estimate_num_clauses sign limit f =
      ...
      let rec aux sign f =
          match T.view f with
              ...
              | T.AppBuiltin (And, l) -> if sign then sum_l sign l else prod_l sign l
              | T.AppBuiltin (Or, l) -> if sign then prod_l sign l else sum_l sign l
  ...
  let mk_and ~proof_cons ~rule_name and_args c ?(parents = [ c ]) lit_idx =
      ...
      let proof = proof ~constructor:proof_cons ~parents ~name:rule_name c in
          List.map
            (fun t -> C.create ~penalty:(C.penalty c) ~trail:(C.trail c) (L.mk_true t :: lits) proof)
            and_args
          ...
          let rec maxiscoping_eligible l =
              ...
              match l with
                  ...
                  | x :: xs ->
                      ...
                      if Builtin.equal b b' && Type.equal ty ty' then Some (b, ty, List.hd body :: bodies)
                      else None
          in
      ...
      let var_tys, body = T.open_fun f in
          ...
          let hd, f =
              if sign then (quant_hd, f)
                ...
                ((if quant_hd = ForallConst then ExistsConst else ForallConst), T.fun_ var_ty (T.Form.not_ body))
          in
                       ...
                       | T.AppBuiltin (((Equiv | Xor) as hd), [ a; b ]) when should_clausify sign lhs ->
                           let hd = if sign then hd else if hd = Equiv then Xor else Equiv in
                               if eligible_to_ignore_eq ~ignore_eq a b then continue acc
                                 ...
                                 let rule_name =
                                     CCFormat.sprintf "lazy_cnf_%s" (if hd = Equiv then "equiv" else "xor")
                                 in
                                     if hd = Equiv then
                                       return acc
