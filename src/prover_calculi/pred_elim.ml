          ...
          ID.Map.update sym
            (fun old ->
              ...
              | `Offending ->
                ...
                if TaskSet.in_heap entry && not (should_schedule entry) then TaskSet.remove_el _task_queue entry
        ...
        (fun cl ->
          ...
          match
            CCArray.find_idx
              (fun lit ->
                match get_sym_sign lit with
                | Some (sym', sign') -> ID.equal sym sym' && (CCOpt.is_none sign || CCOpt.get_exn sign == sign')
                | None -> false)
              (C.lits cl)
          with
          | Some (i, lit) ->
      ...
      | cl :: cls ->
        ...
        let new_cls =
          ...
          | None ->
            ...
            | Some _ -> CCList.filter_map (fun pos_cl -> get_resolver () ~sym ~pos_cl ~neg_cl:cl) pos
            | None ->
        ...
        | Some (pos_gates, neg_gates) ->
          if Env.flex_get k_prefer_spe && CS.is_empty task.offending_cls then (
            ...
            if measure_decreases () (Env.flex_get k_relax_val) task results then
              ...
              calc_resolvents ~sym ~pos:pos_gates ~neg:neg_cls @ calc_resolvents ~sym ~pos:pos_cls ~neg:neg_gates
          ) else
