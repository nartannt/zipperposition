  ...
  let pp_task out task =
      let original = ID.payload_pred ~f:(function ID.Attr_cnf_def -> true | _ -> false) task.sym in
          CCFormat.fprintf out
            "%a(%b) {@. +: @[%a@];@. -:@[%a@];@. ?:@[%a@]@. g:@[%a@]@. v^2:@[%g@]; |l|:@[%d@]; |%a|:@[%d@]; \
             h_idx: @[%d@] @.}@."
            ID.pp task.sym original (CS.pp C.pp) task.pos_cls (CS.pp C.pp) task.neg_cls (CS.pp C.pp)
            task.offending_cls
          ...
          | L.Equation (lhs, rhs, _) ->
              let sign = L.is_positivoid lit in
                  if Type.is_fun (T.ty lhs) then None
                    ...
                    if L.is_predicate_lit lit then CCOpt.map (fun hd_sym -> (hd_sym, sign)) (T.head lhs)
                    else None
                  else (
                    _logic := Equational;
                    None)
  ...
  let should_schedule task =
      let eligible_for_non_singular_pe task =
          ...
          match task.maybe_gate with
              | Some (pos, neg) ->
                  let limit =
                      if Env.flex_get k_max_resolvents < 0 then max_int else Env.flex_get k_max_resolvents
                  in
            ...
            match task.maybe_gate with
                | Some (pos_cls, neg_cls) ->
                    if CCList.mem ~eq:C.equal cl pos_cls || CCList.mem ~eq:C.equal cl neg_cls then (
                      ...
                      if
                        Env.flex_get k_non_singular_pe && TaskSet.in_heap task
                  ...
                  (function
                        | Some task ->
                            ...
                            let old = copy_task task in
                                if
                                  CS.mem cl task.offending_cls || mem_gate cl task.maybe_gate
                  ...
                  | Some (i, lit) ->
                      let free_vars = T.VarSet.of_list (L.vars lit) in
                          CCOpt.is_none
                            (CCArray.find_map_i
                               (fun j lit' ->
                                 if i = j || T.VarSet.subset (T.VarSet.of_list (L.vars lit')) free_vars then
                                   None
                                 else Some j)
                               (C.lits cl))
                  | _ -> false)
              ...
              in
                  if
                    List.exists
                      (fun pos_cl ->
                        List.exists
                          (fun neg_cl -> CCOpt.is_some @@ neq_resolver ~sym:task.sym ~pos_cl ~neg_cl)
                          neg_cls)
                      pos_cls
                  then None
          ...
          let find_definition_set cls =
              ...
              match SAT.check ~full:true () with
                  | Sat_solver.Unsat _ ->
                      ...
                      let parents =
                          List.map (fun p -> Proof.S.step @@ Proof.Parent.proof p) (Proof.Step.parents proof)
                      in
                          ...
                          let used_cls =
                              CCList.filter_map
                                (fun (_, _, cl) ->
                                  CCOpt.return_if (CCList.mem ~eq:Proof.Step.equal (C.proof_step cl) parents) cl)
                                cls
                          in
