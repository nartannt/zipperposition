               ...
               (fun old ->
                 ...
                 let old_ = copy_task entry in
                    (match action with
                       ...
                       | `Gates -> if handle_gates then entry.possible_gates <- CS.add cl entry.possible_gates);
           ...
           | cl :: cls ->
              ...
              let new_cls =
                 match find_lit_by_sym_opt true cl with
                    | Some _ -> CCList.filter_map (fun neg_cl -> get_resolver () ~sym ~pos_cl:cl ~neg_cl) neg
                    | None -> (
