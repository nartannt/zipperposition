    ...
    |> Iter.flat_map (fun (u_p, passive_pos) ->
        ...
        |> Iter.filter_map (fun (rule, us) ->
            ...
            Util.debugf ~section 3
              "@[<2>term narrowing:@ from `@[%a@]`@ to `@[%a@]`@ using rule `%a`@ and subst @[%a@]@]" (fun k ->
    ...
    | Some (clauses, r, subst, sc_r, renaming, tags) ->
      let proof =
        Proof.Step.simp ~rule:(Proof.Rule.mk "rw_clause") ~tags
          [
            C.proof_parent_subst renaming (c, 0) subst; RW.Rule.lit_as_proof_parent_subst renaming subst (r, sc_r);
          ]
      in
      let clauses = List.map (fun c' -> C.create_a ~trail:(C.trail c) ~penalty:(C.penalty c) c' proof) clauses in
      ...
      | _ ->
        ...
        |> List.map (fun rule_clause ->
            ...
            let idx_active =
              ...
              | _ -> assert false
            in
            let ctx = Literal.apply_subst_list renaming subst (CCArray.except_idx (C.lits c) idx_active, sc_a) in
            (* build new clause *)
            let proof =
              Proof.Step.inference
