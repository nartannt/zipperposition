...
let simpl_term t =
    let t', rules = RW.Term.normalize_term ~max_steps t in
        if T.equal t t' then (
          ...
          Util.debugf ~section 3 "@[<2>@{<green> simpl rewrite@} `@[%a@]`@ :into `@[%a@]`@ :using %a@]"
...
module Make (E : Env_intf.S) = struct
  ...
  let narrow_term_passive_ c : C.t list =
      ...
      let sc_rule = 1 in
      let sc_c = 0 in
          Literals.fold_terms ~vars:false ~subterms:true ~ty_args:false ~ord:(C.Ctx.ord ()) ~which:`All
            ~eligible (C.lits c)
              ...
              match Ordering.compare ord s' t' with
                  ...
                  | _ ->
                      ...
                      |> List.map (fun rule_clause ->
                             ...
                             let new_c =
                                 C.create (c_guard @ new_lits @ ctx) proof ~trail:(C.trail c) ~penalty
                             in
                                 Util.debugf ~section 4
                                   "(@[<2>ctx_narrow@ :rule %a[%d]@ :clause %a[%d]@ :pos %a@ :subst %a@ :yield \
                                    %a@])" (fun k ->
...
let post_cnf stmts st =
    CCVector.iter Statement.scan_stmt_for_defined_cst
      (if not !rewrite_before_cnf then stmts
         ...
         CCVector.filter
           (fun st -> match Statement.view st with Statement.Rewrite _ -> false | _ -> true)
           stmts);
            ...
            | Lemma fs -> (
                match aux_l fs with
                    | Some (fs', parents) ->
                        let rule = Proof.Rule.mk "definition expansion" in
                        let fs_parents =
                            List.map (fun f -> Proof.Parent.from (Proof.S.mk_f_esa ~rule f stmt_parents)) fs
                            @ parents
                        in
            ...
            | NegatedGoal (skolems, ngs) -> (
                match aux_l ngs with
                    | Some (ng', parents) ->
                        let rule = Proof.Rule.mk "definition expansion" in
                        let ng_parents =
                            List.map (fun f -> Proof.Parent.from (Proof.S.mk_f_esa ~rule f stmt_parents)) ngs
                            @ parents
                        in
            ...
            (fun stmt ->
              ...
              with Type.Conv.Error t ->
                Util.debugf ~section 1
                  "@[%a@] cannot be converted because it uses unsupported features (such as ite, let and \
                   others)" (fun k -> k Statement.pp_input stmt);
                stmt)
            stmts
    else stmts
