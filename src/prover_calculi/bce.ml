...
module Make (E : Env.S) : S with module Env = E = struct
  ...
  let resolvent_is_valid_neq lit_idx orig_cl partner =
                            ...
                            CCOpt.is_some
                              (CCArray.find_map_i
                                 (fun idx lit ->
                                   if
                                     idx != l_idx
                                     && are_opposite_lits_up_to ~subst (lit, sc_orig) (lit', sc_partner)
                                   then Some ()
                                   else None)
                                 (C.lits orig_cl)))
                          nonunifiable
      ...
      let lhs, sign =
          match lit with
              | L.Equation (lhs, _, _) when L.is_predicate_lit lit -> (lhs, L.is_positivoid lit)
  ...
  let resolvent_is_valid_eq lit_idx orig_cl partner =
      ...
      let are_opposite_lits_up_to ~cc l1 l2 =
          ...
          &&
          match (l1, l2) with
              | Equation (lhs, _, _), Equation (lhs', _, _) when L.is_predicate_lit l1 -> CC.is_eq cc lhs lhs'
              | Equation (lhs, rhs, _), Equation (lhs', rhs', _) ->
      ...
      let check_resolvents l_idx orig_cl (same_hd_atms, diff_hd_lits) =
          ...
          let orig_pos, orig_neg =
              CCList.partition L.is_positivoid (CCArray.except_idx (C.lits orig_cl) l_idx)
          in
                ...
                || (not (CCList.is_empty rest))
                   ...
                   in
                       ...
                       if CCList.is_empty congruent then false
                         ...
                         let cc =
                             List.fold_left (fun acc lhs -> add_flat_resolvent ~cc (T.args lhs)) cc congruent
                         in
                             check_lit ~cc rest
            in
            (* check if all l-resolvents are valid in polynomial time *)
                         ...
                         if idx' == lit_idx then None
                           ...
                           match lit' with
                               | L.Equation (lhs', _, _) when L.is_predicate_lit lit' ->
                                   ...
                                   let sign' = L.is_positivoid lit' in
                                       if Option.equal ID.equal sym' (Some hd_sym) && sign' == sign then
                                         Some lit'
                                       else None
                               | _ -> None)
                       (C.lits cl))
  ...
  let initialize_regular () =
      ...
      in
          (try
             ...
             List.iter
               (fun cl ->
                 CCArray.iteri (fun lit_idx _ -> register_task ~update_others:false lit_idx cl) (C.lits cl))
               init_clauses;
             (* eliminate clauses *)
             ignore (do_eliminate_blocked_clauses ());
