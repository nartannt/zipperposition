    ...
    let check_resolvents l_idx orig_cl (unifiable, nonunifiable) =
      ...
      if CCList.is_empty unifiable then
        ...
        let rec check_l_resolvents rest = function
          | ((lhs, subst) as x) :: xs -> check_lit lhs subst (rest @ xs) && check_l_resolvents (x :: rest) xs
          | [] -> true
        in
            ...
            List.exists
              (fun lit' ->
                CCOpt.is_some
                  (CCArray.find_map_i
                     (fun idx lit ->
                       if idx != l_idx && are_opposite_lits_up_to ~cc lit lit' then
                         Some ()
                       else
                         None)
                     (C.lits orig_cl)))
       ...
       if Env.flex_get k_processing_kind != `PreprocessingOnly || Env.flex_get k_fp_mode then (
         ...
         if not @@ Env.flex_get k_fp_mode then
           if Env.flex_get k_processing_kind = `InprocessingFull then
             ...
             Env.add_clause_elimination_rule ~priority:1 "BCE_SAT" eliminate_bce_sat
       ) else
