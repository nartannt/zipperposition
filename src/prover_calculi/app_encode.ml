    ...
    let ty' =
        match T.view ty with
            ...
            | T.AppBuiltin (Builtin.Arrow, ret :: args) when not (T.Ty.is_tType ret) ->
                ...
                in
                    ret_ty
            | T.AppBuiltin (f, args) -> T.app_builtin ~ty:T.tType f (CCList.map app_encode_ty args)
            | T.Const _ -> ty
            | T.Var _ -> ty
...
let app_encode_stmt stmt =
    ...
    let proof = Proof.Step.esa ~rule:(Proof.Rule.mk "app_encode") [ as_proof |> Proof.Parent.from ] in
        match Statement.view stmt with
            ...
            | Statement.Def defs ->
                let map_single =
                    Statement.map_def ~form:app_encode_lits ~term:(app_encode_term true) ~ty:app_encode_ty
                in
                    Statement.def ~proof (List.map map_single defs)
            | Statement.Rewrite def ->
                let new_def =
                    Statement.map_def_rule ~form:app_encode_lits ~term:(app_encode_term true) ~ty:app_encode_ty
                      def
                in
            ...
            | Statement.TyDecl (id, ty) -> Statement.ty_decl ~proof:Proof.Step.trivial id (app_encode_ty ty)
        ...
        if !mode_ != `None then (
          ...
          let seq =
              if !mode_ = `Extensional then
                Iter.append seq (Iter.of_list [ ty_decl_ext_diff; extensionality_axiom ])
              else seq
          in
