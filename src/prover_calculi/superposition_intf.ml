...
module type S = sig
  ...
  (** Extracts at most as many clauses from the stream queue as there are streams in the queue. If called with
      [~full=true] extracts only one clause but may loop forever. *)

  val extract_from_stream_queue_fix_stm : Env.generate_rule
  (** Same as [extract_from_stream_queue] with a different extraction heuristic If possible, all clauses are
      taken from the first stream *)
  ...
  (** semantic tautology deletion, using a congruence closure algorithm to see if negative literals imply some
      positive Literal.t *)
