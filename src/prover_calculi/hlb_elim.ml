...
module Make (E : Env.S) : S with module Env = E = struct
  ...
  let[@inline] get_predicate lit =
     match lit with
        | L.Equation (lhs, _, _) when L.is_predicate_lit lit -> Some (lhs, Lit.is_positivoid lit)
        | L.Equation (lhs, rhs, sign) when tracking_eq () -> Some (T.Form.eq lhs rhs, sign)
        | _ -> None
  ...
  let[@inline] flip_eq t =
     match T.view t with
        | T.AppBuiltin (Builtin.Eq, ([ a; b ] | [ _; a; b ])) when tracking_eq () -> T.Form.eq b a
        | T.AppBuiltin (Builtin.Neq, ([ a; b ] | [ _; a; b ])) when tracking_eq () -> T.Form.neq b a
        | _ -> t
  ...
  let[@inline] cl_is_ht_trackable cl =
     ...
     match C.lits cl with
        | [| l1; l2 |] -> CCOpt.is_some (get_predicate l1) && CCOpt.is_some (get_predicate l2)
        | _ -> false
  ...
  let get_unit_predicate cl =
     if Trail.is_empty (C.trail cl) then
       match C.lits cl with
          ...
          | [| L.Equation (lhs, rhs, sign) |] when tracking_eq () -> Some (lit_to_term (T.Form.eq lhs rhs) sign)
          | _ -> None
     else None
                                 ...
                                 | Some (j_lhs, j_sign) when CCBV.get bv j && i != j -> (
                                    ...
                                    let j_neg_t = lit_to_term ~negate:true j_lhs j_sign in
                                       (if Env.flex_get k_reduce_tautologies && C.length cl != 2 then
                                          match
                                            find_implication cl i_neg_t j_t <+> find_implication cl j_neg_t i_t
                                          with
                                       ...
                                       if Env.flex_get k_delete_lits then
                                         match
                                           find_implication cl i_neg_t j_neg_t <+> find_implication cl j_t i_t
                                         with
