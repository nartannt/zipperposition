                         ...
                         when List.length ss >= List.length args && List.length tt >= List.length args ->
                          ...
                          in
                                  ...
                                  |> List.for_all (fun idx ->
                                         match T.as_var (Array.get s_args idx) with
                                            | Some v ->
                                               ...
                                               (not
                                                  (CCArray.exists (T.var_occurs ~var:v) (Array.sub s_args 0 idx)))
                                               && not
                             ...
                             then
                               ...
                               let t_prefix =
                                  T.app g
                                    (Array.to_list (Array.sub t_args 0 (Array.length t_args - List.length args)))
                               in
                                  Some (head, t_prefix)
                             else None
                       | _ -> None)
                   ...
                   let new_lits =
                      List.mapi
                        (fun i lit ->
                          Lit.map
                            (fun t ->
                              Term.Map.fold (fun sk sk_v acc -> Term.replace ~old:sk ~by:sk_v acc) sk_with_vars t)
                            lit)
                        new_lits
                   in
                      ...
                      let new_clause = C.create ~trail:new_trail ~penalty new_lits proof in
                         ...
                         if not (List.for_all (Lit.for_all Term.DB.is_closed) new_lits) then (
                           CCFormat.printf
                             ...
                             (kind_to_str info.sup_kind) (Term.Set.cardinal lambdasup_vars) C.pp info.active sc_a
                             T.pp info.s T.pp info.t T.pp t' C.pp info.passive sc_p Lit.pp info.passive_lit
     ...
     let norm t = T.normalize_bools @@ Lambda.eta_expand @@ Lambda.snf t in
        ...
        then (
          CCFormat.printf
            ...
            info.passive info.scope_passive Lit.pp info.passive_lit Position.pp info.passive_pos US.pp info.subst;
                         ...
                         match Lits.View.get_eqn (C.lits active) s_pos with
                            | Some (s, t, true) ->
                               ...
                               then acc
                                 ...
                                 let var_w = HVar.fresh ~ty:(Type.arrow arg_types (T.ty t)) () in
                                 let var_z =
                                    HVar.fresh ~ty:(Type.arrow (List.append arg_types [ T.ty t ]) (T.ty u_p)) ()
                                 in
                                 let db_args = List.mapi (fun i ty -> T.bvar ~ty (n - 1 - i)) arg_types in
                                 let term_w, term_z = (T.var var_w, T.var var_z) in
                                 let w_db = T.app term_w db_args in
  ...
  let demod_nf (st : demod_state) c t : T.t =
              ...
              |> Iter.find (fun (l, r, (_, _, sign, unit_clause), subst) ->
                     ...
                     let r' = norm @@ Subst.FO.apply Subst.Renaming.none subst (r, cur_sc) in
                        ...
                        then (
                          ...
                          let l' =
                             Lambda.eta_expand @@ norm_b @@ Subst.FO.apply Subst.Renaming.none subst (l, cur_sc)
                          in
                             (* sanity checks *)
                             ...
                             in
                                Util.debugf ~section 2
                                  "@[<2>demod(%d):@ rewrite `@[%a@]`@ into `@[%a@]`@ resulting `@[%a@]`@ nf \
                                   `@[%a@]` using %a[%d]@]" (fun k ->
     ...
     *)
     ...
     and normal_form_l l k =
        match l with
           | [] -> k []
           | t :: tail -> normal_form ~toplevel:false t (fun t' -> normal_form_l tail (fun l' -> k (t' :: l')))
     in
                ...
                let new_lits = CCList.uniq ~eq:Lit.equal_com new_lits in
                   if (not !has_changed) && List.length new_lits = Array.length lits then (
                     ...
                     let new_clause = C.create ~trail:(C.trail c) ~penalty:(C.penalty c) new_lits proof in
                        Util.debugf ~section 3 "@[<>@[%a@]@ @[<2>basic_simplifies into@ @[%a@]@]@ with @[%a@]@]"
