...
module Make (Env : Env.S) : S with module Env = Env = struct
              ...
              let rec drip_stream i stm =
                  let mk_stm stm = Stm.make ~penalty ~parents stm in
                      if i = 0 then aux (clauses, mk_stm stm :: streams) xs
                        ...
                        match stm () with
                            | OSeq.Nil -> aux acc xs
                            | OSeq.Cons (Some cl, stm') -> aux (cl :: clauses, mk_stm stm' :: streams) xs
          ...
          | Some v ->
              ...
              |> Iter.exists (fun (t, _) ->
                     match T.view t with
                         ...
                         | Fun (_, body) -> Iter.exists (HVar.equal Type.equal v) (T.Seq.vars body)
                         | _ -> false)
          | None -> false
        ...
        let var' = S.FO.apply renaming subst (var, info.scope_passive) in
            if (not (Type.is_fun (Term.ty var'))) || not (O.might_flip ord var' replacement') then (
              ...
              let unique_args_of_var c =
                  C.lits c
                  |> Lits.fold_terms ~vars:true ~ty_args:false ~which:`All ~ord ~subterms:true
                       ~eligible:(fun _ _ -> true)
                  |> Iter.fold_while
                       (fun unique_args (t, _) ->
              ...
              in
                  match unique_vars with
                      ...
                      | None ->
                          ...
                          let passive'_lits =
                              Lits.apply_subst renaming subst (C.lits info.passive, info.scope_passive)
                          in
                          ...
                          in
                              ...
                              then (
                                Util.debugf ~section 3 "Sup at var condition is not fulfilled because: %a >= %a"
          ...
          try
            ...
            in
                Util.debugf ~section 1
                  "@[<2>sup, kind %s(%d)@ (@[<2>%a[%d]@ @[s=%a@]@ @[t=%a, t'=%a@]@])@ (@[<2>%a[%d]@ \
                   @[passive_lit=%a@]@ @[p=%a@]@])@ with subst=@[%a@]@]" (fun k ->
                    k (kind_to_str info.sup_kind) (Term.Set.cardinal lambdasup_vars) C.pp info.active sc_a T.pp
                      info.s T.pp info.t T.pp t' C.pp info.passive sc_p Lit.pp info.passive_lit Position.pp
                ...
                (let exit_negative_tl =
                     ExitSuperposition
                       ("negative literal must paramodulate " ^ "into top-level positive position")
                 in
                 let exit_double_sup =
                ...
                let subst', new_sk =
                    if info.sup_kind = LambdaSup then S.FO.unleak_variables subst else (subst, [])
                in
                ...
                let new_trail = C.trail_l [ info.active; info.passive ] in
                    ...
                    let s' = S.FO.apply ~shift_vars renaming subst (info.s, sc_a) in
                        ...
                        in
                            ...
                            let tags =
                                (if subst_is_ho || info.sup_kind != Classic then [ Proof.Tag.T_ho ] else [])
                                @ Unif_subst.tags us
                            in
                            let proof =
                            ...
                            let new_clause = C.create ~trail:new_trail ~penalty new_lits proof in
                                ...
                                if not (List.for_all (Lit.for_all Term.DB.is_closed) new_lits) then (
                                  CCFormat.printf
                                    ...
                                    (kind_to_str info.sup_kind) (Term.Set.cardinal lambdasup_vars) C.pp
                                    info.active sc_a T.pp info.s T.pp info.t T.pp t' C.pp info.passive sc_p
  ...
  let do_superposition info =
      ...
      let norm t = T.normalize_bools @@ Lambda.eta_expand @@ Lambda.snf t in
          if info.sup_kind != SubVarSup && not (Term.equal (norm @@ s) (norm @@ u_p) || US.has_constr info.subst)
          then (
            CCFormat.printf
              "@[<2>sup, kind %s@ (@[<2>%a[%d]@ @[s=%a@]@ @[t=%a@]@])@ (@[<2>%a[%d]@ @[passive_lit=%a@]@ \
               @[p=%a@]@])@ with subst=@[%a@]@].\n"
              (kind_to_str info.sup_kind) C.pp info.active info.scope_active T.pp info.s T.pp info.t C.pp
              info.passive info.scope_passive Lit.pp info.passive_lit Position.pp info.passive_pos US.pp
              info.subst;
  ...
  let infer_passive_aux ~retrieve_from_index ~process_retrieved clause =
      ...
      let new_clauses =
          Lits.fold_terms ~vars:(Env.flex_get k_sup_at_vars) ~var_args:(Env.flex_get k_sup_in_var_args)
            ~fun_bodies:(Env.flex_get k_sup_under_lambdas) ~subterms:true ~ord ~which:`Max ~eligible
                             ...
                             | Some (s, t, true) ->
                                 ...
                                 in
                                     Util.debugf ~section 10
                                       "[Trying lambdasup from %a into %a with term %a into term %a]" (fun k ->
                                         k C.pp active C.pp clause T.pp s T.pp u_p);
                                     do_superposition info
                             | _ -> None
                 in
                     (* all terms that occur in an equation in the active_set
                   ...
                   I.fold !_idx_fluidsup_into
                     (fun acc u_p with_pos ->
                       ...
                       let var_h =
                           T.var
                             (HVar.fresh
                                ~ty:(Type.arrow [ T.ty s ] (Type.var (HVar.fresh ~ty:Type.tType ())))
                                ())
                       in
  ...
  let infer_fluidsup_passive clause =
      ...
      let new_clauses =
          if fluidsup_applicable clause then
            Lits.fold_terms ~vars:true ~var_args:false ~fun_bodies:false ~subterms:true ~ord ~which:`Max
                   ...
                   (fun acc u_p with_pos ->
                     ...
                     then acc
                       ...
                       let y_subst_val = T.fun_l arg_types z_db in
                           ...
                           let subst_y =
                               US.FO.bind US.empty (var_up, scope_passive) (y_subst_val, scope_passive)
                           in
  ...
  let infer_dupsup_passive clause =
      ...
      (* do the inferences in which clause is passive (rewritten),
         so we consider both negative and positive literals *)
      let new_clauses =
          Lits.fold_terms ~vars:false ~var_args:false ~fun_bodies:false ~subterms:true ~ord ~which:`Max
            ~eligible ~ty_args:false (C.lits clause)
                             ...
                             match Lits.View.get_eqn (C.lits active) s_pos with
                                 | Some (s, t, true) ->
                                     ...
                                     then acc
                                       ...
                                       let var_z =
                                           HVar.fresh
                                             ~ty:(Type.arrow (List.append arg_types [ T.ty t ]) (T.ty u_p))
                                             ()
                                       in
                                       let db_args = List.mapi (fun i ty -> T.bvar ~ty (n - 1 - i)) arg_types in
                                       let term_w, term_z = (T.var var_w, T.var var_z) in
  ...
  let infer_equality_resolution_complete_ho clause =
      ...
      in
          if Env.should_force_stream_eval () then Env.get_finite_infs inf_res
            ...
            let cls, stm_res =
                force_getting_cl (List.map (fun stm -> (C.penalty clause, [ clause ], stm)) inf_res)
            in
                StmQ.add_lst (Env.get_stm_queue ()) stm_res;
                cls
  ...
  *)
                                       ...
                                       Util.debugf ~section 2
                                         "@[<2>demod(%d):@ rewrite `@[%a@]`@ into `@[%a@]`@ resulting \
                                          `@[%a@]`@ nf `@[%a@]` using %a[%d]@]" (fun k ->
                                           k (C.id c) T.pp t T.pp r T.pp r' T.pp (Lambda.snf r') Subst.pp subst
                                             cur_sc);
                                       Some r')
                             else (
                  ...
                  if not (Env.flex_get k_quant_demod) then reduce_at_root ~toplevel t k
                    ...
                    let vars, unfolded = T.open_fun body in
                        normal_form ~toplevel:false unfolded (fun unfolded' ->
                            let u =
                                if T.equal unfolded unfolded' then t else mk_quant (T.fun_l vars unfolded')
                            in
                                reduce_at_root ~toplevel u k)
              | T.AppBuiltin (b, l) ->
                  normal_form_l l (fun l' ->
      ...
      let cc =
          Array.fold_left
            (fun cc lit ->
              match lit with
                  (* NOTE: Based on the representation of the literals *)
                  | Lit.Equation (l, r, true) when T.equal r T.false_ -> Congruence.FO.mk_eq cc l T.true_
                ...
                let us = ref US.empty in
                    ...
                    let new_lits = CCList.uniq ~eq:Lit.equal_com new_lits in
                        if (not !has_changed) && List.length new_lits = Array.length lits then (
                          ...
                          let new_lits =
                              if List.exists Lit.is_trivial new_lits then [ Lit.mk_tauto ] else new_lits
                          in
                          let new_clause = C.create ~trail:(C.trail c) ~penalty:(C.penalty c) new_lits proof in
                              Util.debugf ~section 3
                                "@[<>@[%a@]@ @[<2>basic_simplifies into@ @[%a@]@]@ with @[%a@]@]" (fun k ->
          ...
          match simplifier c with
              ...
              | Some (new_lits, premises) ->
                  ...
                  let new_c = C.create ~trail ~penalty new_lits proof in
                      Util.debugf ~section 3 "@[@[%a@]@ pos_simplify_reflect into @[%a@]@]" (fun k ->
                          k C.pp c C.pp new_c);
                      ZProf.exit_prof _span;
                      SimplM.return_new new_c
  ...
  let negative_simplify_reflect c =
      ...
      let lits, premises = iterate_lits [] (C.lits c |> Array.to_list) [] in
          if List.length lits = Array.length (C.lits c) then (
            ...
            let proof =
                Proof.Step.simp ~rule:(Proof.Rule.mk "simplify_reflect-") (C.proof_parent c :: premises)
            in
            let new_c = C.create ~trail:(C.trail c) ~penalty:(C.penalty c) lits proof in
                Util.debugf ~section 3 "@[@[%a@]@ neg_simplify_reflect into @[%a@]@]" (fun k ->
                    k C.pp c C.pp new_c);
                ZProf.exit_prof _span;
                SimplM.return_new new_c
  ...
  let all_lits_match a sc_a b sc_b =
      CCArray.for_all
        (fun lita ->
          CCArray.exists (fun litb -> not (Iter.is_empty (Lit.subsumes (lita, sc_a) (litb, sc_b)))) b)
        a
  ...
  let eq_subsumes_with (a, sc_a) (b, sc_b) =
      ...
      and equate_terms a b u v =
          match anti_unify u v with None -> None | Some (u', v') -> equate_root a b u' v'
        ...
        let remove_one_lit lits =
            ...
            |> Iter.find_map (fun (i, old_lit) ->
                   ...
                   |> Iter.find_map (fun c' ->
                          let subst =
                              match
                                if try_eq_subsumption then eq_subsumes_with (C.lits c', 1) (lits, 0) else None
                              with
                                  | Some s -> Some (s, [])
        ...
        in
            match remove_one_lit (Array.copy (C.lits c)) with
                ...
                | Some (new_lits, _, c', subst, tags) ->
                    ...
                    let new_c = C.create ~trail:(C.trail c) ~penalty:(C.penalty c) new_lits proof in
                        Util.debugf ~section 3
                          "@[<2>contextual literal cutting@ in @[%a@]@ using @[%a@]@ gives @[%a@]@]" (fun k ->
                            k C.pp c C.pp c' C.pp new_c);
      ...
      if Array.length (C.lits c) <= 1 || Lits.num_equational (C.lits c) > 3 || Array.length (C.lits c) > 8 then
        ...
        let n = Array.length lits in
            ...
            with CondensedInto (new_lits, subst, renaming, tags) ->
              ...
              let proof =
                  Proof.Step.simp ~rule:(Proof.Rule.mk "condensation") ~tags
                ...
                match (pos_lit, neg_lit) with
                    | Equation (x, y, true), Equation (lhs, rhs, sign) ->
                        ...
                        let (sk_id, sk_ty), inv_sk =
                            Term.mk_fresh_skolem (List.map T.as_var_exn same_vars)
  ...
  let normalize_equalities c =
      ...
      let normalized = List.map Literal.normalize_eq lits in
          if List.exists CCOpt.is_some normalized then
            ...
            in
            let proof =
                Proof.Step.simp [ C.proof_parent c ] ~rule:(Proof.Rule.mk "simplify nested equalities")
            in
            let new_c = C.create ~trail:(C.trail c) ~penalty:(C.penalty c) new_lits proof in
                SimplM.return_new new_c
          else SimplM.return_same c
    ...
    Params.add_opts
      [
        ...
        ( "--fluidsup",
          ...
          " enable/disable FluidSup inferences (only effective when complete higher-order unification is \
           enabled)" );
        ...
        ( "--lambdasup",
          Arg.Int
            (fun l ->
                if l < 0 then
                  raise (Util.Error ("argument parsing", "lambdaSup argument should be non-negative"));
                _lambdasup := l),
