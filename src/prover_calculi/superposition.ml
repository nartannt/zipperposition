...
module Make (Env : Env.S) : S with module Env = Env = struct
  ...
  let do_classic_superposition info =
    ...
    try
      ...
      (let exit_negative_tl =
       ...
       | _ ->
         if
           T.equal t' T.false_ && ((not (Env.flex_get k_superpose_w_formulas)) || (not @@ T.is_appbuiltin info.s))
         then
           raise @@ exit_negative_tl);
      ...
      then (
        ...
        let c3 = (not bool_inference) && not (BV.get (C.eligible_res (info.passive, sc_p) subst) passive_idx) in
  ...
  let infer_lambdasup_into clause =
    ...
    let new_clauses =
      ...
      |> Iter.flat_map (fun (u_p, passive_pos) ->
          ...
          (* all terms that occur in an equation in the active_set
                     and that are potentially unifiable with u_p (u at position p) *)
          I.retrieve_unifiables (!_idx_sup_from, 1) (u_p, 0) |> Iter.filter_map (fun (t, p, s) -> do_sup t p s))
      |> Iter.to_rev_list
    in
    ZProf.exit_prof _span;
    new_clauses
        ...
        |> Iter.flat_map (fun (s, t, _, s_pos) ->
            I.fold !_idx_fluidsup_into
              (fun acc u_p with_pos ->
                ...
                in
                let penalty =
                  max (C.penalty clause) (C.penalty with_pos.C.WithPos.clause) + Env.flex_get k_fluidsup_penalty
                in
                (* /!\ may differ from the actual penalty (by -2) *)
      ...
      |> Iter.flat_map (fun (s, t, _, s_pos) ->
          I.fold !_idx_dupsup_into
            (fun acc u_p with_pos ->
              ...
              assert (T.DB.is_closed u_p);
              if T.Seq.vars s |> Iter.append (T.Seq.vars t) |> Iter.exists (fun v -> Type.is_tType (HVar.ty v))
              then
                acc
              else (
            ...
            if eligible (* subst(lit) is maximal, we can do the inference *) then (
              ...
              let penalty =
                if C.penalty clause = 1 then
                  ...
                  C.penalty clause + 1
              in
              let proof = Proof.Step.inference ~rule ~tags [ C.proof_parent_subst renaming (clause, 0) subst ] in
  ...
  *)
  ...
  (** Compute normal form of term w.r.t active set. Clauses used to rewrite are added to the clauses hashset. *)
    ...
    let rec reduce_at_root ~toplevel t k =
      ...
      let step =
        ...
        |> Iter.find (fun (l, r, (_, _, sign, unit_clause), subst) ->
            ...
            then (
              Util.debugf ~section 3 "@[<hv2>demod(%d):@ @[<hv>t=%a[%d],@ l=%a[%d],@ r=%a[%d]@],@ subst=@[%a@]@]"
                ...
                |> Iter.fold
                     (fun subst v ->
                       if S.mem subst (v, cur_sc) then
                         ...
                         S.bind subst (v, cur_sc) (InnerTerm.var (HVar.fresh ~ty:(HVar.ty v) ()), cur_sc))
                     subst
              in
              Util.debugf ~section 2
                "@[<2>demod(%d):@ rewrite `@[%a@]`@ into `@[%a@]`@ resulting `@[%a@]`@ nf `@[%a@]` using \
                 %a[%d]@]" (fun k -> k (C.id c) T.pp t T.pp r T.pp r' T.pp (Lambda.snf r') Subst.pp subst cur_sc);
              Some r'
            ) else (
  ...
  let formula_simplify_reflect c =
    ...
    if Env.flex_get k_formula_simplify_reflect && !Lazy_cnf.enabled then (
      let lits =
        List.map
          (function
            | Lit.Equation (lhs, rhs, sign) -> Lit.mk_lit (do_sr lhs) (do_sr rhs) sign
            | x -> x)
      ...
      let premises =
        CCArray.foldi
          (fun premises i lit ->
            ...
            | Lit.Equation (lhs, rhs, true) when T.equal T.false_ rhs -> find_simplifying_premise lhs T.true_
            | _ -> premises)
  ...
  (** Compare literals by subsumption difficulty (see "towards efficient subsumption", Tammet). We sort by
      increasing order, so non-ground, deep, heavy literals are smaller (thus tested early) *)
  ...
  (** Check whether [a] subsumes [b], and if it does, return the corresponding substitution *)
  ...
  let subsumes a b =
    ...
    if (not @@ Env.flex_get k_solid_subsumption) || Env.flex_get Combinators.k_enable_combinators then
      ...
      try SS.subsumes a b with SolidSubsumption.UnsupportedLiteralKind -> subsumes_classic a b
    )
