                      ...
                      let sc = 0 in
                         match find_ho_disagremeents (lhs, sc) (rhs, sc) with
                            | Some (disagremeents, subst) ->
                               ...
                               in
                               let proof =
                                  Proof.Step.inference [ C.proof_parent c ] ~rule:(Proof.Rule.mk "ext_eqres")
                               in
        ...
        match T.view t with
           | T.App (hd, [ arg ]) -> (
              if Term.is_var hd && inst_vars then
                ...
                let choice_ops =
                   Term.Map.filter (fun t _ -> Type.equal (Term.ty t) hd_ty) !choice_ops |> Term.Map.to_list
                    ...
                    let hd, args = T.as_app lhs in
                       ...
                       in
                          ...
                          @@ CCList.flat_map_i
                               (fun i arg_i ->
                                 CCList.mapi
                                   (fun j arg_j -> if j > i && T.equal arg_i arg_j then return i j else None)
                                   args)
                               args)
                          @ cmd_list)
                  else cmd_list)
               ...
               if i < List.length args then
                 let t = List.nth args i in
                    match arg_opt with
                       | Some arg_l ->
                          let res_l =
                             if all_covers then T.cover_with_terms t arg_l else [ t; T.max_cover t arg_l ]
                          in
                             T.Set.of_list res_l
                       | None -> Term.Set.empty
               else Term.Set.empty)
