...
module Make (E : Env.S) : S with module Env = E = struct
  ...
  let retrieve_from_extdec_idx idx id =
    ...
    | None -> Iter.empty
    | Some cl_map ->
      C.Tbl.to_iter cl_map |> Iter.flat_map (fun (c, l) -> Iter.of_list l |> Iter.map (fun (t, p) -> c, t, p))
    ...
    if ext_rule_eligible given then (
      ...
      |> Iter.flat_map (fun (l, _, sign, pos) ->
          ...
          if T.is_const hd && T.has_ho_subterm l then (
            ...
            Iter.map (fun (into_c, into_t, into_p) -> do_ext_sup given pos l into_c into_p into_t) inf_partners
          ) else
            Iter.empty)
    ...
    let build_choice_inst t =
      ...
      | T.App (hd, [ arg ]) ->
        if Term.is_var hd && inst_vars then (
          ...
          let choice_ops =
            Term.Map.filter (fun t _ -> Type.equal (Term.ty t) hd_ty) !choice_ops |> Term.Map.to_list |> fun l ->
            ...
            let new_cls =
              ...
              |> Iter.flat_map_l (fun pas_cl ->
                  if C.id pas_cl = C.id c then
                    ...
                    insantiate_choice ~inst_vars:false ~choice_ops:(ref (Term.Map.singleton sym (Some c))) pas_cl)
  ...
  let complete_eq_args (c : C.t) : C.t list =
    ...
    let new_c =
      ...
      |> Iter.flat_map_l (fun (lit_idx, lit) ->
          match lit with
          | Literal.Equation (t, u, true) when Type.is_fun (T.ty t) -> aux ~poly:false (C.lits c) lit_idx t u
          | Literal.Equation (t, u, true) when Type.is_var (T.ty t) && not is_poly_arg_cong_res ->
            ...
            let res =
              ...
              |> OSeq.mapi (fun arrarg_idx arrow_args ->
                  ...
                  let funty = T.of_ty (Type.arrow arrow_args (Type.var (HVar.fresh ~ty:Type.tType ()))) in
  ...
  (** Removal of fixed/duplicate arguments of variables.
      - If within a clause, there exists a variable F that's always applied to at least i arguments and the ith
        argument is always the same DB-free term, we can systematically remove the argument (and repair F's
        type).
      - If within a clause, there exist a variable F, and indices i < j such that all occurrences of F are
        applied to at least j arguments and the ith argument is syntactically same as the jth argument, we can
