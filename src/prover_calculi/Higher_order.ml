...
module Make (E : Env.S) : S with module Env = E = struct
      ...
      |> Iter.find_map (fun (lit', skolems) ->
             let subst = Literal.variant (lit', 0) (lit, 1) |> Iter.head in
                 match subst with
                     | Some (subst, _) ->
                         let skolems =
                             List.map (fun t -> Subst.FO.apply Subst.Renaming.none subst (t, 0)) skolems
                         in
                             Some skolems
                     | None -> None)
  ...
  let remove_ff_constraints c =
      ...
      let is_neg_ff = function
          | Literal.Equation (lhs, rhs, false) -> T.is_var (T.head_term lhs) && T.is_var (T.head_term rhs)
          | _ -> false
      in
      ...
      let blocked_vars =
          CCArray.filter (fun l -> not @@ is_neg_ff l) (C.lits c) |> Literals.vars |> VS.of_list
      in
  ...
  let neg_ext (c : C.t) : C.t list =
      ...
      let mk_clause ~lits ~penalty =
          ...
          in
          let new_c =
              C.create ~penalty:(C.penalty c + penalty) ~trail:(C.trail c) (CCArray.to_list lits) proof
          in
  ...
  let update_ext_dec_indices f c =
      ...
      in
          if Env.flex_get k_ext_rules_kind != `Off && ext_rule_eligible c then (
            Lits.fold_terms ~vars:false ~var_args:false ~fun_bodies:false ~ty_args:false ~ord ~which
              ~subterms:true ~eligible (C.lits c)
            ...
            let eligible =
                if Env.flex_get k_ext_dec_lits = `OnlyMax then C.Eligible.param c else C.Eligible.always
            in
          ...
          let sc = 0 in
              match find_ho_disagremeents (s, sc) (u, sc) with
                  | Some (disagrements, subst) ->
                      ...
                      in
                      let proof =
                          Proof.Step.inference [ C.proof_parent cl ] ~rule:(Proof.Rule.mk "ext_eqfact")
                      in
                      (* ext_eqfact is rarely used *)
                      let new_c =
                          C.create ~trail:(C.trail cl) ~penalty:(C.penalty cl + C.proof_depth cl) new_lits proof
                      in
                          [ new_c ]
                  | None -> []
      in
                 ...
                 match lit with
                     | Lit.Equation (s, t, _)
                       when Lit.is_positivoid lit
                            && (Env.flex_get k_ext_dec_lits != `OnlyMax || BV.get maximal i) ->
  ...
  let ext_sup_pas given =
      if ext_rule_eligible given then
        (let which, eligible =
         ...
         in
             Lits.fold_terms ~vars:false ~var_args:false ~fun_bodies:false ~ty_args:false ~ord ~which
               ~subterms:true ~eligible (C.lits given)
                    ...
                    let hd, args = T.as_app t in
                        if T.is_const hd && T.has_ho_subterm t then
                          let inf_partners = retrieve_from_extdec_idx !_ext_dec_from_idx (T.as_const_exn hd) in
                              Iter.map
                                (fun (from_c, from_t, from_p) -> do_ext_sup from_c from_p from_t given p t)
                                inf_partners
                        else Iter.empty))
        |> Iter.filter_map CCFun.id |> Iter.to_list
      else []
          ...
          if ext_rule_eligible c then (
            let res =
                ...
                |> CCList.flat_map (fun (lhs, rhs, sign, pos) ->
                       assert (sign = false);
                       let idx = Lits.Pos.idx pos in
                           if Env.flex_get k_ext_dec_lits != `OnlyMax || BV.get (C.eligible_res_no_subst c) idx
          ...
          let offset = C.Seq.vars c |> T.Seq.max_var |> succ in
              ...
              |> Iter.flat_map_l (fun v ->
                     HO_unif.enum_prop ~add_var:(Env.flex_get k_prim_enum_add_var)
                       ~enum_cache:(Env.flex_get k_prim_enum_terms) ~signature:(Ctx.signature ()) ~mode ~offset
                       (v, sc_c))
              |> Iter.map (fun (subst, penalty) ->
                     ...
                     let new_c = C.create_a lits proof ~penalty:(C.penalty c + penalty) ~trail:(C.trail c) in
                         Util.debugf ~section 1 "(@[<hv2>ho.refine@ :from %a@ :subst %a@ :yields %a@])"
  ...
  let insantiate_choice ?(proof_constructor = Proof.Step.inference) ?(inst_vars = true)
      ...
      let is_choice_subterm t =
          match T.view t with
              ...
              | T.AppBuiltin (Builtin.ChoiceConst, l) -> CCList.length l == 2 && T.DB.is_closed t
              | _ -> false
      in
        ...
        |> Iter.fold
             (fun var_map subterm ->
               match T.view subterm with
                   | T.App (hd, args)
                     when T.is_var hd
                          && List.exists (fun t -> (not (Type.is_fun (T.ty t))) && not (T.is_bvar t)) args ->
      ...
      let offset = C.Seq.vars c |> T.Seq.max_var |> succ in
          ...
          |> List.map (fun (new_pairs, us, penalty, renaming) ->
                 ...
                 let new_c = C.create all_lits proof ~trail:(C.trail c) ~penalty:(C.penalty c + penalty) in
                     Util.debugf ~section 5 "(@[ho_unif.step@ :pairs (@[%a@])@ :subst %a@ :yields %a@])"
      ...
      if C.lits c |> CCArray.exists Literal.is_ho_constraint then (
        ...
        let pairs, others =
            ...
            |> CCList.partition_map (function
                   | Literal.Equation (t, u, false) as lit when Literal.is_ho_constraint lit -> `Left ([], t, u)
                   | lit -> `Right lit)
        in
            assert (pairs <> []);
  ...
  let mk_diff_const () =
      let diff_id = ID.make "zf_ext_diff" in
          ...
          let diff_type =
              Type.forall_fvars [ alpha_var; beta_var ] (Type.arrow [ alpha_to_beta; alpha_to_beta ] alpha)
          in
  ...
  let early_bird_prim_enum cl var =
      ...
      HO_unif.enum_prop ~enum_cache:(Env.flex_get k_prim_enum_terms) ~add_var ~signature:(Ctx.signature ())
        ~mode ~offset
        (T.as_var_exn var, sc)
                     ...
                     | Literal.Equation (t, u, true) when Type.is_var (T.ty t) && not is_poly_arg_cong_res ->
                         ...
                         let res =
                             ...
                             |> OSeq.mapi (fun arrarg_idx arrow_args ->
                                    ...
                                    let funty =
                                        T.of_ty
                                          (Type.arrow arrow_args (Type.var (HVar.fresh ~ty:Type.tType ())))
                                    in
              ...
              |> CCList.filter_map (fun (var, var_status) ->
                     ...
                     in
                         if CCList.for_all (( = ) true) keep then None
                           ...
                           (* Create substitution: *)
                           let ty_args' =
                               ty_args |> CCList.combine keep |> CCList.filter fst |> CCList.map snd
                           in
        ...
        (if Env.flex_get Combinators.k_enable_combinators then
           ...
           let ho_norm t =
               ...
               match opt with
                   ...
                   | Some t' -> ( match eta_normalize t' with None -> Some t' | Some tt -> Some tt)
           in
              ...
              CCVector.iter
                (fun c ->
                  match Statement.get_rw_rule c with
                      | Some (sym, r) ->
                          Util.debugf ~section 1 "@[<2> Adding constant def rule: `@[%a@]`@]" (fun k ->
                              k Rewrite.Rule.pp r);
...
let () =
    Options.add_opts
      [
        ...
        ( "--ext-decompose-lits",
          Arg.Symbol
            ([ "all"; "max" ], fun str -> _ext_dec_lits := if String.equal str "all" then `All else `OnlyMax),
