...
let bool_encode_term t_orig =
    let rec aux t =
        ...
        with Invalid_argument ty_err ->
          let err =
              CCFormat.sprintf "Subterm @[%a@]:@[%a@] of @[%a@] cannot be encoded because of type error: @[%s@]"
...
let bool_encode_stmt stmt =
    ...
    let res =
        match Statement.view stmt with
            ...
            | Statement.Def def ->
                let map_single =
                    Statement.map_def ~form:bool_encode_lits ~term:bool_encode_term ~ty:bool_encode_ty
                in
            ...
            | Statement.TyDecl (id, ty) -> Statement.ty_decl ~proof:Proof.Step.trivial id (bool_encode_ty ty)
    in
        res
