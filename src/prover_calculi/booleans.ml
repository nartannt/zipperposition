...
module Make (E : Env.S) : S with module Env = E = struct
  ...
  let get_triggers c =
    ...
    let rec get_terms t k =
      ...
      | T.AppBuiltin ((ForallConst | ExistsConst), [ _; body ]) -> if Env.flex_get k_include_quants then k body
      | _ -> ()
    in
            ...
            |> OSeq.map (fun unif_subst_opt ->
                CCOpt.map
                  (fun unif_subst ->
                    ...
                    let renaming = Subst.Renaming.create () in
                    let new_lits = Literals.apply_subst renaming subst (mk_sc new_lits) |> CCArray.to_list in
                    let rule =
                      Proof.Rule.mk
  ...
  let setup () =
    ...
    | _ ->
      ...
      if Env.flex_get k_bool_reasoning = BoolHoist then (
        ...
        if Env.flex_get k_rename_nested_bools then Env.add_multi_simpl_rule ~priority:500 rename_nested_booleans;
  ...
  let find_fool_subterm ?(free_vars = Var.Set.empty) p =
    let rec aux ~top p =
      ...
      | AppBuiltin (hd, args)
        ...
        return p
      | Bind ((Binder.Exists | Binder.Forall), var, body) when (not top) && no_leaky_variables p -> return p
      ...
      | AppBuiltin (b, args) ->
        CCOpt.map
          ...
          (aux_l args)
      | App (hd, args) ->
        CCOpt.map (fun (args_t, args_f, s) -> T.app ~ty:p_ty hd args_t, T.app ~ty:p_ty hd args_f, s) (aux_l args)
      | _ -> None
    and aux_l = function
      | [] -> None
      | x :: xs ->
