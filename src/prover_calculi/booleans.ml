...
module Make (E : Env.S) : S with module Env = E = struct
  ...
  let get_triggers c =
      ...
      Literals.fold_terms ~ord ~subterms:false ~eligible:C.Eligible.always ~which:`All (C.lits c)
        ~fun_bodies:false
  ...
  let get_green_eligible c =
      let ord = C.Ctx.ord () in
          Ls.fold_terms ~vars:false ~var_args:false ~fun_bodies:false ~ty_args:false ~ord ~which:`Max
  ...
  let bool_hoist_simpl (c : C.t) : C.t list option =
      let proof =
          Proof.Step.simp [ C.proof_parent c ] ~rule:(Proof.Rule.mk "bool_hoist") ~tags:[ Proof.Tag.T_ho ]
      in
      ...
      let bool_subterms = get_bool_hoist_eligible c in
          ...
          |> CCFun.tap (function
                 ...
                 | None -> Util.debugf ~section 2 "bool_hoist_simpl(@[%a@])= None@." (fun k -> k C.pp c))
      ...
      let mk_res sub at partner =
          ...
          let lits = Literals.apply_subst renaming sub (C.lits c, sc_cl) in
              ...
              let step = Proof.Step.inference ~rule [ C.proof_parent_subst renaming (c, sc_cl) sub ] in
                  C.create
                    ~penalty:
                      (C.penalty c + C.proof_depth c + if Proof.Step.has_ho_step (C.proof_step c) then 2 else 0)
                 ...
                 let lit_pos = PB.arg idx PB.empty in
                     match lit with
                         | Literal.Equation (u, v, true)
                           when (Type.is_prop (T.ty u) || Type.is_var (T.ty u)) && T.is_app_var u ->
                             let app_vars =
                                     ...
                                     List.fold_left
                                       (fun acc p ->
                                         ...
                                         if Env.should_force_stream_eval () then
                                           ...
                                           let stm_res =
                                               Env.Stm.make ~penalty:(C.penalty c + 2) ~parents:[ c ] seq
                                           in
                                               Env.StmQ.add (Env.get_stm_queue ()) stm_res;
                                               acc)
                                       acc partners)
          ...
          |> Iter.fold
               (fun acc (lit, idx) ->
                 let lit_pos = PB.arg idx PB.empty in
                     match lit with
                         | Literal.Equation (u, v, true)
                           when (Type.is_var (T.ty u) || Type.is_prop (T.ty u)) && T.is_app_var u ->
                             let app_vars =
                                 if Literal.is_predicate_lit lit then [ (u, PB.to_pos (PB.left lit_pos)) ]
                                 else if Term.is_app_var v then
                                   [ (u, PB.to_pos (PB.left lit_pos)); (v, PB.to_pos (PB.right lit_pos)) ]
  ...
  let replace_bool_vars (c : C.t) =
      let p =
          Proof.Step.simp
            [ C.proof_parent c ]
            ~rule:(Proof.Rule.mk "replace_bool_vars") ~tags:[ Proof.Tag.T_ho ]
      in
      ...
      let quant_rw ~at b body =
          ...
          if quant_rw_unapplicable then None
            ...
            let proof =
                Proof.Step.simp
                  [ C.proof_parent c ]
                  ~rule:(Proof.Rule.mk "quantifier_rw") ~tags:[ Proof.Tag.T_ho ]
            in
           ...
           (fun acc (t, p) ->
             match T.view t with
                 | T.AppBuiltin ((Builtin.(ForallConst | ExistsConst) as b), [ _; body ]) ->
                     ...
                     let rest =
                         if CCArray.exists Literal.is_trivial (C.lits hoisted) then acc else hoisted :: acc
                     in
                           ...
                           |> OSeq.map (fun unif_subst_opt ->
                                  CCOpt.map
                                    (fun unif_subst ->
                                      ...
                                      let repl =
                                          if hd = Builtin.Eq || hd = Builtin.Equiv then T.true_ else T.false_
                                      in
                                      let new_lits = Array.copy (C.lits c) in
                                          ...
                                          let rule =
                                              Proof.Rule.mk
                                                ((if T.equal repl T.true_ then "eq" else "neq") ^ "_rw")
                                          in
                                          let proof =
      ...
      let rename_lit lit =
          let sign = L.is_positivoid lit in
              if L.is_predicate_lit lit then
                match lit with
                    | L.Equation (lhs, _, _) -> CCOpt.get_exn (FR.rename_form ~polarity_aware:false ~c lhs sign)
                    | _ -> assert false
              else
  ...
  let fix_unsupported_quant t =
      ...
      let rec aux t =
          match T.view t with
              ...
              | AppBuiltin ((ExistsConst | ForallConst), []) -> invalid_arg "type argument must be present"
              | AppBuiltin (hd, args) ->
                  let args' = List.map aux args in
  ...
  let replace_unsupported_quants c =
      let new_lits = Literals.map fix_unsupported_quant (C.lits c) in
          if Literals.equal (C.lits c) new_lits then None
            ...
            let proof =
                Proof.Step.simp [ C.proof_parent c ] ~rule:(Proof.Rule.mk "replace unsupported quants")
            in
      ...
      try
        let new_lits = Literals.map simplify_bools (C.lits c) in
            if Literals.equal (C.lits c) new_lits then SimplM.return_same c
              ...
              let proof =
                  Proof.Step.simp [ C.proof_parent c ] ~rule:(Proof.Rule.mk "simplify boolean subterms")
              in
              let new_ = C.create ~trail:(C.trail c) ~penalty:(C.penalty c) (Array.to_list new_lits) proof in
  ...
  let nnf_bools t =
      ...
      let rec aux t =
          match T.view t with
              ...
              | AppBuiltin (Builtin.Not, [ f ]) -> (
                  match T.view f with
                      ...
                      | AppBuiltin (((ForallConst | ExistsConst) as b), ([ g ] | [ _; g ])) ->
                          let flipped =
                              if b = Builtin.ForallConst then Builtin.ExistsConst else Builtin.ForallConst
                          in
              ...
              | AppBuiltin (Xor, [ f; g ]) -> aux (F.and_ (F.or_ f g) (F.or_ (F.not_ f) (F.not_ g)))
              | AppBuiltin (b, l) ->
                  let l' = List.map aux l in
  ...
  let cnf_otf c : C.t list option =
      ...
      let opts =
          match max_formula_weight with
              ...
              | Some m -> if m < renaming_weight then [ Cnf.DisableRenaming ] else []
      in
      ...
      match idx with
          | Some _ ->
              ...
              let proof =
                  Proof.Step.simp ~rule:(Proof.Rule.mk "cnf_otf") ~tags:[ Proof.Tag.T_ho ] [ C.proof_parent c ]
              in
                  ...
                  then k t
                    ...
                    match T.view t with
                        | App (f, l) ->
                            ...
                            List.iter aux l
                        | AppBuiltin (b, l) when not (Builtin.is_quantifier b) -> List.iter aux l
                        | _ -> ()
          in
              aux t
      in
    ...
    let transformer = if top then id else f in
        transformer
          (match view t with
              ...
              | Let (binds, expr) -> let_ (map (CCPair.map_snd (re false)) binds) (re false expr)
              | Bind (b, x, t) ->
            ...
            match T.view p with
                ...
                | Bind (Binder.Lambda, var, body) ->
                    CCOpt.map
                      ...
                      (aux ~top:false body)
                | App (hd, args) when (not top) && T.Ty.is_prop p_ty && no_leaky_variables p -> return p
                            ...
                            match find_fool_subterm a with
                                ...
                                | Some (a_t, a_f, subterm) ->
                                    ...
                                    let if_false = T.Form.or_ [ subterm'; aux ~vars @@ cons a_f b ] in
                                        T.Form.and_ [ if_true; if_false ])
                    | AppBuiltin (hd, args) -> T.app_builtin ~ty:p_ty hd (List.map (aux ~vars) args)
                    | App (hd, args) -> (
    ...
    Options.add_opts
      [
        ...
        ( "--bool-hoist-simpl",
          ...
          " use BoolHoistSimpl instead of BoolHoist; NOTE: Setting this option triggers nested booleans \
           renaming" );
