...
module Make (E : Env.S) : S with module Env = E = struct
  ...
  let rec check_all_distinct_ acc l =
      match l with
          | [] -> true
          | v :: l' -> (not (CCList.mem ~eq:(HVar.equal Type.equal) v acc)) && check_all_distinct_ (v :: acc) l'
  ...
  let declare_ ~proof ~ty_id:id ~ty_vars ~var cases =
      ...
      try
        let decl = find_decl_ id in
            Util.debugf ~section 3 "@[an enum is already declared for type %a@]" (fun k ->
                k pp_id_or_builtin id);
      ...
      let rec _check_all_vars ~ty ~var acc lits =
          match lits with
              | [] ->
                  ...
                  if check_uniq_var_is_ ~var acc && (!_accept_unary_types || List.length acc >= 2) then
                    Some (var, acc)
                  else None
  ...
  let instantiate_vars_ c =
      ...
      let vars =
          if !_instantiate_shielded then vars_under_eq_ (C.lits c) |> Iter.to_rev_list
                            ...
                            (fun case ->
                              ...
                              let c' = C.create_a ~trail ~penalty (CCArray.append c_guard lits') proof in
                                  Util.debugf ~section 3
                                    "@[<2>deduce @[%a@]@ from @[%a@]@ @[(enum_type switch on %a)@]@]" (fun k ->
                                      k C.pp c' C.pp c Type.pp decl.decl_ty);
                                  c')
                            decl.decl_cases
                      in
                          Some l)
            ...
            *)
            ...
            let t = T.app (T.const ~ty:ty_s s) vars in
                ...
                let lits =
                    List.map
                      (fun case ->
                        Lit.mk_eq (S.FO.apply renaming subst (t, 1)) (S.FO.apply renaming subst (case, 0)))
                      decl.decl_cases
                in
                let proof =
                    let parent = match decl.decl_proof with `Data (src, _) -> src | `Clause src -> src in
                        Proof.Step.inference ~rule:(Proof.Rule.mk "axiom_enum_types")
                ...
                let c' = C.create ~trail ~penalty (c_guard @ lits) proof in
                    Util.debugf ~section 3
                      "@[<2>instantiate axiom of enum type `%a` on @[%a@]:@ clause @[%a@]@]" (fun k ->
