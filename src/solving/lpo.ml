...
(** Functor to use Sat, and encode/decode the solution. Use "Solving Partial Order Constraints for LPO
    Termination", Codish & al *)
module MakeSolver (X : sig end) = struct
  ...
  let rec encode_lt ~n a b =
    if n = 0 then
      ...
      F.make_or
        [ F.make_and [ F.make_not d_a; d_b ]; F.make_and [ F.make_equiv d_a d_b; encode_lt ~n:(n - 1) a b ] ]
    )
...
module FO = struct
  ...
  let rec orient_lpo a b =
    ...
    | _, _ when T.subterm ~sub:b a -> C.true_ (* trivial subterm property --> ok! *)
...
module TypedSTerm = struct
  ...
  let rec orient_lpo a b =
    ...
    | _ when T.is_subterm ~strict:true b ~of_:a -> C.true_ (* trivial subterm property --> ok! *)
    | T.App (f, l), T.App (g, l') ->
