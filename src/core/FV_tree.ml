...
module Feature : sig
  ...
  let pp out (f : t) : unit =
      match f with
          ...
          | M m -> Fmt.fprintf out "(@[mset@ %a@])" Fmt.(iter (pair ~sep:silent ID.pp int)) (ID.Map.to_iter m)
          | L l -> Fmt.fprintf out "(@[labels@ %a@])" Fmt.(iter int) (Util.Int_set.to_iter l)
  ...
  let combine_ ~default ~op f1 f2 : feature =
      match (f1, f2) with
          ...
          | M m1, M m2 ->
              ID.Map.merge
                (fun _ o1 o2 -> Some (op (CCOpt.get_or ~default o1) (CCOpt.get_or ~default o2)))
                m1 m2
              |> mk_m
  ...
  let leq (f1 : t) (f2 : t) : bool =
      match (f1, f2) with
          ...
          | M m1, M m2 -> ID.Map.for_all (fun k i1 -> i1 <= ID.Map.get_or ~default:~-1 k m2) m1
            ...
            match (trie, IArray.get fv i) with
                | TrieNode m, c -> (
                    ...
                    with Not_found ->
                      (* no subtrie found *)
                      let subtrie =
                          if i + 1 = IArray.length fv then TrieLeaf C_set.empty else TrieNode Feat_map.empty
                      and rebuild' subtrie =
                          match subtrie with
                              | _ when empty_trie subtrie -> rebuild (TrieNode (Feat_map.remove c m))
                              | _ -> rebuild (TrieNode (Feat_map.add c subtrie m))
