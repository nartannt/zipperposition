...
let prec_compare prec a b =
    match (a, b) with
        | Head.I a, Head.I b -> ( match Prec.compare prec a b with 0 -> C.Eq | n when n > 0 -> Gt | _ -> Lt)
        | Head.B a, Head.B b -> ( match Builtin.compare a b with 0 -> Eq | n when n > 0 -> Gt | _ -> Lt)
        | Head.DB a, Head.DB b -> ( match CCInt.compare a b with 0 -> Eq | n when n > 0 -> Gt | _ -> Lt)
          ...
          let t = ty1comb_to_var t balance.comb2var in
              if T.is_var t || (P.lambda_mode && is_fluid t) then balance_weight_var wb t s ~pos ~below_lam
                ...
                match (Head.term_to_head t, Head.term_to_args t) with
                    ...
                    | h, args ->
                        let wb' =
                            if pos then W.(wb + weight prec h ~below_lam) else W.(wb - weight prec h ~below_lam)
                        in
          ...
          let ts_below_lam = below_lam || g = Head.LAM in
              if prec_compare prec f g = C.Eq then
                match prec_status prec f with
                    ...
                    | Prec.LengthLexicographic -> tckbolenlex wb ss ts ~below_lam:ss_below_lam
              else
...
module MakeRPO (P : PARAMETERS) : ORD = struct
  ...
  and alpha ~prec ss t =
      match ss with
          | [] -> Incomparable
          | s :: ss' -> ( match rpo6 ~prec s t with Eq | Gt -> Gt | _ -> alpha ~prec ss' t)
  ...
  let multiply_unknowns poly unks =
      if not (CCList.is_empty unks) then (
        let old_hashtbl = poly.hashtbl in
            ...
            ULH.iter
              (fun key coeff -> ULH.add poly.hashtbl (mk_key (List.rev_append unks key)) coeff)
              old_hashtbl)
...
module LambdaKBO : ORD = struct
  ...
  let rec normalize_consts ~prec t =
      match T.view t with
          ...
          | App (s, ts) -> Term.app (normalize_consts ~prec s) (List.map (normalize_consts ~prec) ts)
          | _ -> t
          ...
          match (T.view t_hd, T.view s_hd) with
              ...
              | AppBuiltin (t_b, t_bargs), AppBuiltin (s_b, s_bargs) -> (
                  match Builtin.compare t_b s_b with
                      | 0 -> process_args ~prec (t_bargs @ t_tyargs @ t_args) (s_bargs @ s_tyargs @ s_args)
                      | n when n > 0 -> consider_weights_poly ~prec t s Gt
                      | _ -> consider_weights_poly ~prec t s Lt)
              | _, _ -> assert false
      ...
      match ss with
          ...
          | si :: ss' -> (
              match do_compare_terms ~prec t si with
                  ...
                  | C.Geq | C.Incomparable -> if check_subs ~prec ss' t then C.Lt else C.Incomparable)
  ...
  and do_compare_terms ~prec t s =
      ...
      let s_hd, (s_tyargs, s_args) = break_term_up s in
          match (T.view t_hd, T.view s_hd) with
              ...
              | Var _, Fun (_, s_body) -> if check_subs ~prec [ s_body ] t then C.Lt else C.Incomparable
              | Var _, (DB _ | Const _) -> if check_subs ~prec s_args t then C.Lt else C.Incomparable
              | Var _, AppBuiltin (s_b, s_bargs) ->
                  if check_subs ~prec (List.rev_append s_bargs s_args) t then C.Lt else C.Incomparable
              | (Const _ | DB _), Var _ -> if check_subs ~prec t_args s then C.Gt else C.Incomparable
              | Const gid, Const fid -> (
                  match Prec.compare prec gid fid with
                      | 0 -> (
                          match ID.compare gid fid with
                      ...
                      | n when n > 0 -> compare_rest ~prec t s_args
                      | _ -> C.opp (compare_rest ~prec s t_args))
              | Const _, AppBuiltin (_, s_bargs) -> compare_rest ~prec t (List.rev_append s_bargs s_args)
              | (Const _ | AppBuiltin _), DB _ -> compare_rest ~prec t s_args
              | (Const _ | AppBuiltin _ | DB _), Fun (_, s_body) -> compare_rest ~prec t [ s_body ]
              ...
              | Fun (_, t_body), Var _ -> if check_subs ~prec [ t_body ] s then C.Gt else C.Incomparable
              | Fun (_, t_body), (Const _ | AppBuiltin _ | DB _) -> C.opp (compare_rest ~prec s [ t_body ])
              | Fun (t_ty, t_body), Fun (s_ty, s_body) -> (
                  match compare_types ~prec t_ty s_ty with
...
let derived_ho_kbo ~ignore_quans_under_lam prec =
    ...
    let compare prec a b =
        CCCache.with_cache cache_compare (fun (a, b) -> KBO.compare_terms ~prec a b) (a, b)
    in
    let cache_might_flip = mk_cache 256 in
    let might_flip prec a b =
        CCCache.with_cache cache_might_flip (fun (a, b) -> KBO.might_flip prec a b) (a, b)
    in
...
let lambdafree_rpo prec =
    ...
    let cache_compare = mk_cache 256 in
    let compare prec a b =
        CCCache.with_cache cache_compare (fun (a, b) -> RPO.compare_terms ~prec a b) (a, b)
    in
    let cache_might_flip = mk_cache 256 in
    let might_flip prec a b =
        CCCache.with_cache cache_might_flip (fun (a, b) -> RPO.might_flip prec a b) (a, b)
    in
    let monotonic = false in
...
let epo prec =
    ...
    let compare prec a b =
        CCCache.with_cache cache_compare (fun (a, b) -> EPO.compare_terms ~prec a b) (a, b)
    in
