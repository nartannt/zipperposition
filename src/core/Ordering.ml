...
(** Hopefully more efficient (polynomial) implementation of LPO, following the paper "Things to Know when
    Implementing LPO" by Löchner. We adapt here the implementation clpo6 with some multiset symbols (=). *)
...
module EPO : ORD = struct
  ...
  and epo_behind_cache ~prec (t, tt) (s, ss) =
    if T.equal t s && CCList.length tt = CCList.length ss && CCList.for_all2 T.equal tt ss then
      C.Eq
    else (
      ...
      | _ ->
        ...
        | g, f -> epo_composite ~prec (t, tt) (s, ss) (g, Head.term_to_args t @ tt) (f, Head.term_to_args s @ ss))
    )
...
module LambdaKBO : ORD = struct
  ...
  let rec add_weight_of ~prec w sign t =
    let add_eta_extra_of w ty =
      ...
      | Var var -> add_monomial w sign (W.add (Prec.lam_weight prec) my_db_weight) [ Polynomial.EtaUnknown var ]
      | _ -> ()
    in
