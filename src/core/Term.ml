...
let max_cover t ts =
    let rec aux depth t =
        let hit =
            CCList.find_mapi (fun i x -> match x with Some t' when equal t t' -> Some i | _ -> None) ts
        in
            match hit with
          ...
          match view t with
                  ...
                  let l = CCList.filter (fun t -> not (Type.is_tType (ty t))) l in
                      if CCList.is_empty l then Format.fprintf out "@[%a@]" Builtin.TPTP.pp b
                        ...
                        Format.fprintf out "(@[(%a) @@ %a@])" Builtin.TPTP.pp b
                          (Util.pp_list ~sep:" @ " pp_enclosed) l
              ...
              | Fun _ ->
                  ...
                  let old_d = !depth in
                      ...
                      Format.fprintf out "(@[<hv2>^[@[%a@]]:@ (@[%a@])@])" (Util.pp_list ~sep:"," pp_db) vars
                        pp_rec bod;
                      depth := old_d
              ...
              match view t with
                  ...
                  | DB i -> (
                      match DBEnv.find env i with
                          ...
                          | None ->
                              Util.errorf ~where:"Term" "cannot find `Y%d`@ @[:in [%a]@]" i (DBEnv.pp Var.pp)
                                env)
                  ...
                  | AppBuiltin (b, [ _; body ])
                    when Builtin.equal b Builtin.ForallConst || Builtin.equal b Builtin.ExistsConst ->
                      ...
                      if is_true_or_false fun_body then
                        ...
                        let err_msg =
                            CCFormat.sprintf "quantifier wrongly encoded: %a(%a)" T.pp t T.pp orig_term
                        in
