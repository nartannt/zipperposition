...
(** In this section, term smart constructors are defined. They perform hashconsing, and precompute some
    properties (flags). *)
...
module Seq = struct
  ...
  let all_typed_symbols t =
    ...
    let rec aux t =
      ...
      let res =
        match view t with
        | AppBuiltin (_, l) -> List.fold_left (fun acc_res t -> Iter.append acc_res (aux t)) Iter.empty l
...
let rec in_pfho_fragment t =
  ...
  | Const sym ->
    if top_level_exception t || type_ok (ty t) then
      ...
      raise (Failure (CCFormat.sprintf "Constant has out-of-fragment type [%a] " ID.pp sym))
  ...
  | Fun (var_t, body) ->
    if type_ok var_t && type_ok (ty body) && in_pfho_fragment body then
      ...
      raise (Failure (CCFormat.sprintf "Lambda body has out-of-fragment type [%a]" T.pp t))
  | DB _ ->
    ...
    | DB i ->
      ...
      assert (
        if Type.equal ty (List.nth env i) then
          ...
          Format.printf "@[%a@ has type %a@ but bound with type %a@]@." pp t Type.pp ty Type.pp (List.nth env i);
          false
        ));
      bvar ~ty i
...
let rec convert_type ty_list ty =
  ...
  | None ->
    ...
    if List.length args != 0 then (
      ...
      | Fun _ ->
        Printf.printf "qsfqsldfj\n";
        assert false
      | Var _ ->
        Printf.printf "vaaaariaaaaable";
        assert false
      | _ ->
    ...
    | App (f, l) ->
      ...
      begin
        ...
        | Const f_id ->
          ...
          let ty_list, new_type_args =
            List.fold_left_map convert_type ty_list (List.map Type.of_term_unsafe type_args)
          in
          ...
          let str_term_list, new_f_term =
            if List.length type_args = 0 then
              ...
              match
                List.find_opt
                  (fun ((old_f_id, old_f_ty_args), _) ->
                    ID.equal old_f_id f_id && List.for_all2 Type.equal old_f_ty_args new_type_args)
                  str_term_list
              with
