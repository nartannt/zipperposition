...
let pp_attr =
    let rec pp_attr_gen ~inner out = function
        ...
        | A_app (s, l) when not inner -> Format.fprintf out "@[%s@ %a@]" s (Util.pp_list ~sep:" " pp_attr) l
        | A_app (s, l) -> Format.fprintf out "(@[%s@ %a@])" s (Util.pp_list ~sep:" " pp_attr) l
        | A_quoted s -> Format.fprintf out "\"%s\"" s
        | A_list l -> Format.fprintf out "[@[<hv>%a@]]" (Util.pp_list ~sep:"," pp_attr) l
...
let rec pp_attr_tstp out = function
    ...
    | A_app (s, l) -> Format.fprintf out "%s(@[%a@])" s (Util.pp_list ~sep:"," pp_attr_tstp) l
    | A_quoted s -> Format.fprintf out "'%s'" s
    | A_list l -> Format.fprintf out "[@[<hv>%a@]]" (Util.pp_list ~sep:"," pp_attr_tstp) l
...
let pp_statement out st =
    ...
    let fpf = Format.fprintf in
        match st.stmt with
            ...
            | Decl (id, ty) -> fpf out "@[<2>val%a %s :@ @[%a@]@]." pp_attrs attrs id T.pp ty
            | Def l ->
                let pp_def out { def_id = id; def_ty; def_rules } =
                    fpf out "@[<2>@[%s :@ %a@]@ := @[%a@]" id T.pp def_ty (Util.pp_list ~sep:" and " T.pp)
                      def_rules
                in
...
let () =
    Printexc.register_printer (function
        | Parse_error (loc, msg) -> Some (CCFormat.sprintf "@[<4>parse error:@ @[%s@]@ at %a@]" msg Loc.pp loc)
        | _ -> None)
