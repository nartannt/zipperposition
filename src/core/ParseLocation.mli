...
(** A location is a range of characters in file, used for messages (error messages, warning, etc.).
    ...
    The range is delimited by a start position (line,column) and an end position (line, column) + an optional
    file name. *)
...
val combine : t -> t -> t
(** Position that spans the two given positions. The file is assumed to be the same in both case, and is chosen
    from one of the two positions. *)
