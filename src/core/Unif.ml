...
let norm_logical_disagreements ?(mode = `Conservative) b args args' : _ list * _ list =
    ...
    if Builtin.is_flattened_logical b then (
      ...
      match mode with
          ...
          | `Off -> if List.length args = List.length args' then (args, args') else raise Fail)
    else (args, args')
...
module Inner = struct
  ...
  let restrict_fun1 subst ~op ~ty ~to_:subset ~scope (v, args) =
      ...
      match op with
          | O_match_protect _ -> if n = n_old then subst else fail ()
          | _ ->
              (* fresh variable *)
  ...
  and unif_vars ~op subst t1 sc1 t2 sc2 : unif_subst =
      ...
      in
          match (T.view t1, T.view t2, op) with
              | T.Var v1, T.Var v2, O_equal -> if HVar.equal T.equal v1 v2 && sc1 = sc2 then subst else fail ()
              ...
              | _, _, _
                when (not !_unif_bool) && match T.open_poly_fun t1 with _, _, ret -> T.type_is_prop ret ->
                  fail ()
              | _, _, _
                when (not !_unif_bool) && match T.open_poly_fun t2 with _, _, ret -> T.type_is_prop ret ->
                  fail ()
  ...
  *)
  and flex_flex_unif ~bvars subst ~ty_ret v1 l1 v2 l2 ~scope : unif_subst =
      Util.debugf ~section 5 "(@[flex_flex@ `@[%a %a@]`[%d]@ `@[%a %a@]`[%d]@ :subst %a@ :bvars %a@])" (fun k ->
          k HVar.pp v1 (Util.pp_list T.pp) l1 scope HVar.pp v2 (Util.pp_list T.pp) l2 scope US.pp subst
            B_vars.pp bvars);
  ...
  *)
  ...
  let matching ?(subst = Subst.empty) ~pattern b =
      ...
      let subst = US.of_subst subst in
      let subst =
          unif_rec subst pattern b ~root:true ~op:(O_match_protect (P_scope scope)) ~bvars:B_vars.empty
      in
  ...
  let matching_same_scope ?(protect = Iter.empty) ?(subst = S.empty) ~scope ~pattern b =
      ...
      let subst = US.of_subst subst in
      let subst =
          unif_rec subst (Scoped.make pattern scope) (Scoped.make b scope)
