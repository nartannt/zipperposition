  ...
  let rec check ~depth (t, sc_t) =
    ...
    | T.HasType ty ->
      ...
      ||
        ...
        | T.Const _ -> false
        | T.Bind (_, varty, t') -> check ~depth (varty, sc_t) || check ~depth:(depth + 1) (t', sc_t)
...
(** During matching or variant checking, we need to {b protect} some variables from being bound. Typically, when
    matching [u] against pattern [t], we can bind variables from [t] but not from [u] (since [u] must be
    preserved).

    Two styles of protection exist:

    - by explicit set of variables (when both [t] and [u] live in the same scope)
    - by scope: all variables in this scope are protected, EXCEPT fresh variables. Fresh variables are not
      protected because pattern unification/matching will move both terms into the same (protected) scope, but we
      still need to bind (freshly renamed) variables from the pattern. *)
type protected =
...
module Inner = struct
  ...
  let bind ?(check = true) subst v t =
    if (check && occurs_check ~depth:0 subst v t) || not (T.DB.closed (fst t)) then
      fail ()
   ...
   fun subst ~ty_ret ~bvars ~scope (v1, l1) (v2, l2) ->
    ...
    let inter =
      List.filter
        (fun t ->
          match T.view t with
          | T.DB i -> DBEnv.mem bvars.B_vars.left i && List.exists (T.is_bvar_i i) l2
          | _ -> assert false)
        l1
    in
  ...
  *)
  ...
  and unif_term ~op ~root ~bvars subst t1 sc1 t2 sc2 : unif_subst =
    ...
    let is_whnf t sc_t =
      match T.view t with
      | T.App (f, _) -> (not (T.is_lambda f)) && not (T.is_var f && US.mem subst (T.as_var_exn f, sc_t))
      | _ -> true
    in
    ...
    | _ when sc1 = sc2 && T.equal t1 t2 -> subst (* the terms are equal under any substitution *)
    ...
    | T.AppBuiltin (s1, l1), T.AppBuiltin (s2, l2) when Builtin.equal s1 s2 ->
      let l1, l2 =
        if sc1 = sc2 && not (Type.is_fun (Type.of_term_unsafe @@ T.ty_exn t1)) then
  ...
  and unif_vars ~op subst t1 sc1 t2 sc2 : unif_subst =
    ...
    | T.Var v1, T.Var _, O_variant (P_vars s) when not (T.VarSet.mem v1 s) -> US.bind subst (v1, sc1) (t2, sc2)
  ...
  and unif_ho ~op ~root ~bvars subst t1_0 t2_0 ~scope : unif_subst =
    ...
    | T.Var v1, T.Const _ ->
      (match op with
      | O_match_protect (P_scope sc2') when sc2' = scope && not (HVar.is_fresh v1) -> fail ()
  ...
  *)
  and flex_flex_matching ~op ~bvars subst ~ty_ret v1 l1 v2 l2 ~scope : unif_subst =
    ...
    (match op with
    | O_match_protect (P_scope sc') when sc' = scope && not (HVar.is_fresh v1) -> fail ()
    | O_match_protect (P_vars s) when T.VarSet.mem v1 s -> fail ()
    | _ -> ());
...
module FO = struct
  ...
  let unify_full ?(subst = US.empty) sc1 sc2 =
    ...
    if (not (Term.DB.is_closed ta)) || not (Term.DB.is_closed tb) then (
      ...
      (unify_full :> ?subst:unif_subst -> term Scoped.t -> term Scoped.t -> unif_subst) ~subst sc1 sc2
  ...
  let unify_syn ?(subst = Subst.empty) sc1 sc2 =
    ...
    if (not (Term.DB.is_closed ta)) || not (Term.DB.is_closed tb) then (
      ...
      (unify_syn :> ?subst:subst -> term Scoped.t -> term Scoped.t -> subst) ~subst sc1 sc2
  ...
  let anti_unify ?(cut = max_int) (t : term) (u : term) : (term * term) list option =
    ...
    let rec aux t u =
      ...
      | _ when not (Type.equal (T.ty t) (T.ty u)) -> raise Exit (* irreconciliable *)
      | _ when Type.equal (Type.returns (T.ty t)) Type.tType -> raise Exit (* distinct types *)
      | T.App (f, ts), T.App (g, us) when T.equal f g && List.length ts = List.length us -> List.iter2 aux ts us
      | _ ->
        incr len;
        if !len <= cut then
          pairs := (t, u) :: !pairs
