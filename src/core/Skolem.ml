...
let skolem_form ~ctx subst var form =
    ...
    let tyvars, vars = collect_vars subst form in
        Util.debugf ~section 5
          ...
          (fun k ->
            k T.pp form (Util.pp_list Var.pp_full) tyvars (Util.pp_list Var.pp_full) vars (Var.Subst.pp T.pp)
              subst);
      ...
      (fun def ->
        match def with
            | Def_form def when not def.rw_rules ->
                ...
                let df_vars, f_vars =
                    CCPair.map_same (fun x -> Var.Set.of_iter (T.Seq.vars x)) (def_form, form)
                in
                  ...
                  let res =
                      {
                        ...
                        polarity;
                        as_stmt =
                          lazy (stmt_of_form rw_rules polarity proxy def.proxy_id def.proxy_ty form proof);
                      }
                  in
...
let define_term ?(pattern = "fun_") ~ctx ~parents rules : term_definition =
    ...
    let ty_vars, ty_args =
        CCList.partition_map
          (fun t ->
            match T.view t with T.Var v when T.Ty.is_tType (Var.ty v) -> `Left v | _ -> `Right (T.ty_exn t))
          some_args
    in
        (* checks *)
        List.iter
            ...
            List.map
              (fun (args, rhs) ->
                let all_vars =
                    ...
                    |> Iter.flat_map T.Seq.free_vars |> Var.Set.of_iter |> Var.Set.to_list
                in
                    if is_prop then
