...
let typed_sym term =
    ...
    let get_typed_sym app_term =
        ...
        let unsafe_term_to_type (term : T.t) =
            Ty.of_term_unsafe (term :> InnerTerm.t)
        in
            match T.head f with
                | Some id when is_instantiated ty ->
                    Some (id, List.map unsafe_term_to_type (snd (T.as_app f)))
                | _ -> None
    in
...
let merge_map_arg_iter (old_ty_args_1, new_ty_args_1)
    ...
    ( Iter.union ~eq:ty_arg_eq old_ty_args_1 old_ty_args_2,
      Iter.union ~eq:ty_arg_eq new_ty_args_1 new_ty_args_2 )
...
let split_iter type_args_iter =
    ...
    let mono_type_args =
        Iter.filter (List.for_all Ty.is_ground) type_args_iter
    in
    let poly_type_args =
        Iter.filter
          (List.exists (fun ty -> not (Ty.is_ground ty)))
          type_args_iter
    in
...
let derive_type_arg_subst mono_map poly_map =
    ...
    let combine fun_sym (old_poly_args, new_poly_args) acc =
        ...
        let new_poly_subst_res =
            remove_duplicates ~eq:Subst.equal
              (Iter.append new_poly_subst derived_new_poly_subst)
        in
        let old_poly_subst_res =
            remove_duplicates ~eq:Subst.equal
              (Iter.append old_poly_subst derived_old_poly_subst)
        in
...
let select_subst subst_iter ty_vars_to_instantiate ty_var_subst_max =
    ...
    let can_monomorphise =
        let subst_ty_vars =
            Iter.flat_map
              (fun subst -> Iter.map Scoped.get (Subst.domain subst))
              subst_iter
        in
            (* TODO factorise this equality function all over the place*)
            Iter.subset
              ~eq:
                (HVar.equal (fun ty ty' ->
                     my_ty_eq (Ty.of_term_unsafe ty) (Ty.of_term_unsafe ty')))
    ...
    in
        if not can_monomorphise then Iter.empty
          ...
          let select_subst_for_var ty_var =
              let subst_iter_for_var =
                  Iter.filter (fun subst -> Subst.mem subst ty_var) subst_iter
              in
              (* using whatever order the substitutions are in to select them *)
              let selected_subst =
                  iter_truncate ty_var_subst_max subst_iter_for_var
              in
                  selected_subst
          in
          let select_var_fold acc ty_var =
              iter_subst_union acc (select_subst_for_var ty_var)
          in
              Iter.fold select_var_fold Iter.empty
                (Iter.map (fun var -> (var, 0)) sorted_ty_vars)
...
let apply_ty_arg_subst_split subst poly_ty_args max_new_mono max_new_poly =
    ...
    let ty_vars ty_args =
        remove_duplicates ~eq:ty_var_eq
          (Iter.persistent_lazy
             (List.fold_left
                (fun acc ty ->
                  Iter.append (Ty.Seq.vars ty :> InnerTerm.t HVar.t Iter.t) acc)
                Iter.empty ty_args))
    in
    ...
    let mono_candidates, potential_poly_candidates =
        iter_split
          (fun (_, ty_vars) -> Iter.subset ~eq:ty_var_eq ty_vars subst_domain)
    ...
    let poly_candidates =
        Iter.filter
          (fun (_, ty_vars) ->
            Iter.exists
              (fun ty_var -> Iter.mem ~eq:ty_var_eq ty_var subst_domain)
              ty_vars)
          potential_poly_candidates
    in
        split_apply_ty_arg_subst
...
let rec generate_ty_args all_subst poly_ty_args max_new_mono max_new_poly =
    let res =
        match Iter.head all_subst with
            ...
            | _ when max_new_mono <= 0 && max_new_poly <= 0 ->
            ...
            | Some subst ->
                ...
                let new_mono_ty_args, new_poly_ty_args =
                    apply_ty_arg_subst_split subst poly_ty_args max_new_mono
                      max_new_poly
                in
                let new_mono_count =
                ...
                let next_mono_args, next_poly_args, used_substs =
                    generate_ty_args (Iter.drop 1 all_subst) poly_ty_args
                      new_mono_count new_poly_count
                in
...
let restricted_subst_eq (ty_var_iter : Ty.t HVar.t Iter.t) subst_1 subst_2 =
    ...
    in
        Iter.for_all
          (fun ty_var ->
            eq_sc_term_opt
              (Subst.find subst_1 (ty_var, 0))
              (Subst.find subst_2 (ty_var, 0)))
          (ty_var_iter :> InnerTerm.t HVar.t Iter.t)

let apply_subst_map_not_stupid mono_map poly_map new_poly_subst_all
    old_poly_subst_all bounds =
    ...
    let apply_subst_fun_sym fun_sym (old_poly_ty_args, new_poly_ty_args)
        ...
        let max_mono_bound =
            max_nb
              (Iter.length (fst all_mono_ty_args)
              + Iter.length (snd all_mono_ty_args))
              bounds.mono_clause
        in
        let max_poly_bound =
            max_nb
              (Iter.length old_poly_ty_args + Iter.length new_poly_ty_args)
              bounds.poly_clause
        in
            ...
            let ty_var_iter =
                remove_duplicates ~eq:ty_var_eq
                  (Iter.flat_map
                     (fun ty_args ->
                       List.fold_left
                         (fun acc ty -> Iter.append acc (Ty.Seq.vars ty))
                         Iter.empty ty_args)
                     poly_ty_args)
            in
            (* removes substitutions which only differ on type variables that do not appear in poly_ty_args*)
            let subst_all_unique =
                remove_duplicates
                  ~eq:(restricted_subst_eq ty_var_iter)
                  subst_all
            in
        ...
        let new_mono_ty_args_1, new_poly_ty_args_1, used_subst_1 =
            generate_unique new_poly_subst_all new_poly_ty_args max_mono_bound
              max_poly_bound
        in
        ...
        let new_mono_ty_args_2, new_poly_ty_args_2, used_substs_2 =
            generate_unique old_poly_subst_all old_poly_ty_args new_mono_bound
              new_poly_bound
        in
        ...
        let all_new_mono_ty_args =
            Iter.union ~eq:ty_arg_eq new_mono_ty_args_1 new_mono_ty_args_2
        in
        let all_new_poly_ty_args =
            Iter.union ~eq:ty_arg_eq new_poly_ty_args_1 new_poly_ty_args_2
        in
        ...
        let fun_update new_ty_args = function
            | Some curr_ty_args ->
                Some (Iter.union ~eq:ty_arg_eq curr_ty_args new_ty_args)
            | None -> Some new_ty_args
        in
            ( ArgMap.update fun_sym
                (fun_update all_new_mono_ty_args)
                acc_mono_map,
              ArgMap.update fun_sym
                (fun_update all_new_poly_ty_args)
                acc_poly_map,
              Iter.append acc_used_substs all_used_substs )
    in

    ArgMap.fold apply_subst_fun_sym poly_map
      (ArgMap.empty, ArgMap.empty, Iter.empty)
...
let mono_step_clause mono_type_args_map poly_type_args_map susbt_clause_map
    curr_iteration literals bounds =
    ...
    if false then (
      ...
      ArgMap.iter
        (fun fun_sym iter -> print_all_type_args fun_sym iter ~erase_empty:true)
        mono_type_args_map;
    ...
    let new_poly_subst_all, old_poly_subst_all =
        derive_type_arg_subst mono_type_args_map poly_type_args_map
    in
    ...
    let new_mono_map_all, new_poly_map_all, used_substs_iter =
        apply_subst_map_not_stupid mono_type_args_map poly_type_args_map
          new_poly_subst_all old_poly_subst_all bounds
    in
...
let mono_step clause_list mono_map poly_clause_map subst_map curr_iter bounds =
    ...
    let process_clause acc (clause_id, literals) =
        ...
        let merge_iter _ iter_1 iter_2 =
            Some (remove_duplicates ~eq:ty_arg_eq (Iter.append iter_1 iter_2))
        in
    ...
    in
    let res_poly_clause_map =
        ClauseArgMap.mapi clause_map_age poly_clause_map
    in
        ...
        if false then (curr_mono_map, curr_poly_map)
          ...
          let new_mono_map =
              ArgMap.update ty_sym
                (map_update_fun ty_args ty_args_mono)
                curr_mono_map
          in
          let new_poly_map =
              ArgMap.update ty_sym
...
let poly_ty_args_filter mono_map poly_map ty_var_limit =
    ...
    let vars_of_ty_arg ty_list =
        remove_duplicates ~eq:ty_var_eq
          (List.fold_left
             (fun acc ty -> Iter.union ~eq:ty_var_eq acc (Ty.Seq.vars ty))
             Iter.empty ty_list)
    in
    ...
    let ty_arg_map_map (_, ty_args_iter) =
        ( Iter.empty,
          remove_duplicates ~eq:ty_arg_eq
            (Iter.persistent_lazy (Iter.filter ty_args_filter ty_args_iter)) )
    in
...
let map_initialisation_step (mono_map, clause_poly_map, pb_subst_map)
    ...
    let remove_duplicate_init map =
        ...
        ArgMap.map
          (fun (_, iter) ->
            (Iter.empty, remove_duplicates ~eq:ty_arg_eq (Iter.persistent iter)))
          map
    in
        ...
        match ClauseArgMap.find_opt clause_id clause_poly_map with
            ...
            | Some other_poly_map ->
                ...
                ClauseArgMap.add clause_id
                  (ArgMap.union
                     (fun _ pair_1 pair_2 ->
                       Some (merge_map_arg_iter pair_1 pair_2))
              ...
              | Some ty_var ->
                      ...
                      match Subst.merge subst_acc next_subst with
                          ...
                          | merged_subst ->
                              let new_vars_to_instantiate =
                                  remaining_ty_vars vars_to_instantiate
                                    merged_subst
                              in
                              let res_subst, res_count =
                                  create_subst
                                    (merged_subst, acc_iter, acc_count)
                  ...
                  let process_fold (acc_iter, acc_count) next_subst =
                      if acc_count <= 0 then (acc_iter, acc_count)
                        ...
                        let res_subst, res_count =
                            process_subst acc_count next_subst
                        in
                            (iter_subst_union acc_iter res_subst, res_count)
                  in
                      Iter.fold process_fold (Iter.empty, acc_count)
                        candidate_subst
    in
    let res =
        Iter.persistent_lazy
          (create_subst (Subst.empty, Iter.empty, max_new_subst) ty_var_iter
          |> fst)
    in
...
let monomorphise_problem clause_list =
    ...
    let nb_of_lits =
        List.fold_left
          (fun acc (_, lit_arr) -> acc + Array.length lit_arr)
          0 clause_list
    in
        ...
        let init_clause_poly_map =
            ClauseArgMap.map
              (fun poly_map ->
                poly_ty_args_filter init_mono_map poly_map
                  all_bounds.ty_var_limit)
        ...
        if false then (
          ...
          ClauseArgMap.iter
            (fun clause_id poly_map ->
              ...
              ArgMap.iter
                (fun fun_sym iter -> print_all_type_args fun_sym iter)
                poly_map)
            init_clause_poly_map);
        ...
        let init_mono_map =
            ArgMap.map
              (fun (old_iter, new_iter) ->
                (Iter.empty, remove_duplicates ~eq:ty_arg_eq new_iter))
              init_mono_map
        in
        ...
        if List.length poly_clause_list = 0 then
          Printf.printf
            "Warning: All polymorphic functions in this problem's clauses are \
             instantiated,monomorphisation will only consist in syntactically \
             mangling the arguments that are types\n";
        Printf.printf "We begin with %i polymorphic clauses\n"
          (List.length poly_clause_list);

        (* monomorphisation loop *)
        let rec monomorphisation_loop curr_count mono_map poly_clause_map
            subst_map =
            Printf.printf "\nbegin loop %f\n" (Sys.time () -. !begin_time);

            let count_res = count_arg_map_split mono_map in
                Printf.printf
                  "we have %i old and %i new monomorphic type args\n"
                ...
                if curr_count <= 0 then (mono_map, poly_clause_map, subst_map)
                  ...
                  let new_subst, new_mono_map, new_poly_clause_map =
                      mono_step poly_clause_list mono_map poly_clause_map
                        subst_map curr_count all_bounds
                  in
                  let res =
                      monomorphisation_loop (curr_count - 1) new_mono_map
                        new_poly_clause_map new_subst
                  in
                      res
        in
        ...
        let _, _, subst_map_res =
            ...
            let init_mono_map =
                ArgMap.map
                  (fun (old_iter, new_iter) ->
                    (Iter.persistent old_iter, Iter.persistent new_iter))
                  init_mono_map
            in
                monomorphisation_loop loop_count init_mono_map
                  init_clause_poly_map init_subst_map
        in
            ...
            let all_vars =
                remove_duplicates ~eq:var_eq
                  (Iter.persistent_lazy
                     (Iter.flat_map
                        (fun lit ->
                          (Literal.Seq.vars lit :> InnerTerm.t HVar.t Iter.t))
                        (Iter.of_array lit_arr)))
            in
                Iter.filter
                  (fun var -> InnerTerm.equal (HVar.ty var) InnerTerm.tType)
                  all_vars
        in

        let instantiate_clause subst_map (clause_id, lit_arr)
            new_clauses_remaining =
            ...
            let subst_map_filter_age subst_map iteration_count =
                SubstMap.map
                  (fun subst_iter ->
                    Iter.filter
                      (fun (subst, age) -> age = iteration_count)
                      subst_iter)
                  subst_map
            in
            ...
            let rec all_mono_subst curr_iter total_subst_nb =
                if curr_iter < 0 then Iter.empty
                  ...
                  let prev_subst =
                      all_mono_subst (curr_iter - 1) total_subst_nb
                  in
                      if Iter.length prev_subst >= total_subst_nb then
                        ...
                        let curr_subst =
                            generate_monomorphising_subst
                              ...
                              vars_to_instantiate
                              all_bounds.monomorphising_subst
                        in
                        let curr_subst' =
                            iter_truncate
                              (total_subst_nb - Iter.length prev_subst)
                              curr_subst
                        in
