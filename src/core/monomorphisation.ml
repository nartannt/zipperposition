...
let instantiate_clause subst_map (clause_id, lit_arr) new_clauses_remaining =
  ...
  let mono_subst =
    ...
    | "age" -> age_mono_subst subst_map vars_to_instantiate new_clauses_remaining
    | "random" -> random_mono_subst subst_map vars_to_instantiate (new_clauses_remaining * !_loop_count)
    | "arbitrary" ->
      generate_monomorphising_subst subst_map vars_to_instantiate (new_clauses_remaining * !_loop_count)
    | "age-merge" -> age_merge_mono_subst subst_map vars_to_instantiate (new_clauses_remaining * !_loop_count)
    | _ ->
