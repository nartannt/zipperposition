...
let match_type ty ~mono_type =
    let res = Unif.Ty.matching_same_scope ~pattern:ty mono_type ~scope:0 in
    res
...
let derive_type_arg_subst mono_map poly_map = 
   (*derives the substitutions from two sets (iters) of type arguments*)
    ...
    let combine fun_sym (old_poly_args, new_poly_args) acc =
        ...
        in
        let new_poly_subst_res = remove_duplicates ~eq:Subst.equal (Iter.append new_poly_subst derived_new_poly_subst) in
        let old_poly_subst_res = remove_duplicates ~eq:Subst.equal (Iter.append old_poly_subst derived_old_poly_subst) in
...
let select_subst subst_iter ty_vars_to_instantiate ty_var_subst_max =
    ...
    let can_monomorphise =
        ...
        Iter.subset ~eq:(HVar.equal (fun ty ty' -> my_ty_eq (Ty.of_term_unsafe ty) (Ty.of_term_unsafe ty'))) subst_ty_vars ty_vars_to_instantiate
...
let rec generate_ty_args all_subst poly_ty_args max_new_mono max_new_poly =
    let res = match Iter.head all_subst with
...
let apply_subst_map_not_stupid mono_map poly_map new_poly_subst_all old_poly_subst_all bounds =
    ...
    let apply_subst_fun_sym fun_sym (old_poly_ty_args, new_poly_ty_args) (acc_mono_map, acc_poly_map, acc_used_substs) =
        ...
        let generate_unique subst_all poly_ty_args mono_bound poly_bound =
            if Iter.is_empty subst_all then Iter.empty, Iter.empty, Iter.empty
            else
                ...
                let res = remove_duplicates ~eq:ty_arg_eq mono_res, remove_duplicates ~eq:ty_arg_eq poly_res, used_substs in
                let bla, _, _ = res in
        ...
        let new_mono_ty_args_2, new_poly_ty_args_2, used_substs_2 = generate_unique old_poly_subst_all old_poly_ty_args new_mono_bound new_poly_bound in
        ...
        in
        ArgMap.update fun_sym (fun_update all_new_mono_ty_args) acc_mono_map, ArgMap.update fun_sym (fun_update all_new_poly_ty_args) acc_poly_map, Iter.append acc_used_substs all_used_substs
    in
...
let mono_step_clause mono_type_args_map poly_type_args_map susbt_clause_map curr_iteration literals bounds =
    ...
    let new_mono_map_all, new_poly_map_all, used_substs_iter = apply_subst_map_not_stupid mono_type_args_map poly_type_args_map new_poly_subst_all old_poly_subst_all bounds in
        ...
        let rec all_mono_subst curr_iter total_subst_nb =
            if curr_iter < 0 then
                ...
                if Iter.length prev_subst >= total_subst_nb then
                    ...
                    let curr_subst =
                        generate_monomorphising_subst (subst_map_filter_age subst_map curr_iter) vars_to_instantiate all_bounds.monomorphising_subst
                    in
