...
let () =
   Printexc.register_printer (function
      | AlreadyDeclared (id, old, new_) ->
         Some
           (CCFormat.sprintf
              "@[<2>symbol %a@ is already declared with type @[%a@],@ which is not compatible with @[%a@]@]"
...
let arity signature s =
   let ty = find_exn signature s in
      match Type.arity ty with
         | Type.NoArity -> failwith (CCFormat.sprintf "symbol %a has ill-formed type %a" ID.pp s Type.TPTP.pp ty)
         | Type.Arity (a, b) -> (a, b)
