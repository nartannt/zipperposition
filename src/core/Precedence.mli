...
module Constr : sig
  ...
  (** A partial order on symbols, used to make the precedence more precise. ['a] encodes the kind of ordering:
      partial or total {b NOTE}: the ordering must partition the set of ALL symbols into equivalence classes,
      within which all symbols are equal, but symbols of distinct equivalence classes are always ordered. *)
  ...
  (** symbols with high frequency are smaller. Elements of unknown frequency are assumed to have a frequency of
      0. *)
  ...
  (** [compose_sort l] sorts the list by increasing priority (the lower, the earlier an ordering is applied, and
      therefore the more impact it has) before composing *)
  ...
  (** Create a new partial order. {b CAUTION}, this order must respect some properties (see {!'a t}) *)
end
...
val create :
  ...
  ID.t list ->
  t
(** make a precedence from the given constraints. Constraints near the head of the list are {b more important}
    than constraints close to the tail. Only the very first constraint is assured to be totally satisfied if
    constraints do not agree with one another. *)
