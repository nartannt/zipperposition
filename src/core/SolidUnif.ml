           ...
           let res =
              if CCList.is_empty args_l && CCList.is_empty args_r then
                ...
                let all_covers = covered_l @ covered_r in
                   assert (
                     List.for_all (fun (l_arg, r_arg) -> Type.equal (Term.ty l_arg) (Term.ty r_arg)) all_covers);
              ...
              if not (Term.equal s' t') then
                ...
                let hd_t, args_t = T.as_app body_t' in
                   match (T.view hd_s, T.view hd_t) with
                      | T.Var _, T.Var _ ->
                         let solver =
                            if not (T.equal hd_s hd_t) then solve_flex_flex_diff else solve_flex_flex_same
                         in
