...
struct
  ...
  let cover_rigid_skeleton ?(covers_limit = None) t solids =
      ...
      let rec aux ?(recurse = true) ~depth s_args t : T.t list =
          ...
          let db_hits =
              CCList.filter_map (fun (s, s_db) -> if T.equal s t then Some s_db else None) sols_as_db
          in
          let rest =
              try
                match T.view t with
  ...
  let collect_flex_flex ~counter ~flex_args t =
      let replace_var ~bvar_tys ~target =
          ...
          then (target, [])
            ...
            let bvars =
                List.mapi (fun i ty -> (i, ty)) bvar_tys |> List.rev_map (fun (i, ty) -> T.bvar ~ty i)
            in
              ...
              let res =
                  if CCList.is_empty args_l && CCList.is_empty args_r then
                    ...
                    let all_covers = covered_l @ covered_r in
                        ...
                        let subs_l =
                            T.fun_l (List.map T.ty args_l) (T.app fresh_var (List.map fst all_covers))
                        in
                        let subs_r =
                            T.fun_l (List.map T.ty args_r) (T.app fresh_var (List.map snd all_covers))
                        in
          ...
          let res =
              if CCList.is_empty args_l && CCList.is_empty args_r then US.of_subst subst
                ...
                let same_args =
                    ...
                    |> List.mapi (fun i (a, b) ->
                           if T.equal a b then Some (T.bvar ~ty:(T.ty a) (n_l - i - 1)) else None)
  ...
  let project_flex_rigid ~subst ~scope flex rigid =
      ...
      let flex_var, flex_args =
          (T.as_var_exn @@ T.head_term flex, List.mapi (fun i a -> (i, a)) (T.args flex))
      in
                    ...
                    (fun (i, fa) ->
                      match T.head fa with
                          | Some id' when ID.equal id id' && Type.equal (T.ty fa) ret_ty ->
                              ...
                              let flex_arg =
                                  if CCList.is_empty trailing_tys then fa else T.fun_l trailing_tys fa
                              in
          ...
          | (s, t) :: rest ->
              ...
              let s', t' = (norm_deref subst (s, scope), norm_deref subst (t, scope)) in
                  if not (Term.equal s' t') then
                    ...
                    let body_s', body_t', _ =
                        eta_expand_otf ~subst:(US.subst subst) ~scope pref_s pref_t body_s body_t
                    in
                    ...
                    let hd_t, args_t = T.as_app body_t' in
                        match (T.view hd_s, T.view hd_t) with
                            ...
                            | T.Var _, _ ->
                                ...
                                in
                                let covered =
                                    cover_flex_rigid ~subst:(US.subst subst) ~counter ~scope body_s' body_t'
                                in
                            ...
                            | _, T.Var _ ->
                                ...
                                in
                                let covered =
                                    cover_flex_rigid ~subst:(US.subst subst) ~counter ~scope body_t' body_s'
                                in
                                    CCList.flat_map (fun subst -> unify ~scope ~counter ~subst rest) covered
                                    @ CCList.flat_map
                                        (fun (subst, s, t) ->
