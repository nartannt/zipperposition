            ...
            | T.Const id ->
              ...
              | Some (r, subst, sc_r, l_rest) ->
                ...
                Util.debugf ~section 5
                  "(@[<2>rewrite `@[%a@]`@ :using `@[%a@]`@ :with `@[%a@]`[%d]@ :rest [@[%a@]]@])" (fun k ->
                    k T.pp t' Rule.pp r Subst.pp subst sc_r (Util.pp_list ~sep:"," T.pp) l_rest);
...
module Lit = struct
  ...
  let narrow_lit ?(subst = Unif_subst.empty) ~scope_rules:sc_r (lit, sc_lit) =
    rules_of_lit lit
    |> Iter.flat_map (fun r ->
        Literal.unify ~subst (r.lit_lhs, sc_r) (lit, sc_lit) |> Iter.map (fun (subst, tags) -> r, subst, tags))
end
