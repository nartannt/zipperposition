...
let pp_lit_rule out r =
    let pp_c = CCFormat.hvbox (Util.pp_list ~sep:" ∨ " Literal.pp) in
        Format.fprintf out "@[<2>@[%a@] :=@ [@[<v>%a@]]@]" Literal.pp r.lit_lhs (Util.pp_list ~sep:"∧" pp_c)
          r.lit_rhs
...
module Term = struct
  ...
  let normalize_term_ max_steps (t0 : term) : term * Rule_inst_set.t =
                      ...
                      match rules_of_id id |> Iter.head with
                          | Some r when T.is_const r.term_lhs ->
                              ...
                              let cur_sc_r = sc_r in
                                  ...
                                  decr fuel;
                                  Util.debugf ~section 5 "@[<2>rewrite `@[%a@]`@ using `@[%a@]`@]" (fun k ->
                                      k T.pp t Rule.pp r);
                                  reduce r.term_rhs k
                          | Some _ ->
      ...
      and reduce_l (l : _ list) k =
          match l with
              | [] -> k []
              | t :: tail -> reduce_l tail (fun tail' -> reduce t (fun t' -> k (t' :: tail')))
      in
  ...
  module Rule = struct
        ...
        match lhs c with
            | Literal.Equation (lhs, _, _) as lit when Literal.is_predicate_lit lit -> (
                match T.view lhs with
                    | T.Const id -> Some id
                    | T.App (f, _) -> ( match T.view f with T.Const id -> Some id | _ -> assert false)
                    | _ -> assert false)
    ...
    let conv_ ~ctx lhs rhs =
        ...
        let lhs = conv_lit lhs in
            F.equiv lhs (rhs |> List.map (fun l -> List.map conv_lit l |> F.or_) |> F.and_)
          ...
          | Literal.Equation (t, u, sign) ->
              let ty = T.ty t in
                  if sign && (not !allow_pos_eqn_rewrite_) && T.is_var t && T.is_var u then
                    ...
                    Util.debugf ~section 2 "@[<2>ignore positive equational rewrite `%a`@]" (fun k ->
                        k Rule.pp r)
                  else if Type.is_const ty || Type.is_app ty then eq_rules_ := Set.add r !eq_rules_
                  else
                    Util.invalid_argf "Rewrite.Lit.add_eq_rule:@ invalid equation type `@[%a@]`@ for rule `%a`"
                      Type.pp ty Rule.pp r
          | _ -> Util.invalid_argf "Rewrite.Lit.add_eq_rule:@ non-equational rule `%a`" Rule.pp r
              ...
              match step_lit lit with
                  ...
                  | Some (rule, subst, tags) ->
                      let clauses = rule.lit_rhs in
                          Util.debugf ~section 5
                            "@[<2>lit rewrite `@[%a@]`@ :into `@[<v>%a@]`@ :with @[%a@]@ :rule `%a`@]" (fun k ->
                              k Literal.pp lit
    ...
    match r with
        ...
        | L_rule r -> (
            ...
            let view_lit id (lit : Literal.t) =
                match lit with
                    | Equation (lhs, _, _) when Literal.is_predicate_lit lit -> view_atom id lhs
                    | _ -> None
            in
  ...
  let mk_rule_proj_ (p : Ind_ty.projector) proof : rule =
      ...
      let t =
          T.app_full
            ...
            (List.map Type.var ty_vars) (List.map T.var vars)
      in
  ...
  let declare_proj ~proof (p : Ind_ty.projector) : unit =
      let p_id = Ind_ty.projector_id p in
          match as_defined_cst p_id with
              | Some _ -> Util.invalid_argf "cannot declare proj %a, already defined" ID.pp p_id
              | None ->
      ...
      let c_id = c.Ind_ty.cstor_name in
          if not (CCList.is_empty c.Ind_ty.cstor_args) then
            match as_defined_cst c_id with
                | Some _ -> Util.invalid_argf "cannot declare cstor %a, already defined" ID.pp c_id
                | None ->
