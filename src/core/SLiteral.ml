...
let equal eq a b =
   match (a, b) with
      ...
      | Eq (a1, a2), Eq (b1, b2) | Neq (a1, a2), Neq (b1, b2) -> (eq a1 b1 && eq a2 b2) || (eq a1 b2 && eq a2 b1)
...
let of_form f =
   match F.view f with
      | F.Not f' -> ( match F.view f' with F.Atom t -> Atom (t, false) | _ -> not_lit f)
      | F.Eq (t1, t2) -> Eq (t1, t2)
      | F.Neq (t1, t2) -> Neq (t1, t2)
      | F.Atom t -> Atom (t, true)
      | F.True -> True
      | F.False -> False
      | F.Or _ | F.And _ | F.Equiv _ | F.Xor _ | F.Imply _ | F.Forall _ | F.Exists _ -> not_lit f
