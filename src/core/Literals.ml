...
module Pos = struct
  let _fail_lits lits pos =
      let msg =
          CCFormat.sprintf "@[invalid position @[%a@]@ in lits [@[%a@]]@]" Position.pp pos
            (CCFormat.array Lit.pp) lits
      in
          invalid_arg msg
  ...
  let at lits pos =
      match pos with
          | Position.Arg (idx, pos') when idx >= 0 && idx < Array.length lits -> Lit.Pos.at lits.(idx) pos'
          | _ -> _fail_lits lits pos

  let lit_at lits pos =
      match pos with
          | Position.Arg (i, pos') when i >= 0 && i < Array.length lits -> (lits.(i), pos')
          | _ -> _fail_lits lits pos
  ...
  let is_toplevel_pos = function
      | Position.Arg (_, (Position.Left p | Position.Right p)) -> Position.equal p Position.stop
      | _ -> false
end
...
module Conv = struct
  ...
  let to_tst lits =
      ...
      let var_seq = Seq.vars lits in
          ...
          Array.map (fun t -> Lit.Conv.lit_to_tst ~ctx (Lit.Conv.to_form t)) lits |> Array.to_list
          |> fun or_args ->
          let ty = TypedSTerm.Ty.prop in
          let clause_vars = T.VarSet.of_iter var_seq in
          let vars =
              clause_vars |> T.VarSet.to_list |> CCList.map (fun v -> T.Conv.to_simple_term ctx (T.var v))
          in
          let disjuncts =
              match or_args with
...
module View = struct
  let get_eqn lits pos =
      match pos with
          | Position.Arg (idx, pos') when idx < Array.length lits -> Lit.View.get_eqn lits.(idx) pos'
          | _ -> None
...
let fold_eqn_simple ?sign lits k =
    ...
    let rec aux i =
        if i = Array.length lits then ()
          ...
          let sign = Lit.is_positivoid lits.(i) in
              (match lits.(i) with
                  | Lit.Equation (l, r, _) when sign_ok sign -> k (l, r, sign, Position.(arg i @@ left @@ stop))
                  | Lit.Equation _ | Lit.True | Lit.False -> ());
              aux (i + 1)
    in
        aux 0
...
let pp_gen ~false_ ~l ~r ~sep ~pp_lit out lits =
    match lits with
        ...
        | [| l |] -> pp_lit out l
        | _ -> Format.fprintf out "%s@[<hv>%a@]%s" l CCFormat.(array ~sep pp_lit) lits r
...
let pp_tstp_closed out lits =
    let pp_vars out = function
        | [] -> ()
        | l -> Format.fprintf out "![@[%a@]]:@ " (Util.pp_list ~sep:", " Type.TPTP.pp_typed_var) l
    in
...
let is_shielded var (lits : t) : bool =
    ...
    let rec shielded_by_term ~root t =
        match T.view t with
            ...
            | _ when Type.Seq.vars (T.ty t) |> Iter.exists (var_eq var) -> true (* shielded by type *)
            | T.Var _ | T.DB _ | T.Const _ -> false
            | T.AppBuiltin (_, l) -> List.exists (shielded_by_term ~root:false) l
            | T.App (f, l) -> shielded_by_term ~root f || List.exists (shielded_by_term ~root:false) l
