...
let weight_arity0 ~signature =
    let _sig = ref signature in
        ...
        let max_arity a b =
            match (a, b) with
                | None, x -> Some x
                | Some (_, arity1), (_, arity2) -> if arity2 > arity1 then Some b else a
        in
...
let lambda_def_weight lm_w db_w base_weight clauses =
                      ...
                      | Some (hd_id, r) ->
                          ...
                          |> Iter.iter (fun k ->
                                 ID.Tbl.update dependencies
                                   ...
                                   ~k)
                      | _ -> Util.debugf ~section 2 "is not def: %a := %a" (fun k -> k T.pp lhs T.pp rhs))
              | _ -> ()))
      clauses;
    ...
    let eval_weight ~weights t =
        ...
        and aux_l = function
            | [] -> Weight.zero
            | [ x ] -> aux x
            | l -> List.fold_left (fun acc t -> Weight.( + ) acc (aux t)) Weight.zero l
        in
...
let create ?(weight = weight_constant) ?(arg_coeff = arg_coeff_default) ?(db_w = db_w_def) ?(lmb_w = lmb_w_def)
    c l =
...
let add_list ~signature p l =
    ...
    let is_new_sym id =
        if Lazy.is_val p.tbl then ID.Tbl.mem (Lazy.force p.tbl) id else List.mem id p.snapshot
    in

    let l = List.filter is_new_sym l in
    let cutoff = 30 in
    let snapshot =
        if CCList.length l < cutoff then mk_snapshot l else List.fast_sort p.constr (p.snapshot @ l)
    in
