...
let compatible_features_unif f1 f2 =
   match f1 with
      | S s1 -> ( match f2 with S s2 -> ID.equal s1 s2 | A | B | Ignore -> true | N | DB _ | BI _ -> false)
      | BI b1 -> (
         match f2 with BI b2 -> Builtin.equal b1 b2 | A | B | Ignore -> true | N | DB _ | S _ -> false)
      | Ignore -> true
      | B -> true
      | A -> ( match f2 with N -> false | DB _ | Ignore | S _ | A | B | BI _ -> true)
      | DB i -> ( match f2 with DB j -> i = j | B | A | Ignore -> true | S _ | N | BI _ -> false)
      | N -> ( match f2 with N | B | Ignore -> true | A | DB _ | S _ | BI _ -> false)

(** check whether two features are compatible for matching. *)
let compatible_features_match f1 f2 =
   match f1 with
      | S s1 -> ( match f2 with S s2 -> ID.equal s1 s2 | Ignore -> true | _ -> false)
      | BI b1 -> ( match f2 with BI b2 -> Builtin.equal b1 b2 | Ignore -> true | _ -> false)
      | Ignore | B -> true
      | A -> ( match f2 with A | DB _ | S _ | Ignore | BI _ -> true | _ -> false)
      | DB i -> ( match f2 with DB j -> i = j | Ignore -> true | _ -> false)
      | N -> ( match f2 with N | Ignore -> true | _ -> false)
  ...
  let add idx t data =
     ...
     let rec recurse trie features =
        match (trie, features) with
           ...
           | Node _, [] | Leaf _, _ :: _ -> failwith "different feature length in fingerprint trie"
     in
