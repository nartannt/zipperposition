...
module TEF = struct
  ...
  let emit_duration_event oc (prof : t) ~start ~end_ () : unit =
      ...
      let pid = Unix.getpid () in
          emit_sep_ oc;
          Printf.fprintf oc {json|{"pid": %d,"tid":1,"dur": %.2f,"ts": %.2f,"name":"%s","ph":"X"}|json} pid dur
            ts prof.prof_name;
          ()
          ...
          (fun profiler ->
            if profiler.prof_calls > 0 then
              ...
              let total_s = profiler.prof_total *. 1e-6 in
                  Format.fprintf out "@[%-39s %10d %9.3f %9.2f %9.1f %9.1f@]@," profiler.prof_name
                    profiler.prof_calls total_s
                    (total_s *. 100. /. tot)
                    profiler.prof_max
