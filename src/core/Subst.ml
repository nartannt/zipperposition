...
let[@unroll 2] rec deref subst ((t, sc_t) as term) =
    match T.view t with
        | T.Var v -> ( match find subst (v, sc_t) with Some t' -> deref subst t' | None -> term)
        | _ -> term
...
let apply_aux ~sv subst ~f_rename t sc =
              ...
              | T.HasType ty ->
                  ...
                  let res =
                      match T.view t with
                          ...
                          | T.DB i -> if T.equal ty ty' then t else T.bvar ~ty:ty' i
                          | T.Var v -> (
    ...
    and aux_list l sc depth =
        match l with
            | [] -> []
            | [ t1 ] -> [ aux t1 sc depth ]
            | t1 :: t2 :: l' -> aux t1 sc depth :: aux t2 sc depth :: aux_list l' sc depth
    in
        aux t sc sv
...
module FO = struct
  ...
  let compose ~scope s1 s2 =
      ...
      let subs_as_map =
          List.map
            (fun ((v, sc_v), (t, sc_t)) ->
              ( (v, sc_v),
                ((Lambda.snf (apply Renaming.none s2 (Term.of_term_unsafe t, sc_t)) : term :> T.t), scope) ))
            subs_l1
          @ to_list s2
      in
          of_list subs_as_map
  ...
  let filter f s =
      filter
        (fun (v, sc_v) (t, sc_t) ->
          f (HVar.update_ty ~f:Type.of_term_unsafe v, sc_v) (Term.of_term_unsafe t, sc_t))
        s
            ...
            CCList.map
              (fun v ->
                let derefed = deref subst v in
                    if not (Term.is_var (fst derefed)) then raise (invalid_arg "found a non-variable")
                    else derefed)
              subset
