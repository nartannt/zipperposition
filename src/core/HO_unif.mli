...
(** Given a variable of type [τ1…τn -> prop], enumerate possible shapes for it
    ...
    @param mode if [`Neg], only tries negation; [`None], do nothing; otherwise do all connectives *)
...
(** [unif_pairs pairs ~scope_new_vars] returns a list of (partial) solutions to the HO unification problem
    [pairs]. Each solution is a list of remaining constraints (with the substitution already applied), a
    substitution, some penalty to influence the search space, and a renaming used for the substitution *)
