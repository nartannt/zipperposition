           ...
           if redundant = 0 then (0, t)
             ...
             let _, m =
                List.fold_right
                  (fun arg (idx, m) ->
                    if idx = -1 then (idx, m)
                    else if not @@ List.exists (fun tt -> T.DB.contains tt (T.as_bvar_exn arg)) non_redundant
                    then (idx + 1, m + 1)
                    else (-1, m))
                  r_bvars (0, 0)
             in
...
let rec is_lambda_pattern t =
   match T.view (whnf t) with
      ...
      | T.App (hd, args) -> if T.is_var hd then all_distinct_bound args else List.for_all is_lambda_pattern args
