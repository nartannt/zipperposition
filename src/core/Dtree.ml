...
module Make (E : Index.EQUATION) = struct
  ...
  (** get/add/remove the leaf for the given flatterm. The continuation k takes the leaf, and returns a leaf
      option that replaces the old leaf. This function returns the new trie. *)
    ...
    let rec goto trie t rebuild =
      ...
      | c :: t' ->
        ...
        | Some subtrie ->
          let rebuild' subtrie =
            match subtrie with
            | _ when is_empty subtrie -> rebuild { trie with map = CharMap.remove c trie.map }
