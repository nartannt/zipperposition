                                ...
                                CCList.flat_map_i
                                  (fun j db_j ->
                                    if i < j && Type.equal (T.ty db_i) (T.ty db_j) then
                                      [
                                        T.fun_l ty_args
                                          (T.Form.eq (project ~db_vars db_i) (project ~db_vars db_j));
                                        T.fun_l ty_args
                                          (T.Form.neq (project ~db_vars db_i) (project ~db_vars db_j));
                                      ]
                                    else [])
                                  db_vars)
                              db_vars
                 ...
                 | `Full | `Quants ->
                    let n = List.length ty_args in
                       CCList.mapi
                         ...
                         ty_args
                       |> CCList.fold_left
                            (fun acc opt -> match opt with Some (x, y) -> x :: y :: acc | None -> acc)
                            []
                 | _ -> []
           and l_symbols =
              match mode with
                 ...
                 | `Full ->
                    ...
                    let bvars =
                       snd
                       @@ List.fold_right (fun ty (idx, res) -> (idx + 1, T.bvar ~ty idx :: res)) arg_tys (0, [])
                    in
                    let fresh_vars =
                       List.mapi
                         (fun i ty ->
                                  ...
                                  if i < j && Type.equal (T.ty db_i) (T.ty db_j) then
                                    ...
                                    in
                                       if Type.returns_prop (T.ty db_i) then
                                         ...
                                         @ [
                                             T.fun_l ty_args
                                               (T.Form.and_ (project ~db_vars db_i) (project ~db_vars db_j));
                                             T.fun_l ty_args
                                               (T.Form.or_ (project ~db_vars db_i) (project ~db_vars db_j));
                                           ]
                                       else res
                                  else [])
                                db_vars
...
let pp_pair out ((env, t, u) : pair) =
   Format.fprintf out "(@[@[`%a` =?=@]@ `%a`@ :env [@[%a@]]@])" T.pp t T.pp u (Util.pp_list ~sep:", " Type.pp)
     env
  ...
  let whnf_deref (subst : US.t) (t, sc) =
     let t =
        match T.view t with
           ...
           | T.App (f, l) when T.is_var f -> T.app (US.FO.deref subst (f, sc) |> fst) l
           | _ -> t
     in
        Lambda.whnf t
  ...
  let unif_lambda ~offset env v args t : pair list * int =
     ...
     let ty_t_args, t_body = T.open_fun t in
        ...
        let bind_v =
           ...
           let rhs =
              T.app (T.var v') (List.mapi (fun i ty -> T.bvar ~ty (n - i - 1)) all_args) |> T.fun_l all_args
           in
              (env, T.var v, rhs)
        ...
        and new_pair_body =
           ...
           let lhs =
              T.app (T.var v')
                (List.map (T.DB.shift n) args @ List.mapi (fun i ty -> T.bvar ~ty (n - i - 1)) ty_t_args)
           in
           ...
           |> Iter.filter_map (fun (i, ty_arg_i) ->
                  let ty_args_i, ty_ret_i = Type.open_fun ty_arg_i in
                     try
                       ...
                       let offset, f_vars =
                          ty_args_i |> List.map (Type.arrow all_ty_args) |> mk_fresh_vars offset
                       in
                   ...
                   try
                     ...
                     let push_new ~penalty ~subst ~offset rule pairs : unit =
                        ...
                        let pb' = mk_pb ~penalty ~subst ~offset (pairs @ pairs_tl) |> normalize_pb sc in
                           match pb' with
                              | None -> ()
                              | Some pb' ->
                                 Util.debugf ~section 5 "(@[ho_unif.push@ :rule %s@ %a@])" (fun k ->
                                     k rule pp_pb pb');
                     ...
                     let hd2, l2 = T.as_app t2 in
                        match (T.view hd1, T.view hd2) with
                           | _ when T.equal t1 t2 -> push_new "triv" ~penalty ~offset ~subst [] (* trivial *)
                           ...
                           | T.Const _, _ | T.Fun _, _ | T.AppBuiltin _, _ | T.DB _, _ -> fail ()
                   with Unif.Fail ->
