...
let _hash_norec t =
   match view t with
      ...
      | Bind (b, varty, t') -> Hash.combine4 3 (Binder.hash b) (hash varty) (hash t')
      ...
      let h_t =
         if d = 0 then 10 (* fuel is exhausted *)
           ...
           match t.term with
              ...
              | DB v -> Hash.combine2 2 (Hash.int v)
              | Bind (b, varty, t') -> Hash.combine4 3 (Binder.hash b) (aux (d - 1) varty) (aux (d - 1) t')
              | Const s -> Hash.combine2 4 (ID.hash s)
              | App (f, l) -> Hash.combine3 10 (aux (d - 1) f) (Hash.list (aux (d - 1)) l)
              | AppBuiltin (b, l) -> Hash.combine3 20 (Builtin.hash b) (Hash.list (aux (d - 1)) l)
...
let _eq_norec t1 t2 =
   ...
   match (t1.term, t2.term) with
      ...
      | AppBuiltin (b1, l1), AppBuiltin (b2, l2) -> Builtin.equal b1 b2 && _eq_norec_list l1 l2
      | _ -> false
...
let ho_weight_ t t_ty =
   let rec aux t t_ty =
      let init_w s_ty =
         match s_ty with
            | NoType -> 0
            | HasType ty -> ( match view ty with AppBuiltin (Builtin.Arrow, l) -> List.length l - 1 | _ -> 0)
      in
         match t with
            ...
            | App (f, l) -> if is_var f then 1 else init_w t_ty + aux_l (aux (view f) (ty f)) l
...
let make_ ~props ~ty term =
   { term; ty; id = ~-1; payload = No_payload; props; ho_weight = lazy (ho_weight_ term ty) }
...
let rec debugf out t =
   match view t with
      ...
      | AppBuiltin (b, l) -> Format.fprintf out "(@[<1>%a@ %a@])" Builtin.pp b (Util.pp_list debugf) l
      | Var i -> HVar.pp out i
      | DB i -> Format.fprintf out "Y%d" i
      | Const s -> ID.pp out s
      | App (_, []) -> assert false
      | App (s, l) -> Format.fprintf out "(@[<1>%a@ %a@])" debugf s (Util.pp_list debugf) l
      | Bind (b, varty, t') -> Format.fprintf out "(@[<1>%a@ %a@ %a@])" Binder.pp b debugf varty debugf t'
...
module DB = struct
           ...
           | HasType ty -> (
              let ty = recurse ~depth acc ty in
                 match view t with
                    ...
                    | App (f, l) -> app ~ty (recurse ~depth acc f) (List.map (recurse ~depth acc) l)
                    | AppBuiltin (s, l) -> app_builtin ~ty s (List.map (recurse ~depth acc) l))
     in
        recurse ~depth acc t
        ...
        | HasType ty -> (
           let ty = _replace depth ty ~to_replace in
              match view t with
                 ...
                 | Var v -> var (HVar.cast ~ty v)
                 | DB i -> if i < depth then bvar ~ty i else bvar ~ty (i + List.length to_replace) (* shift *)
                 | Const s -> const ~ty s
                 | Bind (s, varty, t') ->
                    let varty' = _replace depth ~to_replace varty in
                    let t' = _replace (depth + 1) t' ~to_replace in
                       bind ~ty ~varty:varty' s t'
                 | App (f, l) -> app ~ty (_replace depth ~to_replace f) (List.map (_replace depth ~to_replace) l)
                 | AppBuiltin (s, l) -> app_builtin ~ty s (List.map (_replace depth ~to_replace) l))
                 ...
                 match view t with
                    ...
                    | DB i -> (
                       match DBEnv.find env i with
                          | None ->
                             if i >= DBEnv.size env then bvar ~ty (i - DBEnv.size env0) (* unshift *)
                             else bvar ~ty i
                          | Some t' ->
  ...
  let apply_subst subst t =
     ...
     and aux' depth ty t =
        match view t with
           | Var v -> ( try shift depth (VarMap.find v subst) with Not_found -> var (HVar.cast ~ty v))
           | DB i -> bvar ~ty i
           | Const s -> const ~ty s
           | Bind (s, varty, t') ->
...
let bind_vars ~ty b vars t =
   ...
   let subst =
      CCList.foldi (fun s i v -> VarMap.add v (bvar ~ty:(HVar.ty v) i) s) VarMap.empty (List.rev vars)
   in
      List.fold_right
        (fun v t ->
...
module Pos = struct
  ...
  let rec at t pos =
     match (view t, pos) with
        | _, P.Type pos' -> ( match t.ty with NoType -> fail_ t pos | HasType ty -> at ty pos')
        | _, P.Stop -> t
        | Var _, _ -> fail_ t pos
        | Bind (_, _, t'), P.Body subpos -> at t' subpos
        | App (t, _), P.Head subpos -> at t subpos
        | App (_, l), P.Arg (n, subpos) when n < List.length l -> at (List.nth l (List.length l - 1 - n)) subpos
  ...
  let rec replace t pos ~by =
     match (t.ty, view t, pos) with
        ...
        | HasType ty, Bind (s, varty, t'), P.Body subpos -> bind ~ty ~varty s (replace t' subpos ~by)
...
let type_is_unifiable (ty : t) : bool =
   match view ty with
      | AppBuiltin ((Builtin.TyInt | Builtin.TyRat), _) | Bind (Binder.ForallTy, _, _) -> false
      | _ -> true
         ...
         | App (f, l) -> (
            ...
            let as_prefix = match view f with Const id -> ID.as_prefix id | _ -> None in
               match (as_infix, as_prefix, l) with
                  ...
                  | _, Some s, [ a ] -> Format.fprintf out "@[<1>%s@ %a@]" s (_pp_surrounded depth) a
                  | _ ->
      ...
      match view t with
         | App (_, l)
           when (not !show_type_arguments)
            ...
            _pp depth out t
         | Bind _ | AppBuiltin (_, _ :: _) | App (_, _ :: _) -> Format.fprintf out "(@[%a@])" (_pp depth) t
         | _ -> _pp depth out t
   in
      _pp depth out t
...
let rec pp_zf out t =
      ...
      match view t with
         ...
         | AppBuiltin (Builtin.Arrow, ret :: args) ->
            Format.fprintf out "@[%a@ -> %a@]"
              ...
              args (_pp_surrounded depth) ret
         | AppBuiltin (b, [ x; body ])
           when Builtin.equal b Builtin.ExistsConst || Builtin.equal b Builtin.ForallConst ->
         ...
         | AppBuiltin (b, l) -> Format.fprintf out "@[%a(%a)@]" Builtin.ZF.pp b (Util.pp_list (pp_ depth)) l
         | App (f, l) -> (
            match l with
               | [] -> pp_ depth out f
               | _ :: _ ->
   ...
   and _pp_surrounded depth out t =
      match view t with
         | Bind _ | AppBuiltin (_, _ :: _) | App (_, _ :: _) -> Format.fprintf out "(@[%a@])" (pp_ depth) t
         | _ -> pp_ depth out t
   in
      pp_ 0 out t
