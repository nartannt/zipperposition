...
let as_clause f =
  ...
  | F.And _ | F.Equiv _ | F.Xor _ | F.Imply _ | F.Forall _ | F.Exists _ -> not_cnf_ f
  ...
  *)
  let flatten_rec ?of_ ~lazy_cnf ~should_define (ctx : Skolem.ctx) stmt (pos : position)
    ...
    let rec aux pos vars t =
      ...
      | T.Const _ -> return t
      | T.App (f, l) -> T.app ~ty:(T.ty_exn t) <$> aux Pos_inner vars f <*> map_m (aux Pos_inner vars) l
      | T.Ite (a, b, c) ->
        (match T.view a with
      ...
      | T.AppBuiltin (Builtin.Eq, [ _; a; b ]) -> F.eq <$> aux Pos_toplevel vars a <*> aux Pos_toplevel vars b
      ...
      | T.AppBuiltin (Builtin.Neq, [ _; a; b ]) when (T.is_fun a || T.is_fun b) && not lazy_cnf ->
        ...
        assert (vars_exist <> []);
        aux pos vars t'
      | T.AppBuiltin (Builtin.Neq, [ _; a; b ]) -> F.neq <$> aux Pos_toplevel vars a <*> aux Pos_toplevel vars b
      (* >|= aux_maybe_define pos *)
      | T.AppBuiltin (Builtin.Imply, [ a; b ]) -> F.imply <$> aux Pos_toplevel vars a <*> aux Pos_toplevel vars b
      (* >|= aux_maybe_define pos *)
      | T.AppBuiltin (Builtin.Equiv, [ a; b ]) -> F.equiv <$> aux Pos_toplevel vars a <*> aux Pos_toplevel vars b
      (* >|= aux_maybe_define pos *)
      | T.AppBuiltin (Builtin.Xor, [ a; b ]) -> F.xor <$> aux Pos_toplevel vars a <*> aux Pos_toplevel vars b
      (* >|= aux_maybe_define pos *)
      ...
      | T.AppBuiltin (Builtin.Not, [ a ]) -> F.not_ <$> aux Pos_toplevel vars a (*>|= aux_maybe_define pos*)
      | T.AppBuiltin (b, l) -> return (T.app_builtin ~ty:(T.ty_exn t) b) <*> map_m (aux Pos_inner vars) l
      ...
      | T.Bind (Binder.ForallTy, _, _) | T.Multiset _ | T.Record _ | T.Meta _ -> assert false
...
let skolemize ~ctx f =
  ...
  let rec skolemize subst f =
    ...
    | F.Xor _ | F.Imply _ | F.Equiv _ -> error_ "can only skolemize a NNF formula"
...
let estimate_num_clauses ~pos f =
  ...
  let rec num pos f =
    ...
    | F.Eq _, _ | F.Neq _, _ | F.Atom _, _ | F.True, _ | F.False, _ -> E.Exactly 1
    ...
    | F.Equiv (a, b), true -> E.((num true a */ num false b) +/ (num false a */ num true b))
    | F.Equiv (a, b), false -> E.((num true a */ num true b) +/ (num false a */ num false b))
    | F.Xor (a, b), true ->
      (* a xor b is defined as  (not a) <=> b *)
      E.((num false a */ num false b) +/ (num true a */ num true b))
    | F.Xor (a, b), false -> E.((num false a */ num true b) +/ (num true a */ num false b))
...
let introduce_defs ~ctx ~is_pos stmt f =
  ...
  let rec maybe_rename ~polarity a b f =
    ...
    let c1, c2 =
      ...
      | `Both -> E.((a */ p true f) +/ (b */ p false f), a +/ b +/ p true f +/ p false f)
    in
...
let flatten ~ctx ~lazy_cnf ~should_define seq : _ Iter.t =
  ...
  |> Iter.flat_map_l (fun stmt ->
      ...
      let new_sts =
        ...
        | Stmt.Rewrite d -> flatten_def stmt d |> List.map (fun d' -> Stmt.rewrite ~attrs ~proof:(proof ()) d')
        | Stmt.Def l ->
          let l =
            List.map
              (fun d ->
        ...
        | Stmt.Assert f -> flatten_axiom stmt f |> List.map (fun f -> Stmt.assert_ ~attrs ~proof:(proof ()) f)
        | Stmt.Lemma l ->
          List.map (fun f -> Stmt.lemma ~attrs ~proof:(proof ()) [ F.and_ (flatten_axiom stmt f) ]) l
        | Stmt.Goal f -> [ Stmt.goal ~attrs ~proof:(proof ()) (F.and_ (flatten_axiom stmt f)) ]
...
let cnf_of_iter ~ctx ?(opts = []) (seq : Stmt.input_t Iter.t) : _ CCVector.t =
  ...
  CCVector.iter
    (fun stmt ->
      ...
      | Stmt.TyDecl (id, ty) -> CCVector.push res (Stmt.ty_decl ~attrs ~proof id ty)
      | Stmt.Lemma l ->
        let proof = proof_cnf stmt in
