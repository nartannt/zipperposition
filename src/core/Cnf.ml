...
module Flatten = struct
  ...
  *)
  let flatten_rec ?of_ ~lazy_cnf ~should_define (ctx : Skolem.ctx) stmt (pos : position)
      ...
      let mk_pat what =
          match of_ with None -> what ^ "_" | Some id -> Fmt.sprintf "%s_%s_" (ID.name id) what
      in
          ...
          match T.view t with
              ...
              | T.AppBuiltin (Builtin.Distinct, l) ->
                  ...
                  let f = if CCList.length l == 1 then List.hd l else T.Form.and_ ?loc:(T.loc t) l in
                      Util.debugf ~section 5 "(@[flatten.expand-distinct@ %a@ :into %a@])" (fun k ->
                          k T.pp t T.pp f);
                      aux pos vars f
              | T.Var v -> (
                  (* look [v] up in subst *)
                  get_subst >>= fun subst ->
                  match T.Subst.find subst v with None -> return t | Some t' -> aux pos vars t')
              | T.Const _ -> return t
                  ...
                  match T.view u with
                      ...
                      | _ ->
                          ...
                          let rules = to_list' cases in
                              Util.debugf ~section 5 "(@[define_match@ :term %a@ :rules %a@])" (fun k ->
                                  k T.pp t pp_rules rules);
                              let def =
...
let introduce_defs ~ctx ~is_pos stmt f =
    ...
    and rename_form ~polarity f =
        ...
        let p = def.Skolem.proxy in
            Util.debugf ~section 2 "@[<2>introduce@ def. @[%a@]@ for subformula `@[%a@]`@ with pol %a@]"
        ...
        |> Iter.flat_map_l (fun stmt ->
               ...
               let new_sts =
                   match stmt.Stmt.view with
                       ...
                       | Stmt.Lemma l ->
                           List.map
                             (fun f -> Stmt.lemma ~attrs ~proof:(proof ()) [ F.and_ (flatten_axiom stmt f) ])
                             l
        ...
        match d with
            | Stmt.Def_term { vars; id; ty = ty_id; args; rhs; as_form } ->
                ...
                let ty_vars, ty_args, ty_ret = T.Ty.unfold (T.ty_exn rhs) in
                    if ty_vars <> [] then
                      errorf_ "cannot deal with polymorphic definition@ `%a`" (Stmt.pp_def_rule T.pp T.pp T.pp)
                        d;
                    if ty_args = [] then Stmt.Def_term { vars; id; ty = ty_id; args; rhs; as_form }
                      ...
                      let rhs = T.app_whnf ~ty:ty_ret rhs (List.map T.var new_vars) in
                          if T.Ty.is_prop ty_ret then
                            let lhs_t =
                                T.app_whnf ~ty:ty_ret (T.const ~ty:ty_id id) (args @ List.map T.var new_vars)
                               ...
                               List.map
                                 (fun d ->
                                   ...
                                   let new_d = { d with Stmt.def_rules = rules } in
                                       Util.debugf ~section 2 "(@[simplify-def@ `@[%a@]`@ :into `@[%a@]`@])"
                                         (fun k ->
    ...
    let conv_form_sk f : (ID.t * type_) list * clause list =
        ...
        let view_as_cnf f =
            ...
            let eligible_for_lazy_cnf f =
                ...
                && T.Seq.vars f |> Iter.for_all (fun sf -> not (T.Ty.is_tType (Var.ty sf)))
            in
                        ...
                        | SLiteral.Atom (lhs_t, true), [ rhs_t ] -> (
                            match (T.as_id_app lhs_t, as_lit_opt rhs_t) with
                                | Some (id, ty_id, args), Some lit when SLiteral.is_pos lit ->
                                    ...
                                    let as_form : clause =
                                        [
                                          SLiteral.eq (T.app (T.const ~ty:ty_id id) ~ty:(T.ty_exn rhs) args) rhs;
                                        ]
                                    in
                                        [ Stmt.Def_term { vars; id; ty = ty_id; args; rhs; as_form } ]
