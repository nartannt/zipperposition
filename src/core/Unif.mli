...
(** in HO, we have [f1 l1] and [f2 l2], where application is left-associative. we need to unify from the right
    (the outermost application is on the right) so this returns pairs to unify (including heads). *)

val pair_lists_left : term list -> term -> term list -> term -> term list * term list
(** in HO, we have [l1 -> ret1] and [l2 -> ret2], where [->] is right-associative. we need to unify from the
    left, so this returns pairs to unify (including return types). *)
...
module FO : sig
  ...
  (** anti-unification of the two terms, returning disagreement pairs
      @param cut
        if [cut=n], then the returned list will have length at most [n] (if it's too long then [None] is
        returned) *)
