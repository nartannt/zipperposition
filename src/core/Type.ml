...
let order ty : int =
  let rec aux ty =
    match view ty with
    | Forall ty -> aux ty
    | Fun (l, ret) -> List.fold_left (fun o arg -> max o (1 + aux arg)) (aux ret) l
...
let apply ty0 args0 =
  ...
  and aux_l ty_ret exp_args args env =
    ...
    | [], _ ->
      ...
      | _ -> err_applyf_ "@[<2>Type.apply:@ unexpected arguments [@[%a@]]@]" (Util.pp_list T.pp_zf) args)
    | exp :: exp_args', a :: args' ->
      ...
      if T.equal exp' (T.ty_exn a) then
        ...
        err_applyf_
          "@[<2>Type.apply:@ wrong argument type,@ expected `@[_ : %a@]`@ but got `@[%a : %a@]`@ when applying \
           `%a` to@ [@[%a@]]@ in env [%a]@]"
          T.pp_zf exp' T.pp_zf a T.pp_zf (T.ty_exn a) T.pp_zf ty0 (Util.pp_list T.pp_zf) args0 (DBEnv.pp T.pp_zf)
          env
  in
...
module TPTP = struct
  ...
  let rec pp_tstp_rec depth out t =
    ...
    | App (p, args) -> Format.fprintf out "@[<2>%a(%a)@]" ID.pp_tstp p (Util.pp_list (pp_tstp_rec depth)) args
    | Fun (args, ret) -> Format.fprintf out "%a > %a" (pp_l depth) args (pp_tstp_rec depth) ret
    | Forall ty' -> Format.fprintf out "!>[Tb%d:$tType]: %a" depth (pp_inner (depth + 1)) ty'
  ...
  let rec pp_ho_depth depth out t =
    ...
    | Fun (args, ret) -> Format.fprintf out "%a > %a" (pp_l depth) args (pp_inner depth) ret
    | Forall ty' -> Format.fprintf out "!>[Tb%d:$tType]: %a" depth (pp_inner (depth + 1)) ty'
  ...
  and pp_l depth out l =
    ...
    | [] -> assert false
    | [ ty ] -> pp_inner depth out ty
    | _ -> Format.fprintf out "@[%a@]" (Util.pp_list ~sep:" > " (pp_inner depth)) l
...
let rec pp_rec depth out t =
  ...
  | App (p, []) -> ID.pp out p
  | App (p, args) -> Format.fprintf out "@[<2>%a %a@]" ID.pp p (Util.pp_list ~sep:" " (pp_inner_app depth)) args
  | Fun (args, ret) ->
    Format.fprintf out "@[%a →@ %a@]" (Util.pp_list ~sep:" → " (pp_inner_fun depth)) args (pp_rec depth) ret
  | Forall ty' -> Format.fprintf out "@[Π T%i.@ %a@]" depth (pp_inner_fun (depth + 1)) ty'
...
module Conv = struct
  ...
  let () =
    Printexc.register_printer (function
      | Error t -> Some (Util.err_spf "@[<2>Type.Conv.Error on `@[%a@]`@]" PT.pp t)
      | _ -> None)
