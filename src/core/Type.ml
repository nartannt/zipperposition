        ...
        match (exp_args, args) with
            ...
            | [], _ -> (
                match T.view (T.DB.eval env ty_ret) with
                    ...
                    | _ ->
                        err_applyf_ "@[<2>Type.apply:@ unexpected arguments [@[%a@]]@]" (Util.pp_list T.pp_zf)
                          args)
            | exp :: exp_args', a :: args' ->
                ...
                let exp' = T.DB.eval env exp in
                    if T.equal exp' (T.ty_exn a) then aux_l ty_ret exp_args' args' env
                      ...
                      err_applyf_
                        "@[<2>Type.apply:@ wrong argument type,@ expected `@[_ : %a@]`@ but got `@[%a : %a@]`@ \
                         when applying `%a` to@ [@[%a@]]@ in env [%a]@]"
...
module Conv = struct
  ...
  and var_to_simple_var ?(prefix = "A") ctx v =
      ...
      with Not_found ->
        let v' =
            Var.of_string ~ty:(to_simple_term ctx (HVar.ty v)) (CCFormat.sprintf "%s%d" prefix (HVar.id v))
        in
