    ...
    match view t with
        ...
        | Bind (s, _, _) ->
            ...
            let pp_bound_var out v = Format.fprintf out "@[%a%a@]" Var.pp_fullc v pp_var_ty v in
                Format.fprintf out "@[<2>%a %a.@ %a@]" Binder.pp s (Util.pp_list ~sep:" " pp_bound_var) vars
                  pp_inner body
        ...
        | Match (u, l) ->
            let pp_branch out (c, vars, rhs) =
                Format.fprintf out "@[<2>case@ @[%a%a%a@] ->@ %a@]" ID.pp c.cstor_id
                  (Util.pp_list0 ~sep:" " pp_inner) c.cstor_args (Util.pp_list0 ~sep:" " Var.pp_fullc) vars pp
                  rhs
            in
...
let () =
    Printexc.register_printer (function IllFormedTerm msg -> Some ("ill formed term: " ^ msg) | _ -> None)
...
let pp_stack out l =
    ...
    let pp_frame out (t1, t2) =
        Format.fprintf out "@[<2>unifying `@[%a@,%a@]`@ and `@[%a@,%a@]`@]" pp t1 pp_ty (ty t1) pp t2 pp_ty
          (ty t2)
    in
...
let () =
    Printexc.register_printer (function
        | UnifyFailure (msg, st, loc) ->
            Some
              (CCFormat.sprintf "@[<2>@{<Red>unification error@}:@ %s@ %a@,%a@]" msg pp_stack st Loc.pp_opt loc)
        | _ -> None)
        ...
        match (view t1, view t2) with
                  ...
                  (fun (c1, vars1, rhs1) (c2, vars2, rhs2) ->
                    ...
                    then (
                      if not (ID.equal c1.cstor_id c2.cstor_id) then
                        fail_ "constructors %a and %a are not compatible (subst {@[%a@]})" ID.pp c1.cstor_id
                          ID.pp c2.cstor_id Subst.pp subst;
            ...
            | AppBuiltin (b1, l1), AppBuiltin (b2, l2) when List.length l1 = List.length l2 ->
                if Builtin.equal b1 b2 then unif_l subst l1 l2
                  ...
                  fail_ "%a and %a are not compatible (subst {@[%a@]})" Builtin.pp b1 Builtin.pp b2 Subst.pp
                    subst
