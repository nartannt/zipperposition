...
let rec compare t1 t2 =
  ...
  if h1 <> h2 then
    ...
    | Bind (s1, v1, t1), Bind (s2, v2, t2) -> Binder.compare s1 s2 <?> (Var.compare, v1, v2) <?> (compare, t1, t2)
...
let rec is_ground t =
  ...
  | Record (l, rest) -> CCOpt.map_or is_ground ~default:true rest && List.for_all (fun (_, t') -> is_ground t') l
...
module Form = struct
  ...
  let view (t : term) =
    ...
    | Ite _ | Match _ | Let _ | Multiset _ | Record _ | Meta _ | Var _ | Const _ | App _ | AppBuiltin _ -> Atom t
  ...
  and unify_terms subst t1 t2 =
    ...
    | Match (u1, l1), Match (u2, l2) when List.length l1 = List.length l2 ->
      ...
      List.iter2
        (fun (c1, vars1, rhs1) (c2, vars2, rhs2) ->
          if List.length vars1 = List.length vars2 && List.length c1.cstor_args = List.length c2.cstor_args then (
    ...
    | AppBuiltin (b1, l1), AppBuiltin (b2, l2) when List.length l1 = List.length l2 ->
      if Builtin.equal b1 b2 then
        ...
        fail_ "%a and %a are not compatible (subst {@[%a@]})" Builtin.pp b1 Builtin.pp b2 Subst.pp subst
...
let rec erase t =
  ...
  | Bind (b, v, t) -> STerm.bind b [ STerm.V (Var.to_string v), Some (erase (Var.ty v)) ] (erase t)
