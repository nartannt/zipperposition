...
module Step = struct
  ...
  let rec has_ho_step p =
    ...
    | Simplification (_, tags) | Inference (_, tags) ->
      List.mem Tag.T_ho tags || List.exists has_ho_step (List.map (fun par -> (Parent.proof par).step) p.parents)
    | _ -> false
      ...
      ~attrs_e:(fun (r, s, infos) ->
        ...
        let label =
          if s = None && infos = [] then
            ...
            _to_str_escape "@[<v>%s%a%a@]@." (Rule.name r) (CCFormat.some pp_subst) s Step.pp_infos infos
        in
