  ...
  let pp_tstp out (k, parents) =
      ...
      let pp_step status out (rule, parents) =
          match parents with
              ...
              | _ :: _ ->
                  Format.fprintf out "inference(@[%a,@ [status(%s)],@ [@[%a@]]@])" Rule.pp rule status
                    pp_parents parents
      in
          match k with
...
module Result = struct
  ...
  let pp_in o out (Res (r, x)) =
      match r.res_of_exn x with None -> assert false | Some x -> r.res_pp_in o out x
  ...
  let make_tc (type a) ~of_exn ~to_exn ~compare ~to_form ?(to_form_subst = fun ~ctx:_ _ _ -> assert false)
      ~pp_in ?name ?(is_stmt = false) ?(is_dead_cl = fun () -> false) ?(flavor = fun _ -> `Vanilla) () :
      a result_tc =
...
module Step = struct
  ...
  let pp out step =
      match kind step with
          ...
          | Define (id, src) ->
              Format.fprintf out "@[<2>define %a@ %a%a%a@]" ID.pp id Src.pp src pp_parents (parents step)
                pp_infos step.infos
          | Inference _ | Simplification _ | Esa _ ->
              Format.fprintf out "@[<hv2>%a%a%a@]" Kind.pp (kind step) pp_parents (parents step) pp_infos
                step.infos
end
  ...
  let pp_tstp out proof =
      ...
      let declare_combinators () =
          ...
          in
              List.iter
                (fun (comb, name, decl) ->
                  Format.fprintf out "thf(@[@[%a@], type, @[%s@]: @[%s@]@]).@." Util.pp_str_tstp
                    (name ^ "_type") (Builtin.TPTP.to_string comb) decl)
                decls
      in
              ...
              |> Iter.iter (fun t ->
                     match F.Ty.view t with
                         | F.Ty.Ty_app (hd, args) when not @@ ID.Set.mem hd !types ->
                             let ty =
                                 F.Ty.( ==> ) (CCList.replicate (List.length args) F.Ty.tType) F.Ty.tType
                             in
      ...
      traverse ~order:`DFS proof (fun p ->
          ...
          let parents =
              List.map (fun p -> `Name (name ~namespace @@ Parent.proof p)) (Step.parents @@ step p)
          in
          let role = "plain" in
          (* TODO *)
          let pp_infos out = function
