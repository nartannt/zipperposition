...
let iterate_one ~counter types_w prefix_types return_type i type_ul =
    ...
    let inner_lambda_expr =
        ...
        in
        let matrix =
            T.app bvar_ul_under_w (vars_y |> List.map (fun y -> T.app y (bvars_u_under_w @ bvars_w)))
        in
    ...
    let positions =
        ...
        |> OSeq.flat_map (fun v ->
               let prefix_types, return_type = Type.open_fun (HVar.ty v) in
                   ...
                   |> (fun l ->
                        if not flex_same then l else List.filter (fun (_, _, _, _, ty) -> Type.is_fun ty) l)
                   |> OSeq.of_list)
    in
...
let find_disagreement s t =
    ...
    and find_disagreement_aux s t =
        if T.is_type s then if Term.equal s t then OSeq.empty else OSeq.return ((s, t), [])
          ...
          match (T.view s, T.view t) with
              | T.App (f, ss), T.App (g, tt) when T.equal f g && not (T.is_var f) -> find_disagreement_l ss tt
    ...
    let s = find_disagreement_aux s t in
        match OSeq.find (fun ((_, _), l) -> CCList.is_empty l) s with
            | Some d -> Some d
            | None -> ( match s () with OSeq.Nil -> None | OSeq.Cons (d, _) -> Some d)
