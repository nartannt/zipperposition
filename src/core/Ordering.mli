...
(** Term orderings are well-founded orderings on terms, that (usually) have some nice properties for
    Superposition, such as being total on ground terms, stable by substitution, and stable by context
    (monotonic).
...
(** Returns false for two terms t and s if for any ground substitution θ the ordering of tθ vs sθ cannot change
    when appending arguments. This function is allowed to overapproximate, i.e. we get no information if it
    returns true. *)
...
(** {2 Ordering implementations}
    An ordering is a partial ordering on terms. Several implementations are simplification orderings (compatible
    with substitution, with the subterm property, and monotonic), some other are not. *)
...
(** Takes a function that is going to be run before the chosen order and the order. For example, if the first
    argument returns Eq, then the order is determined by the second argument, and otherwise the result of the
