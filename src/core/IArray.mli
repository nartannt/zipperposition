...
(** An API over arrays that doesn't expose any mutating function. This type ['a IArray.t] benefits from good
    cache locality and fast access to items, but still is functional. However, most operations that create new
    arrays are linear in time (they copy/traverse the whole input). *)
...
(** Take ownership of the given array. Careful, the array must {b NOT} be modified afterwards! *)
