...
let () =
    Printexc.register_printer (function
        | Error msg -> Some (CCFormat.sprintf "@[@{<Red>type inference error@}:@ %s@]" msg)
        | _ -> None)
...
module Ctx = struct
      ...
      match (!r, k) with
          | None, `BindDefault ->
              let ty = ctx.default in
                  Util.debugf ~section 50 "@[<2>specialize type meta_var %a to@ @[%a@]@]" (fun k ->
                      k Var.pp v T.pp ty);
                  r := Some ty
          | None, `Generalize ->
              let v' = Var.copy v in
  ...
  let pp_names out = function
      | [] -> ()
      | l -> Fmt.fprintf out " (did you mean any of [@[%a@]]?)" (Util.pp_list Fmt.string) l
...
let rec infer_ty_ ?loc ctx ty =
    let rec aux ty =
        match PT.view ty with
            ...
            | PT.AppBuiltin (Builtin.Wildcard, []) -> Ctx.fresh_ty_meta_var ctx ~dest:`Generalize () |> T.meta
        ...
        match PT.view t with
            ...
            | PT.AppBuiltin (((Builtin.Eq | Builtin.Neq) as conn), [ a; b ]) -> (
                ...
                let b = infer_rec ?loc ctx b in
                    ...
                    match conn with
                        | Builtin.Eq ->
                            if
                              T.Ty.is_prop (T.ty_exn a) && (CCOpt.is_none (T.head a) || CCOpt.is_none (T.head b))
            ...
            | PT.AppBuiltin (b, l) -> (
                match TyBuiltin.ty b with
                    | None -> error_ ?loc "@[<2>unexpected builtin in@ `@[%a@]`, expected term@]" PT.pp t
                    | Some ty_b ->
                        ...
                        let l =
                            ...
                            then (
                              Util.debugf ~section 50
                                "@[<2>add %d implicit type arguments to@ `@[<1>%a@ (%a)@]`@]" (fun k ->
...
and infer_match ?loc ctx ~ty_matched t data (l : PT.match_branch list) :
                    ...
                    let c_id, c_ty = Ctx.get_id_ ?loc ~attrs:t.PT.attrs ~arity:(List.length vars) ctx s in
                        ...
                        let _vars, ty_s_args, ty_ret_s = T.Ty.unfold c_ty_applied in
                            ...
                            if List.length ty_s_args <> List.length vars then
                              error_ ?loc "constructor `%a`@ expected %d arguments,@ got %d" ID.pp c_id
                                (List.length ty_s_args) (List.length vars);
    ...
    let missing =
        CCList.filter_map
          (fun (id, _, _) ->
            if List.exists (fun (c, _, _) -> ID.equal id c.T.cstor_id) l then None else Some id)
          data
    in
        match missing with
            | [] -> l
...
let check_vars_rhs ?loc bound rhs =
    ...
    let only_in_rhs = Var.Set.diff vars_rhs bound in
        if not (Var.Set.is_empty only_in_rhs) then
          error_ ?loc "variables @[%a@]@ occur in RHS/cond `@[%a@]`@ but are not bound" Var.Set.pp only_in_rhs
            T.pp rhs
        ...
        in
            ...
            (match of_ with
                | Some id' when not (ID.equal id id') ->
                    error_ ?loc
                      "rule `%a`@ for `%a` has head symbol `%a`@ every rule in the definition of `%a` must \
                       start with `%a`"
                ...
                | SLiteral.Atom (t, _) -> (
                    match (T.head t, of_) with
                        | Some id, Some id' when not (ID.equal id' id) ->
                            error_ ?loc "rule `%a`@ must have `%a` as head symbol, not `%a`" T.pp t ID.pp id'
                              ID.pp id
                        | _ -> ())
                | _ -> ());
...
let read_attrs ~file attrs =
    ...
    and name =
        CCList.find_map
          (function A.A_app ("name", [ (A.A_quoted s | A.A_app (s, [])) ]) -> Some s | _ -> None)
          attrs
    in
        (Proof.Src.from_file ?name file, attrs)

let infer_statement_exn ?(file = "<no file>") ctx st =
    Util.debugf ~section 30 "@[<2>infer types for @{<yellow>statement@}@ `@[%a@]`@]" (fun k ->
        k A.pp_statement st);
...
let infer_statements_exn ?def_as_rewrite ?on_var ?on_undef ?on_shadow ?ctx ?file ~implicit_ty_args seq =
    let ctx =
        match ctx with
            | None -> Ctx.create ?def_as_rewrite ?on_var ?on_undef ?on_shadow ~implicit_ty_args ()
            | Some c -> c
    in
