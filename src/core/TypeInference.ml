           ...
           | PT.V v -> (
              ...
              with Not_found ->
                (match ctx.on_undef with
                   ...
                   | `Guess -> ()
                   | `Warn ->
                      Util.warnf "@[<2>create implicit variable %s@,%a%a@]" v pp_names (find_close_names ctx v)
                        Loc.pp_opt loc);
                let v' = mk_fresh v in
            ...
            match TyBuiltin.ty b with
               ...
               | Some ty_b ->
                  ...
                  let l =
                     ...
                     then (
                       Util.debugf ~section 50 "@[<2>add %d implicit type arguments to@ `@[<1>%a@ (%a)@]`@]"
      ...
      match missing with
         ...
         | _ :: _ ->
            if !error_on_incomplete_match_ then
              ...
              Util.warnf "%a@,missing cases in match: (@[%a@])@ :in `%a`" Loc.pp_opt loc (Util.pp_list ID.pp)
                missing PT.pp t;
              l)
...
let check_vars_rhs ?loc bound rhs =
   ...
   let only_in_rhs = Var.Set.diff vars_rhs bound in
      if not (Var.Set.is_empty only_in_rhs) then
        error_ ?loc "variables @[%a@]@ occur in RHS/cond `@[%a@]`@ but are not bound" Var.Set.pp only_in_rhs T.pp
          rhs
      ...
      in
         ...
         (match of_ with
            | Some id' when not (ID.equal id id') ->
               error_ ?loc
                 "rule `%a`@ for `%a` has head symbol `%a`@ every rule in the definition of `%a` must start \
                  with `%a`"
