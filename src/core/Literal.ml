...
let apply_subst_no_simp renaming subst (lit, sc) =
  match lit with
  | Equation (l, r, sign) -> Equation (S.FO.apply renaming subst (l, sc), S.FO.apply renaming subst (r, sc), sign)
  | True | False -> lit
...
let fold_terms ?(position = Position.stop) ?(vars = false) ?(var_args = true) ?(fun_bodies = true) ?ty_args
  ...
  let at_term ~pos t =
    if subterms then
      T.all_positions ~filter_formula_subterms ?ty_args ~vars ~var_args ~fun_bodies ~pos t k
