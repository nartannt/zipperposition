...
(** An array of literals is one of the major components of a clause.
    ...
    It defines (implicitly) a scope for its variables; applying a substitution should always be done with the
    same {!Subst.Renaming.t} for all literals in the array.

    This also provides printers, comparison, matching, positions, simplifications, etc. for such arrays of
    literals. *)
...
(** Variant checking (alpha-equivalence). It can reorder literals to do its check, so that might be
    computationnally expensive (a bit like subsumption). *)
...
module View : sig
  ...
  (** The following functions will raise [Invalid_argument] if the position is not valid or if the literal isn't
      what's asked for *)
...
(** fold f over all literals sides, with their positions. NB: REPORTED SIGN IS THE SAME AS IF Lit.is_pos WAS
    CALLED! f is given [(left side, right side, sign, position of left side)] if [ord] is present, then only the
    max side of an oriented equation will be visited, otherwise they will both be explored. if [both = true],
    then both sides of a non-oriented equation will be visited, otherwise only one side. if [sign = true], then
    only positive equations are visited; if it's [false], only negative ones; if it's not defined, both. *)
