            ...
            let cmp_branch b1 b2 =
                match (b1, b2) with
                    | Match_case (s1, vars1, rhs1), Match_case (s2, vars2, rhs2) ->
                        CCOrd.(
                          String.compare s1 s2 <?> (list compare_var, vars1, vars2) <?> (compare, rhs1, rhs2))
...
let rec pp out t =
    match t.term with
        ...
        | Bind (s, vars, t') ->
            Format.fprintf out "@[<2>%a @[%a@].@ %a@]" Binder.pp s (Util.pp_list ~sep:" " pp_typed_var) vars pp
              t'
...
module TPTP = struct
  ...
  let rec pp out t =
      match t.term with
          ...
          | Bind (s, vars, t') ->
              Format.fprintf out "@[<2>%a[@[%a@]]:@ %a@]" Binder.TPTP.pp s (Util.pp_list ~sep:"," pp_typed_var)
                vars pp_surrounded t'
          | With (_, u) -> pp out u (* skip bindings *)
          | Ite (c, if_t, if_f) ->
              Format.fprintf out "@[$ite(@[%a@], @[%a@], @[%a@])@]" pp_surrounded c pp_surrounded if_t
                pp_surrounded if_f
...
module TPTP_THF = struct
  ...
  let rec pp out t =
      match t.term with
          ...
          | Bind (s, vars, t') ->
              Format.fprintf out "@[%a[@[%a@]]:@ %a@]" Binder.TPTP.pp s (Util.pp_list ~sep:"," pp_typed_var)
      ...
      match t.term with
          ...
          | Match (u, l) ->
              let pp_branch out = function
                  | Match_default rhs -> Format.fprintf out "_ -> %a" pp rhs
                  | Match_case (c, vars, rhs) ->
                      Format.fprintf out "@[<2>case@ %s %a ->@ %a@]" c (Util.pp_list ~sep:" " pp_var) vars pp
                        rhs
              in
                  Format.fprintf out "@[<hv>@[<hv2>match %a with@ @[<hv>%a@]@]@ end@]" pp u
                    (Util.pp_list ~sep:" | " pp_branch) l
          | Bind (s, vars, t') ->
              Format.fprintf out "@[<2>%a @[%a@].@ @[%a@]@]" Binder.ZF.pp s (Util.pp_list ~sep:" " pp_typed_var)
                vars pp_surrounded t'
