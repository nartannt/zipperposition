...
let rec compare t1 t2 =
  ...
  | Let (l1, t1), Let (l2, t2) -> CCOrd.(compare t1 t2 <?> (list (pair compare_var compare), l1, l2))
  | With (l1, v1), With (l2, v2) -> CCOrd.(list (pair compare_var compare) l1 l2 <?> (compare, v1, v2))
  | Var _, _
  | Const _, _
  | Ite _, _
  | Match _, _
...
let rec hash t =
  ...
  | Bind (s, v, t') -> Hash.combine4 40 (Binder.hash s) (Hash.list hash_ty_var v) (hash t')
  | Ite (a, b, c) -> Hash.combine4 50 (hash a) (hash b) (hash c)
  | Match (u, _) -> Hash.combine2 60 (hash u)
  | Let (_, u) -> Hash.combine2 70 (hash u)
  | Record (l, rest) -> Hash.combine3 80 (Hash.opt hash_var rest) (Hash.list (Hash.pair Hash.string hash) l)
  | With (l, v) -> Hash.combine3 90 (Hash.list (Hash.pair hash_var hash) l) (hash v)
...
let bind ?loc s v l =
  match v, l with
  | [], _ -> l
  | _ :: _, { term = Bind (s', v', l'); _ } when s = s' -> make_ ?loc (Bind (s, v @ v', l')) (* flatten *)
...
let rec pp out t =
  ...
  | AppBuiltin (Builtin.Arrow, [ ret; a ]) -> Format.fprintf out "@[<2>%a@ → %a@]" pp_inner a pp_inner ret
  ...
  | AppBuiltin (b, l) when Builtin.is_infix b && List.length l > 0 -> pp_infix_ b out l
  | AppBuiltin (s, l) -> Format.fprintf out "%a(@[<2>%a@])" Builtin.pp s (Util.pp_list ~sep:", " pp_inner) l
  | App (s, l) -> Format.fprintf out "@[<2>%a %a@]" pp s (Util.pp_list ~sep:" " pp_inner) l
  | Bind (s, vars, t') ->
  ...
  | Match (u, l) ->
    ...
    in
    Format.fprintf out "@[<hv>@[<hv2>match %a with@ @[<hv>%a@]@]@ end@]" pp u (Util.pp_list ~sep:" | " pp_branch)
      l
  | Record (l, None) ->
    Format.fprintf out "{@[<hv>%a@]}" (Util.pp_list (fun out (s, t') -> Format.fprintf out "%s:%a" s pp t')) l
  | Record (l, Some r) ->
    Format.fprintf out "{@[<hv>%a@ | %a@]}" (Util.pp_list (Util.pp_pair ~sep:":" CCFormat.string pp)) l pp_var r

and pp_inner out t =
  match view t with
  | AppBuiltin (_, _ :: _) | App _ | Bind _ -> Format.fprintf out "(@[%a@])" pp t (* avoid ambiguities *)
  | _ -> pp out t
...
and pp_infix_ b out l =
  ...
  | [] -> assert false
  | [ t ] -> pp_inner out t
  | t :: l' -> Format.fprintf out "@[<hv>%a@ @[<hv>%a@ %a@]@]" pp_inner t Builtin.pp b (pp_infix_ b) l'
...
module TPTP = struct
  ...
  let rec pp out t =
    ...
    | AppBuiltin (Builtin.Or, l) ->
      if CCList.is_empty l then
        ...
        Util.pp_list ~sep:" | " pp_surrounded out l
    | AppBuiltin (Builtin.Not, [ a ]) -> Format.fprintf out "@[<1>~@,@[%a@]@]" pp_surrounded a
    | AppBuiltin (Builtin.Imply, [ a; b ]) -> Format.fprintf out "@[%a =>@ %a@]" pp_surrounded a pp_surrounded b
    | AppBuiltin (Builtin.Xor, [ a; b ]) -> Format.fprintf out "@[%a <~>@ %a@]" pp_surrounded a pp_surrounded b
    | AppBuiltin (Builtin.Equiv, [ a; b ]) -> Format.fprintf out "@[%a <=>@ %a@]" pp_surrounded a pp_surrounded b
    | AppBuiltin (Builtin.Eq, ([ _; a; b ] | [ a; b ])) ->
      Format.fprintf out "@[%a =@ %a@]" pp_surrounded a pp_surrounded b
    | AppBuiltin (Builtin.Neq, ([ _; a; b ] | [ a; b ])) ->
      Format.fprintf out "@[%a !=@ %a@]" pp_surrounded a pp_surrounded b
    | AppBuiltin (Builtin.Arrow, [ ret; a ]) -> Format.fprintf out "(@[<2>%a >@ %a@])" pp a pp ret
    | AppBuiltin (Builtin.Arrow, ret :: l) ->
    ...
    | App (s, l) -> Format.fprintf out "@[%a(@[%a@])@]" pp s (Util.pp_list ~sep:"," pp) l
    | Bind (s, vars, t') ->
      Format.fprintf out "@[<2>%a[@[%a@]]:@ %a@]" Binder.TPTP.pp s (Util.pp_list ~sep:"," pp_typed_var) vars
        pp_surrounded t'
    | With (_, u) -> pp out u (* skip bindings *)
    | Ite (c, if_t, if_f) ->
      Format.fprintf out "@[$ite(@[%a@], @[%a@], @[%a@])@]" pp_surrounded c pp_surrounded if_t pp_surrounded if_f
  ...
  and pp_typed_var out (v, o) =
    match o with
    | None -> pp_var out v
    | Some { term = AppBuiltin (Builtin.Term, []); _ } -> pp_var out v (* implicit type *)
...
module TPTP_THF = struct
  ...
  let rec pp out t =
    ...
    | Var v -> pp_var out v
    | Const s -> pp_id out s
    | AppBuiltin (Builtin.And, l) when List.length l >= 2 -> Util.pp_list ~sep:" & " pp_force_surrounded out l
    | AppBuiltin (Builtin.Or, l) when List.length l >= 2 -> Util.pp_list ~sep:" | " pp_force_surrounded out l
    | AppBuiltin (Builtin.Not, [ a ]) -> Format.fprintf out "@[~@[%a@]@]" pp_force_surrounded a
    ...
    | AppBuiltin (Builtin.Arrow, [ ret; a ]) -> Format.fprintf out "@[%a >@ %a@]" pp_surrounded a pp ret
    | AppBuiltin (Builtin.Arrow, ret :: l) ->
    ...
    | AppBuiltin (s, l) ->
      (* Format.fprintf out "@[%a(@[%a@])@]" Builtin.TPTP.pp s (Util.pp_list ~sep:", " pp_surrounded) l *)
      Format.fprintf out "@[%a@ @@@[%a@]@]" Builtin.TPTP.pp s (Util.pp_list ~sep:" @ " pp_surrounded) l
    | App (s, l) -> Format.fprintf out "@[%a@ @@@ @[%a@]@]" pp s (Util.pp_list ~sep:" @ " pp_surrounded) l
    | Bind (s, vars, t') ->
  ...
  and pp_surrounded out t =
    match t.term with
    | AppBuiltin (_, _ :: _) | App (_, _) | Bind _ -> Format.fprintf out "( @[%a@] )" pp t
    | _ -> pp out t
...
module ZF = struct
  ...
  let rec pp out t =
    ...
    | AppBuiltin (Builtin.And, l) -> Format.fprintf out "@[<hv>%a@]" (Util.pp_list ~sep:" && " pp_surrounded) l
    | AppBuiltin (Builtin.Or, l) -> Format.fprintf out "@[<hv>%a@]" (Util.pp_list ~sep:" || " pp_surrounded) l
    | AppBuiltin (Builtin.Not, [ a ]) -> Format.fprintf out "@[<2>~@ @[%a@]@]" pp_surrounded a
    | AppBuiltin (Builtin.Imply, [ a; b ]) ->
      Format.fprintf out "@[@[%a@]@ => @[%a@]@]" pp_surrounded a pp_surrounded b
    | AppBuiltin (Builtin.Xor, [ a; b ]) ->
    ...
    | AppBuiltin (Builtin.Neq, [ _; a; b ]) ->
      Format.fprintf out "@[@[%a@]@ != @[%a@]@]" pp_surrounded a pp_surrounded b
    | AppBuiltin (Builtin.Arrow, [ ret; a ]) -> Format.fprintf out "@[@[%a@] ->@ @[%a@]@]" pp a pp ret
    | AppBuiltin (Builtin.Arrow, ret :: l) ->
      Format.fprintf out "@[%a ->@ %a@]" (Util.pp_list ~sep:" -> " pp_surrounded) l pp_surrounded ret
    ...
    | App (s, l) -> Format.fprintf out "@[<2>%a@ %a@]" pp_surrounded s (Util.pp_list ~sep:" " pp_surrounded) l
    | Ite (a, b, c) -> Format.fprintf out "@[<2>if %a@ then %a@ else %a@]" pp a pp b pp c
    | Let (l, u) ->
      let pp_binding out (v, t) = Format.fprintf out "@[%a := %a@]" pp_var v pp t in
      Format.fprintf out "@[<2>let %a@ in %a@]" (Util.pp_list ~sep:" and " pp_binding) l pp u
  ...
  and pp_surrounded out t =
    match t.term with
    | AppBuiltin (_, _ :: _) | App _ | Ite _ | Bind _ -> Format.fprintf out "(@[%a@])" pp t
    | _ -> pp out t
