        ...
        let t = norm_deref subst (t, scope) in
            match T.view t with
                ...
                | T.App (hd, args) -> (
                    ...
                    match aux ~depth ~under_var hd with
                        ...
                        | Some hd' ->
                            CCOpt.map
                              (fun args' ->
                                if T.same_l args args' && T.equal hd hd' then t else T.app hd' args')
                ...
                | T.Var _ ->
                    ...
                    if T.equal var t then if under_var || Type.is_fun (T.ty t) then None else raise NotUnif
                    else Some t
                | T.Fun _ -> (
                    ...
                    let depth_inc = List.length pref_tys in
                        match aux ~depth:(depth + depth_inc) ~under_var body' with
                            | None -> None
                            | Some t' -> if T.equal t' t then Some t else Some (T.fun_l pref_tys t'))
                | T.DB i when i >= depth -> if under_var then None else raise NotUnif
                | _ -> Some t
