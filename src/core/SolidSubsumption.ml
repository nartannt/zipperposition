...
struct
  ...
  let rec pp out =
      ...
      let sepw = CCFormat.return "@ " in
          function
              ...
              | Fun (ty, repls) -> CCFormat.fprintf out "|l@[%a@].@[%a@]|" Type.pp ty pp repls
              | Repl repls -> CCFormat.fprintf out "{r:@[%a@]}" (TS.pp ~pp_sep:sepw T.pp) repls
  ...
  let cover t solids : multiterm =
      ...
      let rec aux ~depth s_args t : multiterm =
          ...
          match T.view t with
              | AppBuiltin (hd, args) -> app_builtin hd (List.map (aux ~depth s_args) args) db_hits
              | App (hd, args) ->
              ...
              | Fun _ ->
                  ...
                  let res = aux ~depth:(depth + d_inc) s_args' body in
                      fun_l ty_args res
              | DB i when i >= depth -> if TS.is_empty db_hits then raise SolidMatchFail else repl db_hits
      ...
      let rec aux subst l r =
          if T.is_type l then
            ...
            match T.view l with
                ...
                | App (hd, args) when T.is_var hd -> refine_subst_w_term subst (T.as_var_exn hd) (cover r args)
                | App (hd, args) -> (
              ...
              match target with
                  | L.Equation (lhs', rhs', sign') ->
                      ...
                      if sign = sign' then (
                        ...
                        with SolidMatchFail -> ())
                  | _ -> ())
          | L.True -> ( match target with L.True -> k subst | _ -> ())
          | L.False -> ( match target with L.False -> k subst | _ -> ())
