  ...
  let do_unif ~bind_cnt ~hits_cnt problem subst unifscope =
     ...
     let rec aux ?(root = false) subst problem =
        ...
        match problem with
           ...
           | (lhs, rhs, flag) :: rest -> (
              match
                PatternUnif.unif_simple ~subst ~scope:unifscope (T.of_ty (T.ty lhs)) (T.of_ty (T.ty rhs))
              with
                 | None -> OSeq.empty
                 | Some subst -> (
                                     ...
                                     try Some (alg (lhs, unifscope) (rhs, unifscope) subst) with
                                        | P.NotInFragment ->
                                           Util.debugf ~section 1
                                             "@[%a@] =?= @[%a@] (subst @[%a@]) is not in fragment" (fun k ->
                                               k T.pp lhs T.pp rhs Subst.pp subst);
                                           None
                                        | P.NotUnifiable ->
                                           Util.debugf ~section 1 "@[%a@] =?= @[%a@] (subst @[%a@]) is not unif"
                              ...
                              in
                                 match mgu with
                                    ...
                                    | None ->
                                       ...
                                       let all_oracles =
                                          P.pb_oracle (body_lhs, unifscope) (body_rhs, unifscope) flag subst
                                            unifscope
                                       in
                   ...
                   (fun subst ->
                     ...
                     let r = norm @@ S.FO.apply Subst.Renaming.none subst t1s in
                        if not (T.equal l r && Type.equal (Term.ty l) (Term.ty r)) then (
                          ...
                          CCFormat.printf "new:@[%a:%a@]@.=?=@.@[%a:%a@]@." T.pp l Type.pp (T.ty l) T.pp r
                            Type.pp (T.ty r);
                          assert false);
                        subst)
                   opt)
