...
let map_def ~form:fform ~term:fterm ~ty:fty d =
    {
      d with
      def_ty = fty d.def_ty;
      def_rules = List.map (map_def_rule ~form:fform ~term:fterm ~ty:fty) d.def_rules;
    }
...
let as_defined_cst id =
    ID.payload_find id ~f:(function
        | Rewrite.Payload_defined_cst c -> Some (Rewrite.Defined_cst.level c, Rewrite.Defined_cst.rules c)
        | _ -> None)
...
let conv_rule_i ~proof (r : _ def_rule) =
    match r with
        | Def_term { id; ty; args; rhs; _ } ->
            ...
            let proof =
                Proof.S.mk_f (Proof.Step.simp ~rule:(Proof.Rule.mk "simplify_rw_rule") proof_parents) form
            in
...
module Seq = struct
  ...
  let forms st =
      let forms_def = function Def_term { as_form; _ } -> [ as_form ] | Def_form { as_form; _ } -> as_form in
          match view st with
              ...
              | Def l -> Iter.of_list l |> Iter.flat_map_l (fun d -> d.def_rules) |> Iter.flat_map_l forms_def
              | _ -> to_iter st |> Iter.filter_map (function `Form f -> Some f | _ -> None)
  ...
  let symbols st =
      ...
      |> Iter.flat_map (function
             ...
             | `Form f -> Iter.of_list f |> Iter.flat_map SLiteral.to_iter |> Iter.flat_map Term.Seq.symbols
...
let pp ppf ppt ppty out st =
    ...
    let pp_attrs = UntypedAST.pp_attrs in
        match st.view with
            | TyDecl (id, ty) -> fpf out "@[<2>val%a %a :@ @[%a@]@]." pp_attrs attrs ID.pp id ppty ty
            | Def l ->
            ...
            | Rewrite d -> (
                match d with
                    ...
                    | Def_form { lhs; rhs; polarity = pol; _ } ->
                        let op = match pol with `Equiv -> "<=>" | `Imply -> "=>" in
                            fpf out "@[<2>rewrite%a@ @[%a@]@ %s @[%a@]@]." pp_attrs attrs (SLiteral.pp ppt) lhs
                              op (Util.pp_list ~sep:" && " ppf) rhs)
            | Data l ->
            ...
            | Lemma l -> fpf out "@[<2>lemma%a@ @[%a@]@]." pp_attrs attrs (Util.pp_list ~sep:" && " ppf) l
...
let sine_axiom_selector ?(ignore_k_most_common_symbols = None) ?(take_conj_defs = true)
    ?(take_only_defs = false) ?(trim_implications = false) ?(depth_start = 1) ?(depth_end = 3)
    ...
    let symset_of_axs ~trim_implications ?(is_goal = false) axs =
        List.fold_left
          (fun acc c -> ID.Set.union acc (symset_of_ax ~trim_implications ~is_goal c))
          ID.Set.empty axs
    in
    ...
    let ids_to_defs_compute defs =
        let rec aux map d =
            ...
            match view d with
                | Def l -> List.fold_left (fun map { def_id; _ } -> update_map map def_id d) map l
                | Rewrite r -> (
                    match r with
...
module ZF = struct
  ...
  let pp ppf ppt ppty out st =
      ...
      let pp_attrs = UntypedAST.pp_attrs_zf in
          match st.view with
              | TyDecl (id, ty) -> fpf out "@[<2>val%a %a :@ @[%a@]@]." pp_attrs attrs ID.pp_zf id ppty ty
              | Def l ->
                  fpf out "@[<2>def%a@ %a@]." pp_attrs attrs (Util.pp_list ~sep:" and " (pp_def ppf ppt ppty)) l
              | Rewrite d -> (
                  match d with
                      | Def_term { vars; id; args; rhs; _ } ->
                          fpf out "@[<2>rewrite%a@ @[<2>%a@[%a %a@]@ = @[%a@]@]@]." pp_attrs attrs pp_vars vars
                            ID.pp_zf id (Util.pp_list ~sep:" " ppt) args ppt rhs
                      | Def_form { vars; lhs; rhs; polarity = pol; _ } ->
                          let op = match pol with `Equiv -> "<=>" | `Imply -> "=>" in
                              fpf out "@[<2>rewrite%a@ @[<2>%a@[%a@]@ %s @[%a@]@]@]." pp_attrs attrs pp_vars
                                vars (SLiteral.ZF.pp ppt) lhs op (Util.pp_list ~sep:" && " ppf) rhs)
              | Data l ->
          ...
          match st.view with
              ...
              | NegatedGoal (_, l) ->
                  ...
                  in
                      List.iter
                        (fun f ->
                          fpf out "@[<2>thf(%a, %s,@ (@[%a@]),@ @[%a@])@]." pp_name name role ppf f
                            Proof.Kind.pp_tstp
                      ...
                      | Def_form { lhs; rhs; polarity = pol; _ } ->
                          let op = match pol with `Equiv -> "=" | `Imply -> "=>" in
                              fpf out "@[<2>thf(%a, axiom,@ %a %s@ (@[%a@]))@]." pp_name name
                                (SLiteral.TPTP.pp ppt) lhs op (Util.pp_list ~sep:" & " ppf) rhs)
            ...
            let proof = as_proof_c st in
                List.iter
                  (fun d ->
                    ...
                    and cstors =
                        List.map (fun (c, ty, args) -> Ind_ty.mk_constructor c ty args) d.data_cstors
                    in
                  ...
                  (fun d ->
                    ...
                    and cstors =
                        List.map
                          (fun (c, ty, args) ->
                            let args =
                                List.map (fun (ty, (p_id, p_ty)) -> (conv_ty ty, (p_id, conv_ty p_ty))) args
                            in
...
let get_rw_rule ?weight_incr:(w_i = 1000000) c =
    let distinct_free_vars l =
        l
        |> List.map (fun t ->
               Term.as_var t |> fun v -> match v with Some x -> Some (HVar.id x) | None -> None)
        |> OptionSet.of_list
