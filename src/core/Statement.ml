   ...
   match d with
      | Def_term { vars; id; ty; args; rhs; as_form } ->
         let vars = List.map (Var.update_ty ~f:fty) vars in
            Def_term
              { vars; id; ty = fty ty; args = List.map fterm args; rhs = fterm rhs; as_form = fform as_form }
         ...
         match view d with
            ...
            | Rewrite r -> (
               match r with
                  ...
                  | Def_form { lhs; rhs; polarity = pol; _ } -> (
                     match lhs with
                        | Atom (t, _) -> ( match TST.head t with Some hd -> update_map map hd d | None -> map)
                        | _ -> map))
            | _ -> map
      in
        ...
        match st.view with
           ...
           | Rewrite d -> (
              match d with
                 ...
                 | Def_form { lhs; rhs; polarity = pol; _ } ->
                    let op = match pol with `Equiv -> "=" | `Imply -> "=>" in
                       fpf out "@[<2>thf(%a, axiom,@ %a %s@ (@[%a@]))@]." pp_name name (SLiteral.TPTP.pp ppt) lhs
      ...
      | Def l ->
         ...
         let ids_and_levels =
            ...
            |> List.map (fun { def_id; def_rules; _ } ->
                   let lev =
                      Iter.of_list def_rules |> Iter.map level_of_rule |> Iter.max |> CCOpt.get_or ~default:0
              ...
              let t1, t2 = (Lambda.snf @@ Term.app t1 fresh_vars, Lambda.snf @@ Term.app t2 fresh_vars) in
                 if Term.weight t2 - Term.weight t1 <= w_i then
                   match conv_terms_rw t1 t2 with
                      | Some rhs -> Some rhs
                      | None -> if Term.weight t1 - Term.weight t2 <= w_i then conv_terms_rw t2 t1 else None
