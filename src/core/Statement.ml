...
let pp ppf ppt ppty out st =
  ...
  | Def l -> fpf out "@[<2>def%a@ %a@]." pp_attrs attrs (Util.pp_list ~sep:" and " (pp_def ppf ppt ppty)) l
  | Rewrite d ->
    (match d with
    ...
    | Rewrite d ->
      ...
      | Def_form { vars; lhs; rhs; polarity = pol; _ } ->
        let op =
          ...
          | `Imply -> "=>"
        in
        fpf out "@[<2>rewrite%a@ @[<2>%a@[%a@]@ %s @[%a@]@]@]." pp_attrs attrs pp_vars vars (SLiteral.ZF.pp ppt)
          lhs op (Util.pp_list ~sep:" && " ppf) rhs)
    | Data l ->
