...
(** Term traversal in prefix order. This is akin to lazy transformation to a flatterm. *)
...
let rec open_term ~stack t =
  ...
  | T.Var _ | T.DB _ | T.AppBuiltin _ | T.Fun _ | T.Const _ -> Some { cur_term = t; stack = [] :: stack }
  ...
  | T.App (f, _) when T.is_var f -> Some { cur_term = t; stack = [] :: stack } (* higher-order term *)
...
module Make (E : Index.EQUATION) = struct
  ...
  (** get/add/remove the leaf for the given term. The continuation k takes the leaf, and returns a leaf option
      that replaces the old leaf. This function returns the new trie. *)
    ...
    let rec traverse trie iter =
      ...
      | Some i ->
        ...
        | As_star ->
          (match trie.star with
          | None -> ()
          | Some subtrie -> traverse subtrie (next i) (* match "*" against "*" *))
        | As_app (s, _) ->
          (try
    ...
    let rec traverse trie iter =
      ...
      | Some i ->
        ...
        | As_star ->
          (match trie.star with
          | None -> ()
          | Some subtrie -> traverse subtrie (next i) (* match "*" against "*" only *))
        | As_app (s, l) ->
          let arity = List.length l in
          (try
             let subtrie = SIMap.find (s, arity) trie.map in
