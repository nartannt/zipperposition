...
let () =
    let spf = CCFormat.sprintf in
        Printexc.register_printer (function
            ...
            | NotAnInductiveConstructor id -> Some (spf "%a is not an inductive constructor" ID.pp id)
            | _ -> None)
...
let is_inductive_type ty =
    match type_hd ty with Some (I id) -> is_inductive_ty id | Some (B _) | None -> false
...
let as_inductive_type ty =
    match Type.view ty with
        | Type.App (id, l) -> ( match as_inductive_ty id with None -> None | Some ity -> Some (ity, l))
...
let is_rec_ (top : t) : bool =
    ...
    and find_in_ty (seen : t list) (ty : Type.t) =
        match Type.view ty with
            ...
            | Type.App (id, l) -> ID.equal id top.ty_id || List.exists (find_in_ty seen) l
            | Type.Fun (args, ret) -> find_in_ty seen ret || List.exists (find_in_ty seen) args
