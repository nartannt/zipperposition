...
let rec pp_term out (t : term) =
    match t with
        ...
        | Forall (vars, f) -> fpf out "(@[<hv2>forall@ (@[%a@])@ %a@])" (pp_list pp_typed_var) vars pp_term f
...
let pp_stmt out (st : statement) =
    match view st with
        ...
        | Stmt_prove (ty_vars, t) -> fpf out "(@[prove@ %a@])" (pp_par pp_term) (ty_vars, t)
        | Stmt_decl d -> fpf out "(@[declare-fun@ %a@])" (pp_par (pp_fun_decl pp_ty)) (d.fun_ty_vars, d)
        | Stmt_fun_def fr -> fpf out "(@[<2>define-fun@ %a@])" (pp_par pp_fr) (fr.fr_decl.fun_ty_vars, fr)
        | Stmt_fun_rec fr -> fpf out "(@[<2>define-fun-rec@ %a@])" (pp_par pp_fr) (fr.fr_decl.fun_ty_vars, fr)
        | Stmt_funs_rec fsr ->
        ...
        | Stmt_data (tyvars, name, cstors) ->
            ...
            in
                fpf out "(@[<hv2>declare-datatype@ %s@ (par@ (@[%a@])@ (@[<v>%a@])@]))" name (pp_list pp_tyvar)
                  tyvars (pp_list pp_cstor) cstors
        | Stmt_datas (tyvars, l) ->
            let pp_cstor_arg out (sel, ty) = fpf out "(@[%s %a@])" sel pp_ty ty in
            let pp_cstor out c =
                if c.cstor_args = [] then fpf out "(%s)" c.cstor_name
            ...
            let pp_data out (s, cstors) = fpf out "(@[<2>%s@ @[<v>%a@]@])" s (pp_list pp_cstor) cstors in
                fpf out "(@[<hv2>declare-datatypes@ (@[%a@])@ (@[<v>%a@])@])" (pp_list pp_tyvar) tyvars
                  (pp_list pp_data) l
