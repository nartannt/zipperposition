...
let of_decls decls =
  ...
  let read_info info =
    ...
    | A.GNode
        ("inference", [ A.GString rule; A.GList [ A.GNode ("status", [ A.GString status ]) ]; A.GList parents ])
      ->
      ...
      | InferClause (c, (lazy step)) ->
        ...
        let status =
          if step.esa then
            ...
            "thm"
        in
        Format.fprintf out "cnf(%a, plain, %a, inference('%s', [status(%s)], [%a])).\n" A.pp_name id _pp_clause c
          step.rule status (CCFormat.array CCFormat.string) ids)
