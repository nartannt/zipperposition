...
let equal p1 p2 =
    match (p1, p2) with
        ...
        | InferForm (f1, (lazy step1)), InferForm (f2, (lazy step2)) -> step1.id = step2.id && T.equal f1 f2
...
let is_dag proof =
    ...
    let rec check_proof proof =
        if StepTbl.mem closed proof then () (* ok *)
          ...
          (match proof with
              | InferClause (_, (lazy step)) | InferForm (_, (lazy step)) -> Array.iter check_proof step.parents
              | Axiom _ | Theory _ -> ());
...
let of_decls decls =
    ...
    let read_info info =
        match info with
            ...
            | A.GNode
                ( "inference",
                  [ A.GString rule; A.GList [ A.GNode ("status", [ A.GString status ]) ]; A.GList parents ] ) ->
                    ...
                    match read_info info with
                        ...
                        | `Parents (rule, esa, parents) ->
                            ...
                            let p = InferForm (f, step) in
                                add_step name p)
                | A.TypeDecl _ | A.FOF _ | A.CNF _ | A.TFF _ | A.THF _ | A.Include _ | A.IncludeOnly _
                | A.NewType _ ->
                    ())
          decls;
        match !root with
...
let _pp_clause out c =
    match c with
        | [] -> T.pp out T.false_
        | _ -> Format.fprintf out "@[%a@]" (Util.pp_list ~sep:" | " (SLiteral.pp T.pp)) c
            ...
            | InferForm (f, (lazy step)) ->
                ...
                let kind = if _form_is_fof f then "fof" else "tff" in
                    Format.fprintf out "%s(%a, plain, %a, inference('%s', [status(%s)], [%a])).\n" kind
                      A.pp_name id T.pp (T.close_all Binder.Forall f) step.rule status
            ...
            | InferClause (c, (lazy step)) ->
                ...
                let status = if step.esa then "esa" else "thm" in
                    Format.fprintf out "cnf(%a, plain, %a, inference('%s', [status(%s)], [%a])).\n" A.pp_name id
                      _pp_clause c step.rule status (CCFormat.array CCFormat.string) ids)

let pp0 out proof =
    match proof with
        | Axiom (f, n) -> Format.fprintf out "axiom(%s, %s)" f n
        | Theory s -> Format.fprintf out "theory(%s)" s
        | InferClause (c, _) -> Format.fprintf out "proof for %a (id %a)" _pp_clause c A.pp_name (get_id proof)
        | InferForm (f, _) -> Format.fprintf out "proof for %a (id %a)" T.pp f A.pp_name (get_id proof)
