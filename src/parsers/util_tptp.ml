  ...
  try
    ...
    List.iter
      (fun decl ->
        ...
        | (A.CNF _ | A.FOF _ | A.TFF _ | A.THF _ | A.TypeDecl _ | A.NewType _), None -> Queue.push decl q
...
let to_ast st =
  let conv_form name role f =
    ...
    | A.R_definition when !enable_def_as_rewrite && looks_like_def f ->
      ...
      UA.rewrite ~attrs f
    | A.R_definition | A.R_axiom | A.R_hypothesis | A.R_assumption | A.R_theorem | A.R_plain | A.R_finite _
    | A.R_type | A.R_unknown ->
      UA.assert_ ~attrs f
  ...
  | A.TFF (name, role, f, _) | A.THF (name, role, f, _) | A.FOF (name, role, f, _) -> conv_form name role f
  | A.CNF (name, role, f, _) ->
